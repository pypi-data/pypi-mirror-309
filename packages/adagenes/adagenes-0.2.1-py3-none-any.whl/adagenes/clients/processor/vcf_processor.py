            ...
            for line in infile:
                ...
                if len(fields) >= 7:
                    ...
                    if (len(variants) >= variant_batch_size) or (len(vcf_lines) >= line_batch_size):
                        ...
                        #    vcf_lines = ag.LiftoverClient(genome_version=genome_version).process_data(vcf_lines,
                        #                                                                              target_genome="hg38")
                        #print("variant size ",len(variants))

                        vcf_lines = magic_obj.process_data(vcf_lines)
                        #print(output_format)
                        #vcf_lines = magic_obj.process_vcf_chunk(vcf_lines, variants, outfile, *kwargs)
                        #for line_2 in vcf_lines.keys():
                        #    print(vcf_lines[line_2], file=outfile)
