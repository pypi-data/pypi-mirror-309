...
from briton.types import InferParams
...
class Model:
    ...
    async def predict(self, model_input: Dict[str, Any], request: fastapi.Request):
        ...
        resp_iter = await self._infer(briton_request, model_input, stream_predicate)
    ...
    async def _infer(self, briton_request: InferenceRequest, model_input: dict, stream: bool):
        if self._infer_adapter is not None:
            infer_params = InferParams(
                briton_stub=self._stub,
                briton_request=briton_request,
                model_input=model_input,
                generate_request_id=self._calc_request_id,
                tokenizer=self._tokenizer,
                stream=stream,
            )
            return await self._infer_adapter(infer_params)
