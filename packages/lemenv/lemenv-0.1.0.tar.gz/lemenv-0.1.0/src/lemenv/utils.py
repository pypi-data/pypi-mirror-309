...
def create_and_activate_venv(name: str):
    if os.name == 'nt':
        # Create a PowerShell script to activate the virtual environment
        ...
        os.remove(activate_script)  # Clean up the PowerShell script
    else:
        # Unix-like systems
        create_cmd = f"python -m venv {name} && source {name}/bin/activate"
        os.system(f"gnome-terminal -- bash -c '{create_cmd}; exec bash' || " +
                  f"xterm -e 'bash -c \"{create_cmd}; exec bash\"' || " +
                  f"terminal -e 'bash -c \"{create_cmd}; exec bash\"'")
