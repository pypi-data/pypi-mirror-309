__author__ = "Jarne Verhaeghe, Jeroen Van Der Donckt, Carlos Arias"
...
import powershap_statistical_analysis_rs
import polars as pl
...
class PowerShap(SelectorMixin, BaseEstimator):
    ...
    def _automatic_fit(
        ...
        while (
            ...
            shaps_df_pl = pl.from_pandas(shaps_df)
            
            processed_shaps_df_pl = powershap_statistical_analysis_rs.powershap_statistical_analysis(
                shaps_df_pl, self.power_alpha, self.power_req_iterations, include_all=self.include_all
            )
            processed_shaps_df = processed_shaps_df_pl.to_pandas().set_index("feature").rename_axis(None)
        ...
        if self.automatic:
            ...
            if self.force_convergence:
                ...
                while (len(converge_df[converge_df.p_value < self.power_alpha]) > 0) & (
                    ...
                    converge_shaps_df_pl = pl.from_pandas(converge_shaps_df)
                    converge_df_pl = powershap_statistical_analysis_rs.powershap_statistical_analysis(
                        converge_shaps_df_pl,
                        self.power_alpha,
                        self.power_req_iterations,
                        include_all=self.include_all,
                    )
                    converge_df = converge_df_pl.to_pandas().set_index("feature").rename_axis(None)
