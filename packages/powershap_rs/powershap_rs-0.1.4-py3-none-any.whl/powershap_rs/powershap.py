...
class PowerShap(SelectorMixin, BaseEstimator):
    ...
    def _automatic_fit(
        ...
        while (
            ...
            shaps_df_pl = pl.DataFrame(shaps_df)
            ...
            print(processed_shaps_df_pl)
            processed_shaps_df = processed_shaps_df_pl.to_pandas()

            # processed_shaps_df = (processed_shaps_df_pl
            #     .with_columns(pl.col("feature").cast(pl.Utf8))  # Ensure feature column is string type
            #     .to_pandas()
            #     .set_index("feature")
            # )
        ...
        if self.automatic:
            ...
            if self.force_convergence:
                ...
                while (len(converge_df[converge_df.p_value < self.power_alpha]) > 0) & (
                    ...
                    converge_df_pl = pl.DataFrame(converge_shaps_df)
                    converge_df = powershap_statistical_analysis_rs.powershap_statistical_analysis(
                        converge_df_pl,
                        self.power_alpha,
                        self.power_req_iterations,
                        include_all=self.include_all,
                    )
                    converge_df = converge_df_pl.to_pandas()
