...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        self.api_version = getattr(self.__class__, 'api_version', '0.1.31')  # Default to package version if not set
    ...
    def search(self, query, user_id=None, agent_id=None, run_id=None, limit=100, filters=None):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            if self.version == "v1.1" and self.enable_graph and "user_id" in filters:
                future_graph_entities = executor.submit(self.graph.search, query, filters, limit)
                futures = [future_memories, future_graph_entities]
            else:
                future_graph_entities = None
                futures = [future_memories]
        ...
        if self.version == "v1.1":
            if self.enable_graph and graph_entities is not None:
