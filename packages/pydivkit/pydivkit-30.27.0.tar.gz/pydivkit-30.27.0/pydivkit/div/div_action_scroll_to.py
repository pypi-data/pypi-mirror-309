...
class DivActionScrollTo(BaseDiv):
    ...
    destination: div_action_scroll_destination.DivActionScrollDestination = Field(
        description=(
            ...
            "element with the indexprovided in \'value\'\'offset\': "
            "Scroll to the position specified in \'value\' andmeasured "
            "in \'dp\' from the start of the container. Applies only "
            "in\'gallery\';\'start\': Scroll to the container "
            "start;\'end\': Scroll to the containerend."
        ),
    )
