...
class AWS:
    ...
    def get_all_files(self):
        logger.info(f'{self.status} [Get] All files in: {self.bucket_name}')
        return [_.key for _ in self.my_bucket.objects.all()]

    def delete_file(self, key: str):
        self.client.delete_object(Bucket=self.bucket_name, Key=key)
        logger.info(f'{self.status} [Remove]: {key}')
    ...
    def create_presigned_url(self, key: str, expiration: int = 900):
        ...
        logger.info(f'{self.status} [Pre-signed] {key} in {expiration / 3600}h')
        return url
...
# aws = AWS(bucket)
# lst_all_item = aws.get_all_files()
# print(lst_all_item)
#
# path = Path.home() / 'Downloads/Data/fe_esf/test/batch'
# files = [*path.glob('*.parquet')]
# for f in files[:10]:
#     aws.upload_file(f, folder='voucher_sensitivity/prediction')
