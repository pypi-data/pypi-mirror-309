...
class RecordWithValidationSchemaBase(ABC):
    ...
    def validate_dataset(self, ingest_dataset: DataFrame) -> DataFrame:
        ...
        data_with_validations = data_with_validations.withColumn("STRING_VALIDATION_GREATER_THAN_MAX_LENGTH", col("str_v8n").rlike("^.{5,}"))
        data_with_validations = data_with_validations.withColumn("STRING_VALIDATION_LESS_THAN_MAX_LENGTH", col("str_v8n").rlike("^.{50,}").eqNullSafe(False))
        data_with_validations = data_with_validations.withColumn("STRING_VALIDATION_MATCHES_FORMAT", col("str_v8n").rlike("example-regex")
            | col("str_v8n").rlike("[A-Z]*[1-5]+")
            | col("str_v8n").rlike("\\D*"))
        data_with_validations = data_with_validations.withColumn("INTEGER_VALIDATION_GREATER_THAN_MIN", col("int_v8n").cast('double') >= 100)
        data_with_validations = data_with_validations.withColumn("INTEGER_VALIDATION_LESS_THAN_MAX", col("int_v8n").cast('double') <= 999)
        data_with_validations = data_with_validations.withColumn("DECIMAL_VALIDATION_GREATER_THAN_MIN", col("dec_v8n").cast('double') >= 12.345)
        data_with_validations = data_with_validations.withColumn("DECIMAL_VALIDATION_LESS_THAN_MAX", col("dec_v8n").cast('double') <= 100.0)
        data_with_validations = data_with_validations.withColumn("DECIMAL_VALIDATION_MATCHES_SCALE", col("dec_v8n").cast(StringType()).rlike(r"^[0-9]*(?:\.[0-9]{0,3})?$"))
        data_with_validations = data_with_validations.withColumn("FLOAT_VALIDATION_GREATER_THAN_MIN", col("float_v8n").cast('double') >= 12.345)
        data_with_validations = data_with_validations.withColumn("FLOAT_VALIDATION_LESS_THAN_MAX", col("float_v8n").cast('double') <= 100.0)
        data_with_validations = data_with_validations.withColumn("FLOAT_VALIDATION_MATCHES_SCALE", col("float_v8n").cast(StringType()).rlike(r"^[0-9]*(?:\.[0-9]{0,3})?$"))
        data_with_validations = data_with_validations.withColumn("REQUIRED_SIMPLE_TYPE_IS_NOT_NULL", col("simple_v8n").isNotNull())
        data_with_validations = data_with_validations.withColumn("REQUIRED_COMPLEX_TYPE_IS_NOT_NULL", col("complex_v8n").isNotNull())
        data_with_validations = data_with_validations.withColumn("REQUIRED_COMPLEX_TYPE_GREATER_THAN_MAX_LENGTH", col("complex_v8n").rlike("^.{5,}"))
        data_with_validations = data_with_validations.withColumn("REQUIRED_COMPLEX_TYPE_LESS_THAN_MAX_LENGTH", col("complex_v8n").rlike("^.{50,}").eqNullSafe(False))
        data_with_validations = data_with_validations.withColumn("REQUIRED_COMPLEX_TYPE_MATCHES_FORMAT", col("complex_v8n").rlike("example-regex")
            | col("complex_v8n").rlike("[A-Z]*[1-5]+")
            | col("complex_v8n").rlike("\\D*"))
