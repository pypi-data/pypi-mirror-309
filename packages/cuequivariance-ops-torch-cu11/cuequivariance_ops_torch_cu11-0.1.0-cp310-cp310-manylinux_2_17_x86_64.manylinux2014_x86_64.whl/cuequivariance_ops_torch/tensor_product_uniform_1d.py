...
def _fwd_primitive_backward(ctx, in3):
    *ins, data = ctx.saved_tensors
    ...
    for i, n in zip(ins, ngs):
        args.append(i)
        args.append(n)
    outs = tensor_product_uniform_1d_bwd(*args, in3, ctx.num_paths, data, ctx.math_code)
...
def _bwd_primitive_backward(ctx, *grads):
    *ins, data = ctx.saved_tensors
    ...
    args.append(ins[-1])
    args.append(ngs[-1])
    outs = tensor_product_uniform_1d_bwd_bwd(
        *args,
        ctx.num_paths,
        data,
        ctx.math_code,
...
def tensor_product_uniform_1d_fwd(*args):
    if len(args) == 4 + 3:
        ...
        dummy = torch.empty(
            (0,), dtype=args[0].dtype, device=args[0].device, requires_grad=False
        )
        args = list(args)
        args.insert(2, dummy)
...
def tensor_product_uniform_1d_bwd(*args):
    return torch.ops.cuequivariance_ops_torch.tensor_product_uniform_4x1d_bwd_primitive(
        *args
    )


def tensor_product_uniform_1d_bwd_bwd(*args):
    return torch.ops.cuequivariance_ops_torch.tensor_product_uniform_4x1d_bwd_bwd_primitive(
        *args
    )
...
class TensorProductUniform1d(nn.Module):
    ...
    def forward(
        ...
        ins = [
            i.reshape(
                i.shape[0],
                self.operand_num_segments[idx],
                max(1, self.operand_dim[idx] * self.operand_extent),
            )
            for idx, i in enumerate(ins[: len(self.operand_dim) - 1])
        ]
        for i in ins:
            torch._assert(
                i is None or ins[0].dtype == i.dtype, "tensors must have same type"
            )
        ...
        return out.reshape(
            out.shape[0],
            self.operand_num_segments[-1]
            * max(1, self.operand_dim[-1] * self.operand_extent),
        )
