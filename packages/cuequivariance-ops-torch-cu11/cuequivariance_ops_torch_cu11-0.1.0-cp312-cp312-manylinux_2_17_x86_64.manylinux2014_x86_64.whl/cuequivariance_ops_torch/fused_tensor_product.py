...
def get_batch_size(in0: torch.Tensor, in1: torch.Tensor, in2: torch.Tensor):
    bs_0 = in0.size(0) if in0.dim() == 2 else 1
    bs_1 = in1.size(0) if in1.dim() == 2 else 1
    bs_2 = in2.size(0) if in2 is not None and in2.dim() == 2 else 1
    return max(bs_0, bs_1, bs_2)
...
class FusedTensorProductOp3(nn.Module):
    ...
    def forward(
        ...
        dummy = torch.empty((0,), dtype=in0.dtype, device=in0.device)
        out = fused_tensor_product(
            in0,
            in1,
