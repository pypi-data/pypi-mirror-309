...
class SystemInfo:
    ...
    def get_gpu_info():
        ...
        for gpu in gpus:
            gpu_info.append({
                ...
                "gpu_utilization": f"{gpu.load * 100}%",
    ...
    def get_mac_address():
        for interface, addrs in psutil.net_if_addrs().items():
            for addr in addrs:
                if addr.family == psutil.AF_LINK or addr.family == psutil.AF_PACKET or addr.family == 17:  # 17 is the value for AF_PACKET on Windows
                    return addr.address
        return "N/A"
