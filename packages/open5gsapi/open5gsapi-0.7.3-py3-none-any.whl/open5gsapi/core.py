...
class Open5GS:
    ...
    def get_ue_ips(self) -> List[Dict[str, Any]]:
        """
        Get list of available UE IPs and their ports
        Returns: List of dicts containing UE info
        """
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0 and result.stdout.strip():
                interfaces = eval(result.stdout.strip())  # Safe since we control the python command
                return [
                    {
                        'ip': ip,
                        'interface': iface,
