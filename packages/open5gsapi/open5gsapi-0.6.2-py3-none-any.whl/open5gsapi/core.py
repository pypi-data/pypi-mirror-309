...
class Open5GS:
    ...
    def _initialize(self):
        self.policy = Policy()
        self.ue_base_url = "http://10.10.0.132:8080"  # Base URL with default port
    ...
    def send_data(self, url: str, data: Dict[str, Any], port_offset: int = 0) -> Dict[str, Any]:
        try:
            ...
            if is_ue_endpoint:
                base_port = 8080  # Base port for UEs
                port = base_port + port_offset
                url = url.replace(":8080", f":{port}") 
    ...
    def receive_data(self, endpoint: str) -> Dict[str, Any]:
        try:
            url = f"{self.upf_base_url}/{endpoint}"
