...
class Open5GS:
    ...
    def _wait_for_interfaces(self, timeout: int = 60) -> bool:
        """Wait for TUN interfaces to be created"""
    ...
    def get_ue_ips(self) -> List[Dict[str, Any]]:
        """Get list of available UE IPs with interface waiting"""
        ...
        try:
            result = subprocess.run(
                ["docker", "exec", "ue", "python3", "-c", 
                 "import netifaces; print([(iface, [addr['addr'] for addr in netifaces.ifaddresses(iface).get(2, [])][0]) for iface in netifaces.interfaces() if iface.startswith('uesimtun')])"],
                capture_output=True,
                text=True,
                check=True
            )
            ...
            if result.stdout.strip():
                interfaces = eval(result.stdout.strip())
                return [
                    {
                        'ip': ip,
                        'interface': iface,
                        'port': self._get_port_for_interface(iface),
                        'port_offset': self._get_port_offset(iface)
                    }
                    for iface, ip in interfaces
                ]
                
            logger.warning("No TUN interfaces found")
            return []
    ...
    def _get_port_for_interface(self, interface: str) -> int:
        """Internal method to get port for a UE interface"""
        try:
            index = int(interface.replace('uesimtun', ''))
            return 8080 + index
        except ValueError:
            return 8080

    def _get_port_offset(self, interface: str) -> int:
        """Internal method to get port offset for a UE interface"""
        try:
            return int(interface.replace('uesimtun', ''))
        except ValueError:
            return 0
