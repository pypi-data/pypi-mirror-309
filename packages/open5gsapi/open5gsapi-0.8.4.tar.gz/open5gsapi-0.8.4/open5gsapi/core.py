...
class Open5GS:
    ...
    #         if '://' in endpoint:
    #             # For video streaming case - handles full URLs
    #             url = endpoint
    #         else:
    #             # For sensor case - constructs URL from endpoint
    #             url = f"http://10.10.0.132:{port}/{endpoint}"

    #         if isinstance(data, (dict, list)):
    #             response = requests.post(url, json=data)
    #         else:
    #             # For binary data (video frames)
    ...
    def send_data(self, endpoint: str, data: Any, port_offset: int = 0) -> Dict[str, Any]:
        try:
            if port_offset:
                # Modify port in the URL for UE communications
                base_port = 8080
                url = f"http://10.10.0.132:{base_port + port_offset}/{endpoint}"
            else:
                # Use the endpoint directly (already has base URL)
                url = endpoint
    ...
    #             url = endpoint
    #         else:
    #             url = f"http://10.10.0.112:8081/{endpoint}"
