...
class NetworkMetricsCalculator:
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            # Calculate throughput
            recent_throughput = list(self.packet_sizes)[-10:]  # Last 10 samples
            current_throughput = statistics.mean(recent_throughput) if recent_throughput else 0
            current_throughput_mbps = (current_throughput * 8) / 1_000_000
            
            # Calculate latency stats
            latencies = list(self.packet_latencies)
            
            metrics = {
                "throughput": {
                    "total_mbps": current_throughput_mbps,
                    "peak_throughput_mbps": max(recent_throughput) * 8 / 1_000_000 if recent_throughput else 0
                },
                "latency": {
