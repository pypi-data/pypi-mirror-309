...
class NetworkMetricsCalculator:
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            try:
                expected_packets = elapsed_time * 30 
            ...
            if self.total_frames > 0:
                ...
                # Calculate peak throughput from frame sizes
                if self.frame_sizes and self.frame_intervals:
                    recent_size = sum(self.frame_sizes[-5:]) if len(self.frame_sizes) >= 5 else sum(self.frame_sizes)
                    recent_time = sum(self.frame_intervals[-5:]) if len(self.frame_intervals) >= 5 else sum(self.frame_intervals)
                    if recent_time > 0:
