...
async function saveAllJson(luckysheet) {
  console.log("saveAllJson")
  let allSheets = null
  let sheetsJson = null
  ...
  try {
    ...
    if (!allSheets || allSheets.length === 0) {
      throw new Error("No sheets found in Luckysheet.")
    }
    sheetsJson = JSON.stringify(allSheets)
    // console.log(sheetsJson)
    const compressedJson = pako.gzip(sheetsJson, { to: "string" })
    // compressedJson = await compressData(sheetsJson)
    const sheetsBase64 = arrayBufferToBase64(compressedJson)
    // console.log(sheetsBase64)
    return sheetsBase64
  } catch (error) {
    console.error("Error saving JSON:", error)
    throw new Error("Failed to compress and save sheets as Base64 JSON.")
  } finally {
  ...
  function arrayBufferToBase64(buffer) {
    ...
    return btoa(binary) // Encode the binary string as Base64
  }
}
...
async function saveAllExcel(luckysheet, name, judge) {
  console.log("saveAllExcel")
  try {
    workbook = new ExcelJS.Workbook()
    tableArr = luckysheet.getAllSheets()
    ...
    tableArr.forEach(function (table) {
      ...
      setStyleAndValue(luckysheet, table.data, worksheet)
    ...
    // console.log("Converting To Base 64 Please wait . . .")
...
var setStyleAndValue = function (luckysheet, cellArr, worksheet) {
  ...
  cellArr.forEach(function (row, rowid) {
    ...
    row.every(function (cell, columnid) {
      ...
      //console.log("rowid: " + rowid, " columnid: " + columnid + "Object Key : ")
...
export { saveAllExcel, saveAllJson }
