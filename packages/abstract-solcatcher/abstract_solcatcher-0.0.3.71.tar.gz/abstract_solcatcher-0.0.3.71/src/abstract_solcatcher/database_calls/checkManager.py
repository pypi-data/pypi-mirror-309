...
from ..utils import *
from abstract_apis import postRpcRequest,get_async_response,get_json_response,make_request,get_url,make_endpoint,get_headers,get_async_response
...
def call_log_response(method=None,response=None,status_code=False,retry_after=None):
  return FlaskRequest(endpoint='log_response',method=get_method(method),response=get_resp(response) retry_after=retry_after,get_post='POST',status_code=status_code)
