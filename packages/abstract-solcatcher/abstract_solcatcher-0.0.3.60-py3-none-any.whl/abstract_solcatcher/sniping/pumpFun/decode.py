...
class LogManager(metaclass=SingletonMeta):
    def __init__(self):
        if not hasattr(self, "initialized"):
            ...
            self.logsHist = defaultdict(dict)

    def get_log(self, log=None, signature=None):
        # Retrieve by signature if available
        if signature:
            return self.logsHist.get(signature)

        # Parse log if provided and ensure it's a dictionary
        ...
        if not signature:
            signature = self.get_signature(log=log)

        # Cache and return the log
        self.logsHist[signature] = log
        return log

    def get_value(self, log=None, signature=None):
        # Fetch log by signature or use provided log
        log = self.get_log(log=log, signature=signature)
        if not log:
            return None
        return log.get('params', {}).get('result', {}).get('value')

    def get_logs(self, log=None, signature=None):
        value = self.get_value(log=log, signature=signature)
        if not value:
            return None
        return value.get('logs')

    def get_signature(self, log=None):
        value = self.get_value(log=log)
        if not value:
            return None
...
def get_value_from_logs(logNotification):
    """Extract 'value' from log notification."""
    log_data = safe_json_loads(logNotification)
    if not log_data:
        return None
    return log_data.get('params', {}).get('result', {}).get('value')

def get_signature(response=None, signature=None):
    """Fetch signature from the log."""
    return log_manager.get_signature(log=response)

def get_logs(response=None, signature=None):
    """Fetch logs from the log."""
    return log_manager.get_logs(log=response, signature=signature)

def get_err(response=None, signature=None):
    """Fetch error information from the log."""
    return log_manager.get_err(log=response, signature=signature)
