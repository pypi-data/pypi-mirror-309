...
allfuncs = '''class Client(_ClientCore):  # pylint: disable=too-many-public-methods
    """Client class.
    ...
    def __init__(
        ...
        self._provider = get_solcatcher_api(endpoint, timeout=timeout, extra_headers=extra_headers)
    ...
    def get_account_info(
        ...
        """
        return {'method': 'getAccountInfo', 'jsonrpc': '2.0', 'id': 0, 'params': [pubkey, {'encoding': encoding, 'dataSlice': data_slice, 'commitment': commitment, 'minContextSlot': minContextSlot}]}
    ...
    def get_account_info_json_parsed(
        ...
        """
        {'method': 'getAccountInfo', 'jsonrpc': '2.0', 'id': 0, 'params': [pubkey, {'encoding': encoding, 'dataSlice': None, 'commitment': 'confirmed', 'minContextSlot': None}]}
    ...
    def get_signatures_for_address(
        ...
        Example:
            ...
            >>> solana_client.get_signatures_for_address(get_pubkey(pubkey), limit=1).value[0].signature # doctest: +SKIP
            Signature(
    ...
    def get_program_accounts(pubkey, pylint: disable=too-many-arguments
        self,
        pubkey: Pubkey,
        commitment: Optional[Commitment] = "confirmed",
        encoding: str = "base64",
        ...
        Example:
            ...
            >>> solana_client.get_program_accounts(get_pubkey(pubkey), filters=filters).value[0].account.lamports # doctest: +SKIP
            1
        """  # noqa: E501 # pylint: disable=line-too-long
    ...
    def get_token_account_balance(
        ...
        Example:
            ...
            '9864'
        """
        body = json.loads(str(self._get_token_account_balance_body(get_pubkey(pubkey), commitment)))
        return body
    ...
    def confirm_transaction(
        ...
        if last_valid_block_height:  
            ...
            return body
    def get_balance(self, pubkey: Pubkey, commitment: Optional[Commitment] = "finalized",minContextSlot=None) -> GetBalanceResp:
        """Returns the balance of the account of provided Pubkey.
        ...
        Example:
            ...
            4104230290
        """
        return {"method":"getBalance","jsonrpc":"2.0","id":0,"params":[pubkey,{"commitment":commitment,"minContextSlot":minContextSlot}]}
'''
class Client(_ClientCore):  # pylint: disable=too-many-public-methods
    ...
    def get_transaction(
        ...
        body = json.loads(str(self._get_transaction_body(get_sigkey(tx_sig), encoding, commitment, max_supported_transaction_version)))
        return body
    ...
    def get_fee_for_message(
        self, message: VersionedMessage, commitment: Optional[Commitment] = "finalized") -> GetFeeForMessageResp:
        """Returns the fee for a message.
...
all_funcs_spl = allfuncs.replace('**kwargs','').split('def ')
replace_all = []
all_funcs = []
client = Client()
all_bodies = {}
from abstract_utilities import *
for func in all_funcs_spl[1:]:
  
        all_funcs.append(func.split('(')[0])
        arg = eatAll(func.replace('self,','').replace('self','').split('(')[1].split(')')[0].split(',')[0].split(':')[0],[' ','\t','\n'])
        function = getattr(client,all_funcs[-1])
        try:
            if arg:
                response =function(**{arg:{"delegate":"4Nd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T","mint":"4Nd1mBQtrMJVYVfKf2PJy9NZUZdTAsp7D4xWLs4gDB4T","owner":"7fUAJdStEuGbc3sM84cKRL6yYaaSstyLSU4ve5oovLS7","commitment":"finalized","message":"hihihihi","account":"Vote111111111111111111111111111111111111111","commitment":"processed","slot":168036172,"pubkeys":["EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG","EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG"],"pubkey":"EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG","tx_sig":"5VERv8NMvzbJMEkV8xnrLkEaWRtSz9CosKDYjCJjBRnbJLgp8uirBgmQpjKhoR4tjF3ZpRzrFmBV6UjKdiSZkQUW","signature":"5VERv8NMvzbJMEkV8xnrLkEaWRtSz9CosKDYjCJjBRnbJLgp8uirBgmQpjKhoR4tjF3ZpRzrFmBV6UjKdiSZkQUW"}.get(arg,0)})
            else:
                response = function()
            response['init']=arg
            all_bodies[all_funcs[-1]]=response
        except:    
            print(arg)
            print(all_funcs[-1])
            all_bodies[all_funcs[-1]]=None
        input(all_bodies)
import time

def get_regular_call_params():
    timestamp = time.time()
    response = client.get_account_info(pubkey)
    diff = time.time() - timestamp
    print(diff)
    return diff
def get_call_params(function,*args):
    timestamp = time.time()
    ...
    print(diff)
    return diff
def get_regular_call_param(pubkey="EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG"):
    timestamp = time.time()
    response = client.get_account_info(pubkey)
    diff = time.time() - timestamp
    print(response)
    print(diff)
    return diff
diff1 = get_call_params('get_account_info',"EtWTRABZaYq6iMfeYKouRu166VU2xqa1wcaWoxPkrZBG")
diff2 = get_regular_call_params()
diff3 = get_regular_call_param()
input(diff2/diff1)
input(diff3/diff1)
