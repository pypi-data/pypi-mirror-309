...
from abstract_solcatcher.abstract_rate_limit import makeLimitedCall
...
def getAccountInfo(account,encoding=None,commitment=None):
  return get_async_response(client.get_account_info,endpoint="getAccountInfo",account=account,encoding=encoding,commitment=commitment)

def getBlock(slot,encoding=None,maxSupportedTransactionVersion=None,transactionDetails= None,rewards=None):
    return get_async_response(client.get_block,slot=slot,encoding=encoding,maxSupportedTransactionVersion=maxSupportedTransactionVersion,transactionDetails=transactionDetails,rewards=rewards)
