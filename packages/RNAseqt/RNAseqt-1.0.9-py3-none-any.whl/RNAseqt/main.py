...
rule fastqc:
    input:
        ...
        r2_new = "Rawdata/{sample}_R2.fq"
    output:
        r1_zip = "QCresult/{sample}_R1.fastqc.zip",
        r1_html = "QCresult/{sample}_R1.fastqc.html",
        r2_zip = "QCresult/{sample}_R2.fastqc.zip",
        r2_html = "QCresult/{sample}_R2.fastqc.html"
...
rule qc_multiqc:
    input:
        html=expand("QCresult/{sample}_R1.fastqc.html", sample=config["samples"]) +
             expand("QCresult/{sample}_R2.fastqc.html", sample=config["samples"]),
        zip=expand("QCresult/{sample}_R1.fastqc.zip", sample=config["samples"]) +
            expand("QCresult/{sample}_R2.fastqc.zip", sample=config["samples"])
    output:
        report="QCresult/multiqc_report.html"
    shell:
        "multiqc -o QCresult QCresult/*.fastqc.zip"
...
rule trim_galore:
    ...
    output:
        ...
        r1_clean_zip = "Cleandata/{sample}_R1_val_1.fastqc.zip",
        r1_clean_html = "Cleandata/{sample}_R1_val_1.fastqc.html",
        r2_clean_zip = "Cleandata/{sample}_R2_val_2.fastqc.zip",
        r2_clean_html = "Cleandata/{sample}_R2_val_2.fastqc.html"
    threads: config["threads"]
    shell:
...
rule clean_multiqc:
    input:
        html=expand("Cleandata/{sample}_R1_val_1.fastqc.html", sample=config["samples"]) +
             expand("Cleandata/{sample}_R2_val_2.fastqc.html", sample=config["samples"]),
        zip=expand("Cleandata/{sample}_R1_val_1.fastqc.zip", sample=config["samples"]) +
            expand("Cleandata/{sample}_R2_val_2.fastqc.html", sample=config["samples"])
    output:
        report="Cleandata/multiqc_report.html"
    shell:
        "multiqc -o Cleandata Cleandata/*.fastqc.zip"
