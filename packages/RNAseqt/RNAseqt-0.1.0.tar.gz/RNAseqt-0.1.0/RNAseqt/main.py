...
rule all:
    input:
        ...
        expand("{cleandata}/{sample}_1_val_1.fq", cleandata=config["cleandata"], sample=config["samples"]),
        expand("{cleandata}/{sample}_2_val_2.fq", cleandata=config["cleandata"], sample=config["samples"]),
...
rule fastqc:
    input:
        fq="Rawdata/{fq}"
    output:
        zip="QCresult/{wildcards.fq}.fastqc.zip",
        html="QCresult/{wildcards.fq}.fastqc.html"
    threads: config["threads"]
    shell:
        "fastqc -t {threads} -o QCresult {input.fq}"
rule multiqc:
    input:
        expand("QCresult/{fq}.fastqc.zip", fq=[v["R1"] for v in config["samples"].values()] +
                                              [v["R2"] for v in config["samples"].values()])
    output:
        "QCresult/multiqc_report.html"
    shell:
        "multiqc -o QCresult QCresult/*.zip"
rule trim_galore:
    input:
        r1=lambda wildcards: config["samples"][wildcards.sample]["R1"],
        r2=lambda wildcards: config["samples"][wildcards.sample]["R2"]
    output:
        r1_clean="{cleandata}/{sample}_1_val_1.fq",
        r2_clean="{cleandata}/{sample}_2_val_2.fq"
    threads: config["threads"]
    shell:
        "trim_galore -q 20 --length 36 --max_n 3 --stringency 3 --fastqc --paired -o {config[cleandata]} {input.r1} {input.r2}"
rule multiqc_clean:
    input:
        expand("{cleandata}/{sample}_1_val_1.fastqc.zip", sample=config["samples"]) +
        expand("{cleandata}/{sample}_2_val_2.fastqc.zip", sample=config["samples"])
    output:
        "{cleandata}/multiqc_report.html"
    shell:
        "multiqc -o {config[cleandata]} {config[cleandata]}/*.zip"
rule hisat2:
    input:
        r1_clean="{cleandata}/{sample}_1_val_1.fq",
        r2_clean="{cleandata}/{sample}_2_val_2.fq"
    ...
    shell:
        "
        hisat2 -p {threads} -x {config[reference]} -1 {input.r1_clean} -2 {input.r2_clean} 2> Mapping/{wildcards.sample}.log | samtools sort -@ {threads} -o {output.bam}
        samtools index {output.bam}
        "
rule mapping_multiqc:
    input:
        expand("Mapping/{sample}.log", sample=config["samples"])
    output:
        "Mapping/multiqc_report.html"
    shell:
        "multiqc -o Mapping Mapping/*.log"
rule featureCounts:
    input:
