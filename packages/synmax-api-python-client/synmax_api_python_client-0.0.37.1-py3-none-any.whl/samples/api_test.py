...
def wells():
    payload = ApiPayload(start_date='2024-01-01', end_date='2024-12-31')

    # result_df = client.wells(payload)
    result_df = client.wells(payload)
    print(f'Total result count: {len(result_df)}')
...
def test_rigs_with_payload():
    """
    Test for Daniel rigs payload
    :return:
    """
    ...
    # Define your payload with dynamic date values
    payload = ApiPayload(start_date=start_date, end_date=end_date )
    result_df = client.rigs(payload)
    print(result_df.count())
