...
#       where after reassignment,
#       the old [zero]next/previous is still pointing to the original sequence
#       and the original sequence is pointing to the new [zero]
#       as if the connections of the old one have not been updated to point to itself sa it was replaced,
#       and that is causing it to appear as a valid entry point
#       and when _nodes before yielding is asked to check whether head is still in list, it think it is,
#       because it is still connected, but since none of the other points to it, it is never next
#       and can thus not be the trigger point needed to end the loop
#       this is when going through __setitem__ with index, not slice, I think
#       seems like maybe insert_before() doesn't correct connections of single replaced node,
#       at least processed_sequence_* are set to None, which suggests that it is not being modified
