...
# TODO: i have a situation
#       where the head of a CDLL is not correctly reassigned to the head Node of its Node sequence
#       this causes a list() of the CDLL to incorrectly return a list with just 1 item instead of 4
#       and a before_and_after_unique() to raise an exception even though the unique item exist in CDLL
#       the issue appears to be related to the replacement of an EdgePossible with an EdgeSelected
#       which is organised in ShapeSelectedInjector.from_possible()
#       so that is a place to start looking for insights into what is going wrong
#       looking a bit closer I notice that the first edge in this loop is looking fine,
#       and the reference from the related corner is fine too
#       it is only in the second edge, whose position is being attempted to calculate that the issue shows itself
#       I can't tell if it is the position calculation that messes with the CDLL
#       or if it was already badly structured and it is just by having the correct entrypoint from this particular edge
#       that makes the brokenness visible
#       fixed the head pointer manually to see what would happen, and it turns out that it gets worse
#       even if it had been correct there is something about the setup that feeds unexpected data into the process
#       for instance another CornerSelected is needed at some point for calculating the EdgePossible position
#       and it uses a fixed index in the edges list of the corner to get the other_corner,
#       but the index is for a EdgePossible too, possibly the same, which has a CornerPossible in its other_corner,
#       which obviously doesn't have a position to return, so None is returned,
#       which goes on to break the following attempt to calculate a position for the EdgePossible
#       I am just wondering if my manual "fix" broke the list or whether there is something fundamental
#       about the situation that I am not getting
#       maybe the CDLL head was right, and it was the Node sequence that had the wrong head?
#       ok so the alternative manual fix worked for the second breakage
#       the program then went on to break on the first step again with another corner-edge combination
#       which suggests that it is a repeated issue
#       could it be about the second possible edge in each corner?
#       so it would be something structural about constructing the graph maybe
#       where the CDLL heads are set correctly, but the Node sequence heads are not updated...
#       it is just deeply strange that the breakage does not show up until in this position calculation function
#       even when I look specifically at the affected corners earlier in the process they look correct
#       and the issue doesn't appear on the first calculation for an Edge in the Corner, but namely the second
#       which looks kinda suspicious and is so fucking wierd
#       and I found it
#       there is a use of angle_sweep_between_edges(), that makes copy() of the CDLL which is not a deep copy!
#       and rotates them which is basically equivalent to changing the head in the Node sequence
#       without informing the original CDLL about the change :facepalm:
#       so either angle_sweep_between_edges() needs refactoring to create a deepcopy or not need to use rotate
#       or CDLL copy has to be modified to provide a deep copy and thus a new unrelated Node sequence
...
class CDLL(MutableSequence[_T], Generic[_T]):
    ...
    def __repr__(self) -> str:
        # TODO: return __repr__ to run on standard iterator when it is running correctly
    ...
    def _add(self, first: Iterable[_T] | None, second: Iterable[_T] | None, is_inplace: bool) -> "CDLL[_T]":
        ...
        cdll: CDLL[_T] = self if is_inplace else CDLL()
