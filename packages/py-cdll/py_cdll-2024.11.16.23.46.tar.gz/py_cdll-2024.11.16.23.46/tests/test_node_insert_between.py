...
from src.py_cdll.node import Node, insert_between, nodes_from_values, equal, is_consistent, is_value_at_index
...
def test_insert_between_first_three_seven_nine_fifteen_nineteen_second_ten_twelve_two_five_with_head_success():
    ...
    after0: Node = before0
    insert0: Node = head1
    removed0: Node = nodes_from_values(values=values3)
    ...
    assert is_value_at_index(node=head1, reference=values2) is True
    assert is_value_at_index(node=head2, reference=values2) is True
    assert is_value_at_index(node=removed0, reference=values3) is True
