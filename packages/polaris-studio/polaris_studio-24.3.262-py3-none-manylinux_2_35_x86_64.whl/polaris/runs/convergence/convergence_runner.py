...
from polaris.utils.numa.numa import get_numa_nodes_to_use
...
def run_polaris_convergence(
    ...
    _ = SupplyChecker(config.data_dir / config.supply_file()).has_critical_errors(fail_on_errors=False)
    ...
    # Check for NUMA options and set them into a cmd string
    numa_nodes = setup_numa_options(config)
...
def setup_numa_options(config: ConvergenceConfig):
    if config.use_numa:
        return get_numa_nodes_to_use(config.num_threads)
    else:
        return None
