...
class MysqlCompareTable:
    ...
    def __init__(
        ...
        self.logger = init_logger(compare_name)
    ...
    def running_compare(self):
        with connect(**self.src_dsn) as source_con, connect(**self.dst_dsn) as target_con:
            ...
            while True:
                ...
                self.logger.info(f"batch_id[{_batch_id}] source query statment: '{query_statement_src}', params '{query_params_src}'")
                self.logger.info(f"batch_id[{_batch_id}] target query statment: '{query_statement_dst}', params '{query_params_dst}'")
                ...
                self.logger.info(f"batch_id[{_batch_id}] source rows: {len(source_rows)}")
                self.logger.info(f"batch_id[{_batch_id}] target rows: {len(target_rows)}")
                ...
                if len(_diff_rows) >= 1:
                    ...
                    self.logger.info(f"batch_id[{_batch_id}] find diff rows: {len(_diff_rows)}")
                ...
                self.logger.info(f"batch_id[{_batch_id}] source rows: {len(source_rows)}, target rows: {len(target_rows)}.")
                self.logger.info(
    ...
    def start(self):
        self.logger.info("running_compare")
        ...
        self.logger.info(f"table compare done, diff count: {self.diff_rows_cnt}")


if __name__ == "__main__":
    # --src-dsn root:root_password@192.168.161.93:33026 --dst-dsn root:root_password@192.168.161.93:33026 --parallel 10 --batch-size 50000 --database test --work-dir cc --work-dir-clear

    # src_dsn = {"host": "192.168.161.93", "port": 33026, "user": "root", "password": "root_password"}
    # dst_dsn = {"host": "192.168.161.93", "port": 33027, "user": "root", "password": "root_password"}
    # CompareTable(
    #     "abc",
    #     10000,
    #     0,
    ...
    src_dsn = {"host": "172.23.8.63", "port": 3306, "user": "ghost", "password": "54448hotINBOX", "compress": True}
    dst_dsn = {"host": "10.50.0.142", "port": 3306, "user": "ghost", "password": "54448hotINBOX"}
    MysqlCompareTable(
        "merchant_center_vela_v1.mc_products_alias",
        10000,
        0,
        False,
        src_dsn,
