...
class TakeoffClient:
    ...
    def detokenize(self, tokens: Union[list[str], list[int]], reader_id: str, skip_special_tokens: bool = True) -> str:
        ...
        response = self._sync_post(
            self.url + f"/detokenize/{reader_id}", json={"tokens": tokens, "skip_special_tokens": skip_special_tokens}
        )
    ...
    async def detokenize_async(
        self, tokens: Union[list[str], list[int]], reader_id: str, skip_special_tokens: bool = True
    ) -> str:
    ...
    def chat_template(
        ...
        Args:
            input (List[ChatTemplateMessage] | ChatTemplateMessage): A list of messages to send to the model.
