...
style_title	= {
	...
	'fill'		: SpiritLong_excel.cell_fill(color='4169E1'),
	'border'	: None,
	'alignment'	: SpiritLong_excel.alignment_center,
}
...
def set_cell(sheet, i, j, value, number_format='General', font=None, fill=None, border=None, alignment=None):
	...
	cell.font		= font if font is not None else SpiritLong_excel.cell_font()
	# 默认无填充
	cell.fill		= fill if fill is not None else openpyxl.styles.PatternFill(fill_type	= None)
	# 默认左对齐
	cell.alignment		= alignment if alignment is not None else SpiritLong_excel.alignment_left
	# 默认无边界
...
def adjust_all_column_width(sheet, max_width=100):
	for column in range(1, sheet.max_column+1):
		SpiritLong_excel.adjust_column_width(sheet, column, max_width=max_width)
...
def records_to_sheet(sheet, sheet_data, style_title=style_title, style_text={}):
	...
	for i, t in enumerate(titles.values()):
		SpiritLong_excel.set_cell(sheet, 1, i+1, t, **style_title)
	...
	if data_valid:
		for i, r in enumerate(sheet_data['records']):
			for j, field in enumerate(titles):
				if field in r.keys():
					SpiritLong_excel.set_cell(sheet, i+2, j+1, r[field], **style_text)	

	# 自动调整列宽
	if data_valid :
		SpiritLong_excel.adjust_all_column_width(sheet)
...
def records_to_excel(filename, data, style_title=style_title, style_text={}):
	...
	for sheet_name in data:
		...
		SpiritLong_excel.records_to_sheet(sheet, data[sheet_name], style_title, style_text)
	# 保存文件，确保是xlsx后缀名
...
if __name__ == '__main__':
	...
	# 	}
	# }
	# #SpiritLong_excel.records_to_excel("t.xlsx", data)

	# book	= openpyxl.Workbook()
	# sheet	= book.active
	# SpiritLong_excel.set_cell(sheet, 1, 1, "321", border=cell_border())
	# SpiritLong_excel.set_cell(sheet, 2, 3, data["1"]['records'][3]['key1'], border=cell_border(sides='left, bottom'))
	# book.save('t.xlsx')
	# SpiritLong_excel.csv_save_as_xlsx('1.csv', overwrite=True)
	pass