...
import	SpiritLong_word

## 打开docx文件
@SpiritLong_word.export
	...
	if p_start and p_end:
		...
		for p in doc.paragraphs:
			if flag and p._element==p_end._element:
				...
				# 该删除
				SpiritLong_word.delete_paragraph(p)
		# 在开始段落之后添加
		SpiritLong_word.add_paragraphs(p_start, paragraphs)
...
def add_blank_paragraphs(p_start, n):
	...
	for i in range(n):
		...
		p		= SpiritLong_word.add_paragraphs(p, [paragraph])
	return p
...
def add_text_with_style(p_start, text, style=None, style_attributes={}):
	...
	return SpiritLong_word.add_paragraphs(p_start, paragraphs)
...
def append_text(doc, match_text, text):
	p	= SpiritLong_word.find_paragraph(doc, match_text)
...
def table_insert_row(table, row):
	...
	for cell in table.rows[row].cells:
		SpiritLong_word.set_cell_border(cell, top=style, bottom=style, left=style, right=style)
		# 竖直居中
...
def table_cell_fill(table, row, column, content, style=None, style_attributes={}):
	...
	SpiritLong_word.add_text_with_style(p_start, content, style, style_attributes)
	SpiritLong_word.delete_paragraph(p_start)
...
def merge_cell(table, row_min, row_max, column_min, column_max, text, style=None, style_attributes={}):
	SpiritLong_word.table_cell_fill(table, row_min, column_min, text, style, style_attributes)
