...
class ObjectStorage:
    ...
    def _get_object_size(self, container_url, req_header, object_path):
    ...
    def get_object_size(self, easymaker_obs_uri):
        ...
        for obj in object_list:
            if (object_prefix.endswith("/") == False) and (obj == object_prefix):  # target object is file
                object_size_total += int(self._get_object_size(container_url, self._get_request_header(), object_prefix))
        ...
        if isDirectoryObject:
            for file_object in file_object_list:
                object_size_total += int(self._get_object_size(container_url, self._get_request_header(), file_object))
