...
class ClipDataTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        print('ClipDataTransformer: Fitting dataset, shape = {}, {}', X.shape, y.shape if y is not None else None)
        self.target = y
        return self

    def transform(self, X, y=None):
        print('ClipDataTransformer: Transforming dataset, shape = {}, {}', X.shape, y.shape if y is not None else None)
