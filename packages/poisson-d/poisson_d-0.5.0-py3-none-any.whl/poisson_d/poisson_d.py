...
class Grid:
    ...
    def add(self, point: Point, grey: float = 0):
...
def create_grid(
    ...
    console.print(
        f"Grid({rows}x{cols}) created with cell_size={math.floor(cell_size * 100)/100}"
    )
    return Grid(
        height=height,
        width=width,
    ...
    while to_process.not_empty() and len(sample_points) <= max_points:
        ...
        for _ in range(candidates_count):
            ...
            if new_point.in_rectangle(height, width) and not is_neighbour(
                ...
                greyscale = compute_greyscale(new_point, img, grey_func)
                grid.add(new_point, greyscale)
...
def min_dist_from(
    ...
    grey = compute_greyscale(point, img, grey_func)
...
def compute_greyscale(
    ...
    greyscale = (grey1 + grey2 + grey3 + grey4) / 4
    return greyscale
...
def task_cmyk(
    ...
    def function():
        ...
        points = filter_out_white(points, th)
...
def poisson_d_cmyk(
    ...
    task_c = task_cmyk("c", c, r, cnt, n, th, f)
    task_m = task_cmyk("m", m, r, cnt, n, th, f)
    task_y = task_cmyk("y", y, r, cnt, n, th, f)
    task_k = task_cmyk("k", k, r, cnt, n, th, f)
