...
def test_poisson_d_variant(img: np.ndarray, radius_min_max, candidates_count):
    """Test for poisson_d_work function."""
...
def test_filter_out_white():
    points = [
        p.Point(0, 0, 1.0),  # White
        p.Point(1, 1, 0.5),  # Grey
        p.Point(2, 1, 0.0),  # Black
    ]

    filtered_points = p.filter_out_white(points)
...
def test_compute_greyscale():
    def mock_grey_func(pixel):
        return sum(pixel) / 3 / 0xFF
    ...
    point = p.Point(0, 1)  # Middle of the first row
    assert p.compute_greyscale(point, img, mock_grey_func) == 0.41666666666666663
    ...
    point = p.Point(1, 2)
    assert p.compute_greyscale(point, img, mock_grey_func) == 0.5833333333333333

    # Test case 4: Out of bounds (negative index)
    point = p.Point(-1, -1)
    assert p.compute_greyscale(point, img, mock_grey_func) == 0.5
