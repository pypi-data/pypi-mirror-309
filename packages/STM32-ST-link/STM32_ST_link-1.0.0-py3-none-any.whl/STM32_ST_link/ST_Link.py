...
import logging
from Find_App import find_software_path
...
class STLinkCLI:
    ...
    def __init__(self, logging_level: int = logging.INFO):
        ...
        Args:
            logging_level: 日志级别
        """
        if not find_software_path("STM32 ST-LINK Utility"):
            raise FileNotFoundError("STM32 ST-LINK Utility not found")
        self.cli_path = os.path.join(find_software_path("STM32 ST-LINK Utility")[0], "ST-LINK Utility",
                                     "ST-LINK_CLI.exe")
        print(self.cli_path)
        ...
        if not os.path.exists(self.cli_path):
            raise FileNotFoundError(f"ST-LINK_CLI executable not found at: {self.cli_path}")
