...
from .convert import tecton_type_to_python_type
...
class FuncWrapper:
    ...
    def make_feature_view(
        self, update_params: Any = None, **rtfv_kwargs: Any
    ) -> FeatureView:
        name = self.name
        func = self.func
        func_name = func.__name__
        ...
        def service(request_context, *fvs):
            ...
            for key, fv in zip(fv_names, fvs):
                params[key] = fv
            if "agent_context" in request_context:
                ctx = json.loads(request_context["agent_context"])
            else:
                ctx = {}
            with agent_context(ctx, mode="update"):
...
def entity_to_pydantic(entity: Entity, fields: List[TectonField]) -> BaseModel:
    ...
    for name in entity.join_keys:
        schema[name] = (tecton_type_to_python_type(_fields[name].dtype), Field())
