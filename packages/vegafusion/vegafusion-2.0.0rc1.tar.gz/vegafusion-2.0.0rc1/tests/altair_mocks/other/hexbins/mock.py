# https://altair-viz.github.io/gallery/hexbins.html
...
alt.Chart(source).mark_point(size=size**2, shape=hexagon).encode(
    x=alt.X(
        "xFeaturePos:Q",
        axis=alt.Axis(title="Month", grid=False, tickOpacity=0, domainOpacity=0),
    ),
    y=alt.Y(
        "day(" + yField + "):O",
        axis=alt.Axis(title="Weekday", labelPadding=20, tickOpacity=0, domainOpacity=0),
    ),
