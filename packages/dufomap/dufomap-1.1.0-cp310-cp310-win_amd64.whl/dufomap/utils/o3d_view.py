...
class MyVisualizer:
    def __init__(self, view_file=None, window_title="Default", save_folder="logs/imgs"):
        ...
        self.view_file = view_file if os.path.exists(view_file) else None
        ...
        print(
            f"\n{window_title.capitalize()} initialized. View set: {self.view_file if self.view_file is not None else 'None'}\n"
            "Press:\n"
    ...
    def save_screen(self, figure_name=None):
        png_name = f"{str(figure_name if figure_name is not None else self.vis_id).zfill(3)}"
        png_file = f"{self.save_img_folder}/{png_name}.png"
        self.vis.capture_screen_image(png_file)
        self.vis_id += 1
