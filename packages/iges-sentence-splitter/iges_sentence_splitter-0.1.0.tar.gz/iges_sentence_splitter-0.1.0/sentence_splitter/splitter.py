...
class SentenceSplitter:
    ...
    def _is_model_downloaded(self):
        ...
        return os.path.exists(os.path.join(self.model_dir, 'model.safetensors'))
    ...
    def _download_model(self):
        ...
        model_files = {
            "model.safetensors": "1xc35kq2mKfPi78k-YYVPWjgIaXoRz_ru",
        }

        # Download each file
        for filename, file_id in model_files.items():
            file_path = os.path.join(self.model_dir, filename)
            if not os.path.exists(file_path):  # Skip download if file already exists
                ...
                if response.status_code == 200:
                    ...
                    raise RuntimeError(
                        f"Failed to download {filename} from Google Drive (status code: {response.status_code})")
