...
def _load_plugins_from_file(filename) -> list[click.Command]:
    ns = {}
    plugin_commands = []
    ...
    if not plugin_commands:
        logger.error(f"No plugin command found in {filename}.py")
...
def discover_plugins(cache_buster=False) -> dict[str, click.Command]:
    ...
    for filename in os.listdir(plugin_folder):
        if filename.endswith(".py") and filename != "__init__.py":
            ...
            plugin_commands = _load_plugins_from_file(filename)
