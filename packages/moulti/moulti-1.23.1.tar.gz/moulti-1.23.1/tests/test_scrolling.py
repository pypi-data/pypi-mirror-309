...
def test_programmatic_scrolling(moulti_test):
	...
	"""
	moulti_test_steps(moulti_test, 'tests/scripts/prog-scrolling-through-steps.bash', 7)

def test_lock_scroll(moulti_test):
	# No scroll lock: do not wait, let it scroll:
	assert moulti_test(command=['tests/scripts/manual-scrolling-through-steps.bash', '0'])
	# Scroll lock:
	async def scroll_lock(pilot):
		await pilot.press('l') # enable scroll lock
		await wait_script(pilot)
	assert moulti_test(command=['tests/scripts/manual-scrolling-through-steps.bash'], run_before=scroll_lock)
...
def test_auto_scrolling(moulti_test):
	...
	assert moulti_test(command=['tests/scripts/scrolling-inside-steps.bash'], run_before=interrupt_auto_scroll)
