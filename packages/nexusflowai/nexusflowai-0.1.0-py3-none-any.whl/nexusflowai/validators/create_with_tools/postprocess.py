...
from nexusflowai.validators.create_with_tools.utils import CodeExecutionHelper
from nexusflowai.validators.create_with_tools.preprocess import RavenFunctionCall
...
class FunctionCallResponseTranslator:
    ...
    def raw_response_to_tool_calls(
        ...
        for fun_calls in par_fun_calls:  # pylint: disable=redefined-argument-from-local
            ...
            chat_fc = dict()
            chat_fc["name"] = fun_call.name
            try:
                chat_fc["arguments"] = json.dumps(fun_call.kwargs)
