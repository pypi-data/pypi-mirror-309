...
import emoatlas.textloader as _load_object
...
def _make_baseline(
    ...
    if baseline:
        ...
        if not emotions:
            raise ValueError(
                "Baseline has no emotions. Fallback to the default baseline."
            )
    else:
    ...
    baseline_distr = [
        [list(k) for k in baseline_distr.keys()],
        list(baseline_distr.values()),
    ]
