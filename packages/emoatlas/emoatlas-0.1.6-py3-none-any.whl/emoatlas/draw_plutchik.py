...
import emoatlas.dyads as _dyads_pluthick_
import warnings
...
def _polar_coordinates(
    ...
    *ax*:
    ...
    for i in rg:
        c = plt.Circle(
            (0, 0), offset + i / 100, color="grey", alpha=0.3, fill=False, zorder=-20
        )
        ax.add_artist(c)

    if rescale:
        true_values = list(
            np.arange(rescale[0], rescale[1], (rescale[1] - rescale[0]) / 5)
        ) + [rescale[1]]
        true_values = true_values[-5:]

    else:
        true_values = np.arange(0.2, 1.2, 0.2)

    # Tick labels
    if show_ticklabels:
        for x, tv in zip(np.arange(0.2, 1.2, 0.2), true_values):
            ...
            ax.annotate(
                ...
                xy=(x, y),
                fontfamily=font,
                size=ticklabels_size,
                fontweight=fontweight,
                zorder=8,
                rotation=ticklabels_angle,
            )
    return ax


def _neutral_central_circle(ax, r=0.15):
    """
...
def _check_scores_kind(tags):
    ...
    *tags*:
    ...
    if len(unique_kinds) > 1:
        ...
        error_str = "Bad input: can't draw {} altogether. Please input only one of them as 'scores'.".format(
            unique_kinds_str
        )
...
def _draw_rejection_region(ax, reject_range, rescale, offset=0.15):
    ...
    *offset*:
    ...
    c = plt.Circle(
        (0, 0), mmax, color="grey", alpha=0.4, fill=False, zorder=-1, linestyle="--"
    )
    ax.add_artist(c)
    ...
    if reject_range[0] > rescale[0]:
        ...
        c = plt.Circle(
            (0, 0), mmin, color="grey", alpha=0.4, fill=False, zorder=-1, linestyle="--"
        )
        ax.add_artist(c)
...
def get_random_dyads(dyads_kind="primary"):
    ...
    *dyads_kind*:
    ...
    if dyads_kind == "primary":
        ...
        print(
            "Dyads kind must be one of 'primary', 'secondary', 'tertiary' or 'opposite'. Did not recognized '{}': fall back to primary dyads.".format(
                dyads_kind
            )
        )
        dyads = [
    ...
    *reject_range*:
    ...
    for key in scores.keys():
        ...
        if hasattr(scores[key], "__iter__"):
            if (not reject_range) and any([e < 0 for e in scores[key]]):
                raise Exception(
                    "Bad input for `{}`. Emotion/dyads scores should be positive.".format(
                        key
                    )
                )
            tmp = sum(scores[key])
    ...
    # Case B: users tell a reject_range, and they decide a normalization scale: we check correctness
        ...
        if (rescale[0] > reject_range[0]) or (rescale[1] < reject_range[1]):
            raise Exception(
                "Bad input for `rescale` and `reject_range` arguments: rejection range is outside the rescaled area."
            )

        if (rescale[0] > min_score) or (rescale[1] < max_score):
            raise Exception(
                "Bad input for `rescale` argument: at least one petal is outside the rescale area. Minimum values are ({}, {})".format(
                    min_score, max_score
                )
            )
    ...
    # Case D: users don't tell a reject_range nor a normalization: we decide [0,1] unless max_value is higher
        ...
        if min_score >= 0 and max_score <= 1:
            ...
            emo_diff = max_score - min_score
            min_score -= emo_diff * 0.1
            max_score += emo_diff * 0.1
            rescale = (min_score, max_score)
...
def draw_plutchik(
    ...
    *ax*:
    ...
    if emotions:
        ...
        for emo in emotions:
            # Draw emotion petal
            _emo_pluthick_._draw_emotion_petal(
                ax,
                emotion_score=emotions[emo],
                emotion=emo,
                font=font,
                fontweight=fontweight,
                fontsize=fontsize,
        ...
        for dyad in dyads:
            ...
            _dyads_pluthick_._draw_dyad_petal(
                ax,
                dyad_score=dyads[dyad],
                dyad=dyad,
                font=font,
                fontweight=fontweight,
                fontsize=fontsize,
        ...
        _, _, _, level = _dyads_pluthick_.dyad_params(
            list(dyads.keys())[0]
        )  # get the first dyad level (they all are the same)
        ll = level if level != 4 else "opp."  # what to annotate
        xy = (
            (-0.03, -0.03) if level != 4 else (-0.13, -0.03)
        )  # exact center of '1' or 'opp' is slightly different
        ax.annotate(
            text=ll,
            xy=xy,
            fontsize=fontsize,
    ...
    if title:
        ax.set_title(
            title, fontfamily=font, fontsize=title_size, fontweight="bold", pad=20
        )
