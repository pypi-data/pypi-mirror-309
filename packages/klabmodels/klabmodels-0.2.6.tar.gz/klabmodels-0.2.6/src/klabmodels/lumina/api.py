...
def save_chat(user: str, agent: str, msgs:List[Dict]):
  ...
  """
  try:
    chat_messages =[ChatMessage(role=msg['role'], content=msg['content']) for msg in msgs]
...
def summarize_chat(chat:Chat, max_words=100):
  ...
  prompt = f"Extract the subject of the conversation below and summarize in max {max_words} words:\nTEXT:\n{text}"
  ...
