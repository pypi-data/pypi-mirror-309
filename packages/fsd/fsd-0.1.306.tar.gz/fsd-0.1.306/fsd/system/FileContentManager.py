...
class FileContentManager:
    ...
    # More flexible delimiter patterns using regex
    HEAD_PATTERN = r'^[<]{2,}\s*(?:SEARCH|FIND|MATCH)\s*$'  # Matches <<< SEARCH, <<<< FIND, etc.
    DIVIDER_PATTERN = r'^[=]{2,}\s*$'  # Matches ===, ====, etc.
    UPDATED_PATTERN = r'^[>]{2,}\s*(?:REPLACE|UPDATE|WITH)\s*$'  # Matches >>> REPLACE, >>>> WITH, etc.
    ...
    def parse_search_replace_blocks_line_by_line(cls, response: str) -> List[Tuple[str, str]]:
        ...
        while i < len(lines):
            ...
            # More flexible head pattern matching
            if re.match(cls.HEAD_PATTERN, line) or any(marker in line.upper() for marker in ['SEARCH', 'FIND', 'MATCH']):
                try:
                    search_content = []
                    replace_content = []
                    i += 1
                    
                    # Collect search content until divider or updated pattern
                    while i < len(lines):
                        line = lines[i].strip()
                        if re.match(cls.DIVIDER_PATTERN, line) or re.match(cls.UPDATED_PATTERN, line):
                            break
                        search_content.append(lines[i])
                        i += 1
                        
                    # Skip divider(s)
                    while i < len(lines) and re.match(cls.DIVIDER_PATTERN, lines[i].strip()):
                        i += 1
                        
                    # Collect replace content until updated pattern
                    while i < len(lines):
                        line = lines[i].strip()
                        if re.match(cls.UPDATED_PATTERN, line) or any(marker in line.upper() for marker in ['REPLACE', 'UPDATE', 'WITH']):
                            break
                    ...
                    # Only add block if we have content
                    if search_content or replace_content:
                        blocks.append((
                    ...
                    logger.warning(f"Failed to parse block at line {i}: {str(e)}")
        ...
        for search, replace in blocks:
            if search:
                ...
                if new_content is None:
                    ...
                    #await cls.log_result(f"Search block not found in file {file_path}:\nSearch: {search}\nContent: {content}")
    ...
    async def process_coding_agent_response(cls, file_path: str, coding_agent_response: str):
        ...
        try:
            ...
            try:
                ...
                logger.debug("Line-by-line parsing failed. Attempting regex-based parsing.")
                try:
