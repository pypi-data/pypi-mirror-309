...
class CommandRunner:
    ...
    def open_terminal(self, bash_commands):
        ...
        """
        # Join commands with appropriate separator based on OS
        separator = ' && ' if platform.system() == 'Windows' else '; '
        bash_command = separator.join(bash_commands)
        ...
        try:
            if current_os == 'Windows':
                ...
                self._open_terminal_linux(project_path, bash_command)
            else:
    ...
    def _open_terminal_linux(self, project_path, bash_command):
        """
        Opens terminal on Linux, navigates to the project path, and runs the bash command.
            ...
            try:
                ...
                if subprocess.run(['which', terminal], stdout=subprocess.PIPE, stderr=subprocess.PIPE).returncode == 0:
                    if terminal == 'gnome-terminal':
                        cmd = [terminal, '--', 'bash', '-c', f'cd "{project_path}" && {bash_command}; exec bash']
                    else:
                        cmd = [terminal, '-e', f'bash -c \'cd "{project_path}" && {bash_command}; exec bash\'']
        ...
        logger.error("No suitable terminal emulator found on the system.")
        raise RuntimeError("No suitable terminal emulator found on the system.")
