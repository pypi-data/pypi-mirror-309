...
class ShortIdeaDevelopment:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        ...
        prompt = (
            ...
            f"CRITICAL: Under '#### Directory Structure', you MUST provide a tree structure that ONLY shows:\n"
            ...
            f"```\n"
            f"INCORRECT tree structure (do not include files just being modified):\n"
            f"```plaintext\n"
            f"project_root/\n"
            f"├── src/\n"
            f"│   ├── existing_file.py          # WRONG - Don't include files just being modified\n"
            f"│   └── new_file.py               # OK - New file\n"
            f"```\n"
