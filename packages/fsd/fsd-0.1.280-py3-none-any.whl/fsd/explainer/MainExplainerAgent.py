...
class MainExplainerAgent:
    ...
    async def get_answer_plan(self, conversation_history, user_prompt, language, all_file_content, role, file_attachments, focused_files, assets_link, crawl_logs):
        ...
        prompt += (
            ...
            "CRITICAL INSTRUCTIONS FOR CODE RESPONSES:\n"
            "1. ONLY provide full code implementations when the user EXPLICITLY and DIRECTLY requests them\n"
            "2. Otherwise, provide MINIMAL code snippets focused ONLY on key changes and critical sections\n" 
            "3. Default to high-level explanations and targeted recommendations over code dumps\n"
            "4. For each file modification, clearly state the full file path\n"
            "5. Focus on being an analyst - provide insights and guidance rather than complete solutions\n"
            "6. Project tree should ONLY be shown if specifically requested by user\n"
