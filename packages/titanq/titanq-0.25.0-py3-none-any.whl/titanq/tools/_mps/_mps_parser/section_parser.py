...
from pyparsing import LineEnd, Opt, Or, alphanums, nums, Group, OneOrMore, Word, printables
from typing import List, Union

from ..errors import MPSMalormedFileError, MPSMissingValueError
...
class NameParser(SectionParser):
    ...
    def parse(self) -> List[MPSName]:
        ...
        if len(self._lines) > 1:
            raise MPSMalormedFileError(f"Found more than 2 lines for section '{SectionType.NAME}'")
...
class RowsParser(SectionParser):
    ...
    def parse(self) -> List[MPSRow]:
        ...
        for line in self._lines:
            ...
            parsed_elements = element.parseString(line)
    ...
    def parse(self) -> List[MPSColumns]:
        ...
        for line in self._lines:
            ...
            if len(parsed_elements) > 3:
                raise MPSMalormedFileError(f"Found more than 2 rows identifier in a single '{SectionType.COLUMNS}' line")
...
class RhsParser(SectionParser):
    ...
    def parse(self) -> List[MPSRhs]:
        ...
        for line in self._lines:
            ...
            if len(parsed_elements) > 3:
                raise MPSMalormedFileError(f"Found more than 2 rows identifier in a single '{SectionType.RHS}' line")
...
class RangesParser(SectionParser):
    ...
    def parse(self) -> List[MPSRanges]:
        ...
        for line in self._lines:
            ...
            if len(parsed_elements) > 3:
                raise MPSMalormedFileError(f"Found more than 2 rows identifier in a single '{SectionType.RANGES}' line")
...
class QuadobjParser(SectionParser):
    ...
    def __init__(self, lines: List[str], type: Union[SectionType.QUADOBJ, SectionType.QMATRIX]) -> None:
        super().__init__(lines)
        self._type = type
...
class EndataParser(SectionParser):
    ...
    def parse(self) -> List[MPSEndata]:
        ...
        if len(self._lines) > 1:
            raise MPSMalormedFileError(f"Found more than 2 lines for section '{SectionType.ENDATA}'")
