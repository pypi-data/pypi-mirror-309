...
class NifiEntity(object):
    ...
    def contents(self):
        if not self._contents:
            if isinstance(self.request['contents_pointer'], dict) and self.request['contents_pointer'].get('location') == 'attachment':
                pointer = self.request['contents_pointer'].get('pointer') or f"{self.record['os_workspace']}/{self.record['os_entity_uid']}"
                self._contents = read_file.sync(pointer.split("/")[0], pointer.split("/")[-1], False, client=self.context.client)
