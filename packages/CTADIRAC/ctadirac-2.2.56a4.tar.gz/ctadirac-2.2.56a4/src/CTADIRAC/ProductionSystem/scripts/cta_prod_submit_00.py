...
def create_output_dir(workflow_config, workflow_element):
    ...
    path = prod_dm.createMDTopStructure(meta_data_json, base_path, program_category)
...
def build_workflow(workflow_config, prod_sys_client, prod_name, mode):
    ...
    for workflow_step in workflow_config["ProdSteps"]:
        ...
        output_dir_dict[workflow_step["ID"]] = path
...
def start_production(prod_sys_client, prod_name):
    ...
    print(json.dumps(prod_description))
...
def main():
    ...
    res = build_workflow(workflow_config, prod_sys_client, prod_name, mode)
    print(res)
