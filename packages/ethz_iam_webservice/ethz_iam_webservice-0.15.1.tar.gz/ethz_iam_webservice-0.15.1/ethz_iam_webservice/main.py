...
def get_person(
    ...
    try:
        ...
        raise ClickException(
            f"{exc} No person found with this identifier: {identifier}"
        ) from exc
    ...
    if add_user and username:
        ...
        # try updating the description of a persona
        try:
            person = iam.update_persona(
                username=person.username, description=description
            )
        except ValueError as exc:
            raise ClickException(exc) from exc
...
def get_user(
    ...
    try:
        ...
        raise ClickException(
            f"{exc}. No user found with this username: {username}"
        ) from exc
