...
class FrogMLStorage(Storage):
    ...
    def __upload_entity_version(
        ...
        try:
            ...
            return FrogMLEntityVersion(
                entity_name=entity_name, namespace=namespace, version=version
            )

        except FrogMLValidationError as error:
            logger.error(error.args[0], exc_info=False)

        return FrogMLEntityVersion(entity_name="", namespace="", version="")
    ...
    def __upload_single_file(self, args):
        ...
        if not self.__is_ignored_file(full_path=full_path):
    ...
    def __is_ignored_file(full_path: str) -> bool:
        return os.path.basename(full_path) in FROG_ML_IGNORED_FILES
