...
class Position(Enum):
    ...
    def get_moviepy_position(self, video, background_video, do_normalize: bool = False):
        ...
        if do_normalize:
            # TODO: These '-10000' and '10000' are hardcoded also
            # in CoordinateCenter and CoordinateCorner and must
            # be a constant in somewhere to read it from there. 
            # This is used to be able to work in our specific
            # GraphicRateFunction that works with normalized values
            position_tuple = (
                Math.normalize(position_tuple[0], -10000, 10000),
                Math.normalize(position_tuple[1], -10000, 10000)
            )
