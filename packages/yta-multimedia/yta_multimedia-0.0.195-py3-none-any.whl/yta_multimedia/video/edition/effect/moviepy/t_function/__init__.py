...
# TODO: Move this below to a better place
...
class MoviepyMovementPath:
    """
    Class to represent the path to follow by a moviepy
    clip that is going to move along that path. The clip
    will be set in the first provided position and will
    end in the last one provided.
    """
    positions: list
    """
    The array of positions, in order, to make the video
    follow the path. Each item is an array containing 
    [x, y] coordinate.
    """

    def __init__(self, positions: list[Union[CoordinateCenter, CoordinateCorner, Position]]):
        # background_video = BasePositionMoviepyEffect.get_black_background_clip(1)
        # background_video = BasePositionMoviepyEffect.prepare_background_clip(background_video)

        if not PythonValidator.is_list(positions):
            raise Exception('No list')
        
        [BasePositionMoviepyEffect.validate_position(position) for position in positions]

        background_video = BasePositionMoviepyEffect.get_black_background_clip(1)
        video = background_video = BasePositionMoviepyEffect.get_black_background_clip(1)

        # position = get_moviepy_position(video, background_video, position)
        tmp_positions = []
        for position in positions:
            if PythonValidator.is_instance(position, Position):
                tmp_positions.append(position.get_moviepy_position(video, background_video))
            else:
                tmp_positions.append(position.to_array())

        self.positions = tmp_positions

    # TODO: Continue
    def get_pos(self, t, duration):
        # TODO: Place in first position
        ...
        # Obtain the linear position according to a linear movement
        first_point = self.points[spline_index]
        second_point = self.points[spline_index + 1]

        return TFunctionSetPosition.linear(t, )
