...
class DB():
    ...
    def enqueue(self, tbl, rec, status_col='qstatus', order_col='qts'):
        if type(rec)!=dict: rec = {}
        rec[status_col],rec[order_col] = 0,now().isoformat()
        return self.sync_insert(tbl, rec)
    ...
    def dequeue(self, tbl, uid_col, status_col='qstatus', order_col='qts'):
        ...
        # If update fails, a concurrent process picked the same task, and the task is no longes available
