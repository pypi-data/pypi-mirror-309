...
from .._helper import build_item_from_value, build_query_result
        ...
        if op_parser.op_equals(StoreOperation.CREATE_COLLECTION):
            ...
            with self._lock:
                if (
                    ...
                    and collection_name not in self._db
                ):
            ...
            with self._lock:
                if where is None:
                    ...
                    if not QueryProcessor.eval_expr(
                        ...
                        raise PreconditionFailedError
                    data[db_key] = document
            result = build_item_from_value(processor=processor, value=document)
        # UPDATE
            ...
            if processor.needs_local_etag():
                ...
                set = processor.add_etag_update(set, etag)
            current_document = None
            ...
            with self._lock:
                ...
                updated_document = QueryProcessor.update_item(
                    current_document, set, processor.resolve_field
                )
