...
MANUAL = "manual"
IIIF_URL = "{image_url}/{bbox}/full/0/default{image_format}"
...
def download_image(
    ...
    try:
        ...
        image = PIL_Image.open(BytesIO(resp.content))
        if resize:
        ...
        raise ImageDownloadError(url=url, exc=e) from None


def download_images(urls_paths: list[dict[str, str | tuple[int, int]]]):
    """
...
def get_bbox(polygon: str) -> str:
    """
    Arkindex polygon stored as string.
    Returns a comma-separated string of upper left-most pixel, width and height of the element image.
    """
...
def _parse_polygon(polygon: str) -> Iterator[int]:
    return map(int, get_bbox(polygon).split(","))
