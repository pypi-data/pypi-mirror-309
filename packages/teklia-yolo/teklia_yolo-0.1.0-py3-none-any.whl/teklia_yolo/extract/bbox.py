...
class BaseZone:
    ...
    def __post_init__(self):
        (
            ...
            self.parent_width,
            self.parent_height,
        ) = _parse_polygon(self.parent_polygon)
...
class Zone(BaseZone):
    ...
    def __init__(self, class_idx: int, polygon: str, parent_polygon: str):
        ...
        self.x, self.y, self.width, self.height = _parse_polygon(polygon)
...
def run(
    ...
    Extractor(
        ...
        zone_class=Zone if task == YoloTask.Detect else SegmZone,
    ).run(
        output=datasets_dir,
