...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/formula/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/formula/*")
            ...
            #    "--include=src/saltext/formula/*,tests/*"
            # )
        finally:
