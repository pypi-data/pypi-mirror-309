...
class FFTOptions:
    ...
    Attributes:
        ...
        device_id: CUDA device ordinal (used if the operand resides on the CPU). Device 0 will be used if not specified.
            .. deprecated:: 0.0.2
               The ``device_id`` should be specified as ``execution`` option, see :py:obj:`ExecutionCUDA.device_id`.
...
class DeviceCallable:
    ...
    def __post_init__(self):
        ...
        if isinstance(self.ltoir, int) and self.size is None:
            raise ValueError("The size of the LTO-IR code specified as a pointer must be explicitly provided using the 'size' option.")
