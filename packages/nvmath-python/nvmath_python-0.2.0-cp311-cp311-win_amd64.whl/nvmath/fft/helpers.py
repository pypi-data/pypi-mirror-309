...
def _get_compiler():
    ...
    def _get_compiler_engine():
        ...
        if version == (0, 59):
            ...
            raise RuntimeError(
                f"Unsupported Numba version: {numba.version_info.string}, "
                f"this feature needs Numba 0.59 or 0.60."
            )
...
SHARED_FFT_HELPER_DOCUMENTATION = {
    ...
    "user_info_dtype": (
        ...
        "The offset is computed based on the memory layout (shape and strides) of the operand (input for prolog, output for epilog)."
        "If the user would like to pass additional tensor as `user_info` and access it based on the offset, it is crucial to know memory layout of the operand."
        "Please note, the actual layout of the input tensor may differ from the layout of the tensor passed to fft call."
        "To learn the memory layout of the input or output, please use stateful FFT API and :meth:`nvmath.fft.FFT.get_input_layout` :meth:`nvmath.fft.FFT.get_output_layout` respectively.\n",
    ),
