...
Make it feel like a friend giving weather advice!"""
        ...
        try:
            ...
            yield f"\n{self.EMOJIS['warning']} Oops! Had a little hiccup: {str(e)} 😅"

    def get_insights(self, location: Optional[str] = None) -> str:
        """Get comprehensive weather insights."""
        try:
            loc = location or self._get_ip_location()
            weather_data = self._get_weather_data(loc)
            return self._format_weather_data(weather_data)

        except Exception as e:
            return f"{self.EMOJIS['warning']} Whoops! Something went wrong: {str(e)} 😅"

def print_weather_insights(location=None):
    """Print weather insights for a location."""
    analyzer = WeatherAnalyzer()
    print(analyzer.get_insights(location))

def main():
    """Run weather analysis with error handling."""
    try:
        # You can specify a location or let it use IP-based location
        print_weather_insights("Qazigund")
    except KeyboardInterrupt:
        print("\n\n👋 Thanks for watching! Catch you on the next forecast! 🌈\n")
    except Exception as e:
        print(f"\n😅 Technical difficulties: {str(e)}\n")
