...
from .utils import seconds_until_session_end, seconds_until_idle_time_end
...
def auto_logout_client(request):
    ...
    options = getattr(settings, 'AUTO_LOGOUT', None)
    if not options:
        return {}
    ...
    if options.get('REDIRECT_TO_LOGIN_NOW'):
        at = None
        ...
        if at:
            ctx['redirect_to_login_now'] = mark_safe(_trim(LOGOUT_TIMEOUT_SCRIPT_PATTERN % at))
