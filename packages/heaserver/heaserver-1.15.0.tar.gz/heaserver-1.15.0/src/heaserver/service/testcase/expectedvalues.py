...
from heaobject.root import is_primitive, is_primitive_list, is_heaobject_dict_list, is_heaobject_dict, HEAObjectDict, \
    HEAObjectDictValue, Union, DesktopObjectDict, desktop_object_from_dict, Permission, PermissionContext, are_permissions_read_only
...
async def expected_one(fixtures: Mapping[CollectionKey | str, List[DesktopObjectDict]],
    ...
    desktop_object = desktop_object_from_dict(obj)
    ...
    for action, action_name, rel, itemif in ((wstl_builder.find_action(a.name), a.name, a.rel, a.itemif) for a in get_actions_):
        ...
        if itemif is None or get_eval_for(obj).eval(itemif):
            _set_collection_template(action, collection, obj, 1, rel, context.sub, attr_perms)
    return [collection]
...
async def expected_all(fixtures: Mapping[CollectionKey | str, List[DesktopObjectDict]],
    ...
    for action, action_name, rel, itemif in ((wstl_builder.find_action(a.name), a.name, a.rel, a.itemif) for a in get_all_actions_):
        ...
        if len(set(i['type'] for i in collection)) < 2 and (itemif is None or get_eval_for(f).eval(itemif)):
            _set_collection_template(action, collection_doc, f, len(collection), rel, context.sub, attr_perms)
    top_level_links_ = top_level_links()
    if top_level_links_:
...
def _template_data_generator(sub: str, action: dict[str, Any], desktop_object_dict: DesktopObjectDict, attr_perms: dict[str, list[Permission]], len_fixtures=1):
                ...
                if wstl.has_section(input):
                    if val:
                        ...
                        for i_, v in enumerate(val):
                            yield _new_template_data_item(input, v.get(nm, input.get('value')), sub, perms) | {
                                'index': i_,
                                'section': section
                            } | _section_prompt_key_value(input)
                    else:
...
def _set_collection_template(action, collection_doc, fixture, len_fixtures, rel, sub, attr_perms):
    """
    ...
    if 'cj-template' in targets:
        template = {'data': [d for d in _template_data_generator(sub, action, fixture, attr_perms, len_fixtures)],
...
async def _expected_one_form(
    ...
    template_data = [d for d in _template_data_generator(context.sub, action, obj_dict, attr_perms)]
