...
def EcoDataProGet(indicID, beginDate = "", endDate = "", field = ""):
    ...
    api_name_info = DATAYES.EcoInfoProGet(indicID=indicID, field='indicID,dataApiName,indicName,frequency,unit').drop_duplicates()
    df_list = []
    for idx, row in api_name_info.iterrows():
        ...
        df.insert(1, 'indicName', row['indicName'])
        df.insert(2, 'frequency', row['frequency'])
        df.insert(3, 'unit', row['unit'])
        df_list.append(df)
    if df_list:
