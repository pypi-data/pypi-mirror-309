...
class SopInfraViewTestCase(TestCase):
    ...
    def test_recompute_sizing_perm(self):
        """test recompute sizing perm"""
        instance = SopInfra.objects.first()
        infra = SopInfra.objects.get(site=instance)
        infra.ad_direct_users = 53
        url = self.get_action_url('refresh')

        self.add_permissions(EDIT_PERM)
        response = self.client.get(url)
        self.assertHttpStatus(response, 200)
        self.assertEqual(response.context['return_url'], reverse('plugins:sop_infra:sizing_list'))

        response = self.client.get(f'{url}?qs={instance.pk}')
        self.assertHttpStatus(response, 200)
        self.assertEqual(response.context['return_url'], instance.get_absolute_url())
