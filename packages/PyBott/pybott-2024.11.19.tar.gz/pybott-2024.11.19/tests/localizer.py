...
def localized_dirac_operator(lambda_param, x_op, y_op, ham, kappa):
    """
    ...
    result = np.block([[top_left, kappa*top_right], [kappa*bottom_left, bottom_right]])

    return result

def localized_dirac_operator_2(lambda_param, x_op, y_op, ham, kappa):
    dim_orb = (ham.shape[0] - x_op.shape[0])//x_op.shape[0]
    if dim_orb != 0:
        x_op = np.diag(np.repeat(np.diag(x_op), dim_orb+1))
        y_op = np.diag(np.repeat(np.diag(y_op), dim_orb+1))
    ...
    ldo = np.sum([np.kron(sigma_z, kappa*(x_op-lambda_1 * np.eye(n_size))),
                  np.kron(sigma_x, kappa*(y_op-lambda_2 * np.eye(n_size))),
                  np.kron(sigma_y, kappa*(ham-lambda_3 * np.eye(n_size)))],
                 axis=0)

    return ldo

def localizer_index_spectrum(lambda_param, x_op, y_op, ham, kappa):
    ldo = localized_dirac_operator(lambda_param, x_op, y_op, ham, kappa)
...
def localizer_index(lattice, ham, class_az='A2D', lambda_param=np.array([0, 0, 0]), kappa=1):
    ...
    if class_az == 'A2D':
        evals = localizer_index_spectrum(lambda_param, x_op, y_op, ham, kappa)
        return 0.5 * np.sum(np.sign(evals))

    elif class_az == 'AII2D':
        ldo = localized_dirac_operator_2(lambda_param, x_op, y_op, ham, kappa)
        ...
        # if np.max(np.abs(qhq+qhq.T))>1e-14 :#not np.allclose(qhq, -qhq.T, atol=1e-14):
        #     return np.nan
        ...
        # return np.real(np.sign(np.linalg.det(qhq)))
