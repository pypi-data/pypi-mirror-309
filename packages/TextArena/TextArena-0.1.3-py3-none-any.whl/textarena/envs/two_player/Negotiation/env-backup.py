...
class NegotiationEnv(ta.Env):
    ...
    def _attempt_to_execute_trade(self, player_id: int, action: int) -> None:
        ...
        if proposer_valid and acceptor_valid:
            ...
            for resource, qty in self.state.game_state["current_offer"]["offered_resources"].items():
            ...
            for resource, qty in self.state.game_state["current_offer"]["requested_resources"].items():
                self.state.game_state["player_resources"][player_id][resource] -= qty
                self.state.game_state["player_resources"][1-player_id][resource] += qty
            ...
            self.state.add_observation(
                from_id=ta.GAME_ID,
                to_id=-1, # Broadcast to all
                message=f"Player {player_id} accepted the trade offer from Player {1-player_id}."
            )
