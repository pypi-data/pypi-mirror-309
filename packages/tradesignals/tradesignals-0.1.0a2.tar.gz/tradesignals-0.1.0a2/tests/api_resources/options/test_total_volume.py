...
class TestTotalVolume:
    ...
    def test_method_list(self, client: TradesignalsIo) -> None:
        total_volume = client.options.total_volume.list()
        assert_matches_type(TotalVolumeListResponse, total_volume, path=["response"])
    ...
    def test_raw_response_list(self, client: TradesignalsIo) -> None:
        response = client.options.total_volume.with_raw_response.list()
    ...
    def test_streaming_response_list(self, client: TradesignalsIo) -> None:
        with client.options.total_volume.with_streaming_response.list() as response:
...
class TestAsyncTotalVolume:
    ...
    async def test_method_list(self, async_client: AsyncTradesignalsIo) -> None:
        total_volume = await async_client.options.total_volume.list()
        assert_matches_type(TotalVolumeListResponse, total_volume, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncTradesignalsIo) -> None:
        response = await async_client.options.total_volume.with_raw_response.list()
    ...
    async def test_streaming_response_list(self, async_client: AsyncTradesignalsIo) -> None:
        async with async_client.options.total_volume.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
