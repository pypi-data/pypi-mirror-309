...
class MultipleChoiceQuestion(Question):
    ...
    def display(self):
        print("\n" + Fore.CYAN + "🔢 Pregunta de elección múltiple:" + Style.RESET_ALL)
        ...
        while True:
            answer = input("Escoge una o más opciones y separalas con comas (e.g., a,c): ").strip().lower()
            ...
            if all(ans in option_letters for ans in selected_answers):
                ...
                print(Fore.RED + "Respuesta invalida. Por favor responda con letras válidas separadas por comas." + Style.RESET_ALL)
    ...
    def scoring_mode_puntuacion_parcial(self, selected_indexes, correct_set, option_letters):
        ...
        if self.immediate_feedback:
            if score_fraction == 1:
                ...
                print(Fore.YELLOW + "Has respondido parcialmente bien." + Style.RESET_ALL)
            else:
                correct_answers_text = ", ".join([f"{option_letters[i]}. {self.options[i]}" for i in correct_set])
                print(Fore.RED + f"Incorrecto. Las respuestas correctas eran: {correct_answers_text}" + Style.RESET_ALL)
        return score
    ...
    def scoring_mode_bonificacion_completa(self, selected_indexes, correct_set, option_letters):
        ...
        if num_correct_selected == total_correct and len(incorrect_answers) == 0:
            ...
            if self.immediate_feedback:
                print(Fore.GREEN + "¡Perfecto! Has tenido todas las respuetas bien." + Style.RESET_ALL)
