...
class Gatiab(object):
    ...
    def calc_od(self, gas_content, save=False, dir_save='./'):
        """
        Compute gaseous optical depths for a particular gas content
        ...
        -------
        L : xr.DataArray
            Look-up table with the gas optical depth as function of the
            instrument band, airmass, gas content and ground level pressure
        """
        ...
        if self.gas == 'O3':
            ...
            dens_gas_U =  self.dens_gas_hl * (gas_content / molar_mass[self.gas.lower()] * constants.Avogadro / (simpson(y=self.p_gas_hl, x=-self.z_atm) * 1e5))
        ...
        tau[tau<0] = 0.
            ...
            # Fourth step: Create final LUT and optionnaly save
        ds = xr.Dataset(coords={'z_atm':self.z_atm,
                                'wavenumber':self.wavenumber})
        ds['optical_depth'] = xr.DataArray(tau.astype(np.float32), dims=['z_atm', 'wavenumber'], attrs={'units':'None' , 'description':'cumulated optical depth'})
        ds['z_atm'].attrs = {'units':'Kilometer' , 'description':'atmosphere height profil'}
        date = datetime.now().strftime("%Y-%m-%d")
        ds.attrs = {'name': 'Spectral optical depth profiles of ' + self.gas.lower() + ' for ' + str(gas_content) + ' ' + units,
                    'experiment': self.atm + ' based on Idealized CKDMIP interpolation',
                    'date':date}
        # if save :
        #     save_filename = f"od_{gas}_{atm}_ckdmip_idealized_solar_spectra.nc"
        #     path_to_file = Path.joinpath(Path(dir_save), save_filename)
