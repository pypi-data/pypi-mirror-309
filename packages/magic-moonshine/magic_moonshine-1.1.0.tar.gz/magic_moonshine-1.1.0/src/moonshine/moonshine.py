...
class VideoTarget:
    ...
    Attributes:
        ...
        timestamp (float): The target timestamp.
    
    Raises:
        ValueError: If timestamp or video_id is not provided.
    """
    video_id: str
    timestamp: float
    ...
    def __post_init__(self):
        """
        Validate that both timestamp and video_id are provided.
...
def inquire(index: str, prompt: str, guidance: Optional[str] = None, 
    ...
    if target:
        if isinstance(target, VideoTarget):
            ...
            if target.video_id is not None:
                payload['target_video'] = target.video_id
        else:
            raise ValueError("Target must be a VideoTarget object. Hint: moonshine.VideoTarget(timestamp, video_id)")
