...
class MultiRtfvFeatureExtractionNode(QueryNode):
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        sql_builder = FeatureExtractionSqlBuilder(
            self.events_df_timestamp_field, self.input_node.columns, self.use_namespace_feature_prefix
        )

        feature_query_terms = []
        for fdw, namespace in self.feature_definition_namespaces:
            feature_query_terms.extend(sql_builder.extract_calculations(fdw, namespace))
            feature_query_terms.extend(sql_builder.extract_attributes(fdw, namespace))
        return (
