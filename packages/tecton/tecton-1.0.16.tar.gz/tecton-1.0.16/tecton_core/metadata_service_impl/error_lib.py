...
def raise_for_grpc_status(status_code: int, details: str, host_url: str, request_provider: RequestProvider) -> None:
    ...
    if status_code == gRPCStatus.UNAUTHENTICATED.value:
        msg = f"Tecton credentials are invalid, not configured, or expired ({details}). To authenticate using an API key, set TECTON_API_KEY in your environment or use tecton.set_credentials(tecton_api_key=<key>). To authenticate as your user, run `tecton login` with the CLI or `tecton.login(tecton_url=<url>)` in your notebook."
