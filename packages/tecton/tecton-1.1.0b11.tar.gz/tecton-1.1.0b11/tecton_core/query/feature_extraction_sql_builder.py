...
class FeatureExtractionSqlBuilder:
    ...
    def _get_feature_view_node_internal_column_name(
        fdw: FeatureDefinitionWrapper, source_fv_pipeline_node: PipelineNode, feature_name: str
    ) -> str:
    ...
    def _get_internal_column_name(self, column_reference: str, fdw: FeatureDefinitionWrapper) -> str:
        ...
        if source_pipeline_node.HasField("feature_view_node"):
            internal_column_name = self._get_feature_view_node_internal_column_name(
                fdw, source_pipeline_node, feature_name
            )
        elif (
    ...
    def extract_calculations(self, fdw: FeatureDefinitionWrapper, namespace: str) -> List[Term]:
        separator = fdw.namespace_separator
        column_reference_resolver = partial(self._get_internal_column_name, fdw=fdw)
        calculation_query_terms = []
        for calc in fdw.fv_spec.calculation_features:
            output_column = f"{namespace}{separator}{calc.name}" if self.use_namespace_feature_prefix else calc.name
            calculation_query_term = calc.root.to_query_term(column_reference_resolver).as_(output_column)
            calculation_query_terms.append(calculation_query_term)
        return calculation_query_terms

    def extract_attributes(self, fdw: FeatureDefinitionWrapper, namespace: str) -> List[Term]:
        separator = fdw.namespace_separator
        attribute_query_terms = []
        for attribute in fdw.fv_spec.attribute_features:
            internal_column_name = self._get_internal_column_name(attribute.name, fdw)
            output_column = (
                f"{namespace}{separator}{attribute.name}" if self.use_namespace_feature_prefix else attribute.name
            )
