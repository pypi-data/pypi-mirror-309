...
from xiaoqiangclub.utils.module_installer import install_module
from xiaoqiangclub.utils.terminal_command_executor import execute_terminal_command
...
def install_playwright(browser: str = 'chromium') -> bool:
    ...
    if install_module("playwright"):
        ...
        ret = execute_terminal_command(f"playwright install {browser}", stream_stdout=True)
...
def playwright_codegen(url: str, file_name: str = None, proxy: str = None, directory: str = None) -> bool:
    ...
    ret = execute_terminal_command(command)
