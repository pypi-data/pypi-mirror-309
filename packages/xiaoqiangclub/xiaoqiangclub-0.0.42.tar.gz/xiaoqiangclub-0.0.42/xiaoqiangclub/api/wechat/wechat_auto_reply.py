...
class WeChatAutoReplyBase:
    ...
    @staticmethod
    async def _handle_reply(reply_method, message, user_open_id: str, wechat_id: str):
        """
        处理回复方法，检查是否为协程并调用
        ...
        if inspect.iscoroutinefunction(reply_method):
            return await reply_method(message, user_open_id, wechat_id)
        else:
            return reply_method(message, user_open_id, wechat_id)
    ...
    async def _process_message(self, msg) -> Optional[str]:
        ...
        # 调用相应的回复方法
        reply_content = await self._handle_reply(reply_method, message, user_open_id, wechat_id)
    ...
    @staticmethod
    async def short_video_reply(media_id: str, user_open_id: str, wechat_id: str) -> Optional[str]:
        """
        回复短视频消息
    ...
    @staticmethod
    async def location_reply(location: tuple, user_open_id: str, wechat_id: str) -> Optional[str]:
        """
        回复地理位置消息
    ...
    @staticmethod
    async def event_reply(msg, user_open_id: str, wechat_id: str) -> Optional[str]:
        """
        回复事件消息
