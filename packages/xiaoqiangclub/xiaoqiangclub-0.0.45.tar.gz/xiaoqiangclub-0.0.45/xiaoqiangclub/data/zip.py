...
def add_files_to_zip(zip_file_path: str, files_to_add: List[Union[str, bytes]], password: Optional[str] = None,
    ...
    :param new_zip_file_path: 新的 ZIP 文件路径，如果提供，将生成一个新的 ZIP 文件而不修改原始 zip_file_path 文件。
    """
    try:
...
def zip_file(items: List[Union[str, bytes]], zip_file_path: str, password: Optional[str] = None) -> Optional[str]:
    """压缩指定的文件和文件夹到 ZIP 文件中，并可设置密码。

    :param items: 要压缩的文件和文件夹路径列表。
    :param zip_file_path: 目标 ZIP 文件的路径。
    :param password: 可选的密码，用于加密 ZIP 文件。
    """
    try:
        with pyzipper.AESZipFile(zip_file_path, 'w', compression=pyzipper.ZIP_DEFLATED,
            ...
            for item in items:
                if os.path.isfile(item):  # 如果是文件
                    ...
                    for root, _, files in os.walk(item):  # 遍历文件夹
                        for file in files:
                            file_path = os.path.join(root, file)
                            # 使用相对路径保持文件夹结构
        ...
        log.info(f"压缩成功！压缩文件路径：{zip_file_path}")  # 记录压缩成功的信息
