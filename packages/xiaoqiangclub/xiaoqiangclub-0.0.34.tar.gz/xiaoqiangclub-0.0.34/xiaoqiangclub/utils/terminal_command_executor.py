...
async def execute_terminal_command_async(cmd: str, stream_stdout: bool = True) -> Optional[str]:
    """
    异步运行给定的终端命令，并返回命令的输出作为Python字符串
    示例命令: await execute_terminal_command_async('ls -l')
...
def execute_terminal_command(cmd: str, stream_stdout: bool = True) -> Optional[str]:
    """
    同步方式运行给定的终端命令，并返回命令的输出作为Python字符串
    示例命令: execute_terminal_command('ls -l')
