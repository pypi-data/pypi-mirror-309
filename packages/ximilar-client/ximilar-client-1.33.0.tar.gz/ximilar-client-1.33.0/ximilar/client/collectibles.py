...
class CollectiblesRecognitionClient(RestClient):
    ...
    def construct_data(self, records=[], lang=False, slab_id=False, slab_grade=False, fields_to_return=None):
        if len(records) == 0:
            raise Exception("Please specify at least one record in detect method!")
        data = {RECORDS: self.preprocess_records(records)}
    ...
    def card_id(
        self, records: list, lang: bool = False, slab_id: bool = False, slab_grade: bool = False, fields_to_return=None
    ):
        result = self.post(
            COLLECTIBLES_CARD_ID,
            data=self.construct_data(
                records, lang=lang, slab_id=slab_id, slab_grade=slab_grade, fields_to_return=fields_to_return
            ),
        )
        return result
