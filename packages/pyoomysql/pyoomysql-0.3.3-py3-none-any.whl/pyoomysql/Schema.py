...
class Schema:
    ...
    def get_tables(self):
        if self.exists:
            result = self.database.execute(f"SELECT table_schema AS schema_name, table_name AS table_name, table_type AS table_type, table_rows AS table_rows, avg_row_length AS avg_row_length, max_data_length AS max_data_length FROM information_schema.tables WHERE table_schema = '{self.name}' ORDER by 1,2")
            tables = {}
    ...
    def get_table(self, table_name):
        if self.exists:
            result = self.database.execute(f"SELECT table_schema AS schema_name, table_name AS table_name, table_type, table_rows, avg_row_length, max_data_length FROM information_schema.tables WHERE table_schema = '{self.name}' AND table_name = '{table_name}' ORDER by 1,2")
            table = {}
            if len(result) > 0:
