...
from . import package, settings
from .package import AvailablePackage
...
def search_packages(args):
    ...
    for pkg in available_packages:
        ...
        if args.to_lang and args.to_lang != pkg.to_code:
            continue
        print(f"{package.argospm_package_name(pkg)}: " + f"{pkg.from_code} -> {pkg.to_code}")
...
def main():
    ...
    update_parser = subparser.add_parser("update", help="Downloads remote package index.")
    update_parser.set_defaults(callback=update_index)

    search_parser = subparser.add_parser("search", help="Search package from remote index.")
