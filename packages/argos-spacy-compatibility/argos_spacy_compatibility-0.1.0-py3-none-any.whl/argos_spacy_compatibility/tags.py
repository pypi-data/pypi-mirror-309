...
from .translate import ITranslation
...
class Tag(ITag):
    ...
    def text(self) -> str:
        return "".join([(child.text() if type(child) != str else child) for child in self.children])
...
def inject_tags_inference(underlying_translation: ITranslation, tag: ITag) -> ITag | None:
    ...
    for child in tag.children:
        if depth(child) == 1:
            translated = translate_preserve_formatting(underlying_translation, child.text())
...
def translate_tags(underlying_translation: ITranslation, tag: ITag | str) -> ITag | str:
    ...
    if type(tag) is str:
        ...
        tag.children = [translate_tags(underlying_translation, child) for child in tag.children]
