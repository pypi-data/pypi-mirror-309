...
from . import apis, fewshot, package, sbd, settings
from .models import ILanguageModel
from .package import Package
from .sbd import SpacySentencizerSmall
    ...
    def hypotheses(self, input_text: str, num_hypotheses: int = 4) -> list[Hypothesis]:
        ...
        for t1_hypothesis in t1_hypotheses:
            ...
            for t2_hypothesis in t2_hypotheses:
                to_return.append(
                    Hypothesis(t2_hypothesis.value, t1_hypothesis.score + t2_hypothesis.score)
                )
...
class CachedTranslation(ITranslation):
    ...
    def hypotheses(self, input_text: str, num_hypotheses: int = 4) -> list[Hypothesis]:
        ...
        for paragraph in paragraphs:
            ...
            if translated_paragraph is None or len(translated_paragraph) != num_hypotheses:
...
def get_installed_languages() -> list[Language]:
    ...
    if settings.model_provider == settings.ModelProvider.OPENNMT:
        ...
        for pkg in packages:
            ...
            if len(contain) == 0:
                translation_to_add = CachedTranslation(PackageTranslation(from_lang, to_lang, pkg))
        ...
        supported_languages = libretranslate_api.languages()  # [{"code":"en", "name":"English"}]
        languages = [Language(l["code"], l["name"]) for l in supported_languages]
        for from_lang in languages:
            for to_lang in languages:
                translation = LibreTranslateTranslation(from_lang, to_lang, libretranslate_api)
