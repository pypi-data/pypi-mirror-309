...
class UIWindow(QMainWindow):
    ...
    def btn_clicked(self):
        ...
        if btn.url:
            ...
            return
        btn_name = btn.objectName()
        for submenus_obj in self.left_menu.list_button_frame:
            if submenus_obj['parent_btn_id'] == btn_name:
                if submenus_obj['frame_object'].isHidden():
                    self.left_menu.toggle_animation(False)
                    self.animate_frame(self.left_menu.parent, submenus_obj['frame_object'])
                else:
                    submenus_obj['frame_object'].hide()
            elif not submenus_obj['frame_object'].isHidden() and btn_name not in submenus_obj['btn_id_list']:
                submenus_obj['frame_object'].hide()
