...
def mag_dist_loop(conf, iP=None, filename=None, M=None, Repi=None):
    ...
    for i in range(nepi):
        for j in range(nmag):
            ...
            print(
                "        Proc %d j=%d of %d %s"
                % (iP, j + 1, nmag, datetime.datetime.now().isoformat())
            )
        print(
            "Proc %d done with %d of %d distances %s"
...
def single_event_loop(
    conf, iP=0, rjb_filename="junk", rrup_filename="junk", M=6.0, Repi=None
):
    """
    Args:
        conf(ConfigObj): The configuration info. See
    ...
    for i in range(0, nepi):
        if conf["bytheta"] is True:
            ...
            for j in range(0, conf["ntheta"]):
                rrup_var, rrup_avg, rjb_var, rjb_avg = single_event_inner_loop(
                    conf, Repi[i], theta=np.array([theta[j]]), ntheta=1
                )
                ...
                print(
                    "        Proc %d j=%d of %d %s"
                    % (iP, j + 1, conf["ntheta"], datetime.datetime.now().isoformat())
                )
                print(
                    "Proc %d done with %d of %d distances %s"
                    % (iP, i + 1, nepi, datetime.datetime.now().isoformat())
            ...
            rrup_var, rrup_avg, rjb_var, rjb_avg = single_event_inner_loop(
                conf, Repi[i], theta=0, ntheta=conf["ntheta"]
            )
            ...
            print(
                "        Proc %d j=%d of %d %s"
                % (iP, 1, 1, datetime.datetime.now().isoformat())
            )
            print(
                "Proc %d done with %d of %d distances %s"
                % (iP, i + 1, nepi, datetime.datetime.now().isoformat())
...
def rjb_inner_loop(M, Repi, conf):
    ...
    length, sig_length, width, sigw, area, sig_area = dimensions_from_magnitude(
        M, conf["rup_dim_model"], conf["neps"], conf["trunc"], conf["mech"]
    )

    if conf["LW"] is True:
        if length is None or width is None:
            raise Exception(
                "Selected model does not support direct "
        ...
        for l in range(nl):
            for k in range(conf["ndip"]):
                ...
                for i in range(nx):
                    ...
                    integrand_x2[i] = one_over_ll * np.trapz(integrand_y2, dx=dy)
                if dx == 0:
                    integrand_dip[k] = integrand_x[0]
                    integrand_dip2[k] = integrand_x2[0]
                else:
                    integrand_dip[k] = one_over_sw * np.trapz(integrand_x, dx=dx)
                    integrand_dip2[k] = one_over_sw * np.trapz(integrand_x2, dx=dx)
            if conf["ndip"] == 1:
                integrand_length[l] = integrand_dip[0]
                integrand_length2[l] = integrand_dip2[0]
            else:
                integrand_length[l] = dipnorm * np.trapz(integrand_dip, dx=ddip)
                integrand_length2[l] = dipnorm * np.trapz(integrand_dip2, dx=ddip)
        if nw == 1:
            ...
            for k in range(conf["ndip"]):
                ...
                for z in range(nz):
                    for i in range(nx):
                        ...
                        for j in range(ny):
                            ...
                            if dip[k] == np.pi / 2:
                                ...
                                Rrupp[r2] = Rx[r2] * np.sin(dip[k]) + Ztor[z] * np.cos(
                                    dip[k]
                                )
                            ...
                            integrand_y[j] = one_over_2pi * np.trapz(Rrup, dx=dt)
                            integrand_y2[j] = one_over_2pi * np.trapz(Rrup2, dx=dt)
                    ...
                    if dx == 0:
                        ...
                        integrand_depth[z] = one_over_sw * np.trapz(integrand_x, dx=dx)
                        integrand_depth2[z] = one_over_sw * np.trapz(
                            integrand_x2, dx=dx
                        )
                # end z loop
                if dz == 0:
                    ...
                    integrand_dip[k] = one_over_ztormax * np.trapz(
                        integrand_depth, dx=dz
                    )
                    integrand_dip2[k] = one_over_ztormax * np.trapz(
                        integrand_depth2, dx=dz
                    )
            if conf["ndip"] == 1:
                integrand_length[l] = integrand_dip[0]
                integrand_length2[l] = integrand_dip2[0]
...
def single_event_inner_loop(conf, Repi, theta=0, ntheta=73):
    ...
    length, sig_length, width, sigw, area, sig_area = dimensions_from_magnitude(
        conf["M"], conf["rup_dim_model"], conf["neps"], conf["trunc"], conf["mech"]
    )
    ...
    for m in range(0, conf["neps"]):  # area
        Wprime = np.sqrt(area[m] / conf["AR"])
        for k in range(0, conf["ndip"]):  # dip
            if np.allclose(dip[k], 0) is False:
                ZW = Wprime * np.sin(dip[k])  # vertical projection of W
                # Overwrite W if it extends too far and recompute SW, x, dx
                Ztor = np.max(np.array([conf["zhyp"] - ZW, conf["min_seis_depth"]]))
                Zbor = conf["zhyp"] + ZW
            ...
            L = area[m] / W
            SW = W * np.cos(dip[k])
            if np.allclose(dip[k], np.pi / 2.0) or conf["zhyp"] == 0:
                nx = 1
                SW = 0
                w_range = 1  # This will never matter because it doesn't get used.
                x = np.linspace(0, SW, nx)
                dx = 0
            else:
                nx = conf["nxny"]
                w_range = min(SW, conf["zhyp"] / np.tan(dip[k]))
                x = np.linspace(0, w_range, nx)
                dx = x[1] - x[0]
            one_over_L = 1.0 / L
            one_over_sw = 1.0 / w_range
            ...
            if nx == 1:
                RrupIntegrand_d[k] = RrupIntegrand_x[0]
                RrupIntegrand_d2[k] = RrupIntegrand_x2[0]
                RjbIntegrand_d[k] = RjbIntegrand_x[0]
                RjbIntegrand_d2[k] = RjbIntegrand_x2[0]
            else:
