...
upon:
   ...
   - Thomposn, E. M., C. B. Worden (2017). Estimating rupture distances
     without a rupture, Bull. Seism. Soc. Am. 
...
class PS2FF(object):
    ...
    def fromParams(
        ...
        """  # noqa
        filebase = "%s_%s_mech%s_ar%.1f_seis%d_%d" % (
            dist_type.value,
            mag_scaling.value,
            mechanism.value,
            AR,
    ...
    def fromFile(cls, rfile):
        ...
        self.r2r_ratios_obj = spint.RectBivariateSpline(
            dist_list, mag_list, r2r_ratios_grid, kx=1, ky=1
        )

        varfile = Path(str(filepath).replace("_Ratios", "_Var"))
        r2r_var_tbl = pd.read_csv(varfile, comment="#")

        r2r_var_grid = r2r_var_tbl.values[:, 1:]
        self.r2r_var_obj = spint.RectBivariateSpline(
            dist_list, mag_list, r2r_var_grid, kx=1, ky=1
        )
        self.rfile = rfile
        self.vfile = varfile.name
        return self
    ...
    def getValidFiles(cls):
        ...
        """
        datadir = importlib.resources.files("ps2ff") / "tables"
        validfiles = datadir.glob("*_Ratios.csv")
        return [x.name for x in validfiles]
