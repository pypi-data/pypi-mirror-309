...
def get_configspec():
    ...
    """
    return importlib.resources.files("ps2ff") / "data" / "configspec.conf"
...
def magScalingType(value):
    ...
    print(
        'Config error: "rup_dim_model" must be in %s'
...
def mechType(value):
    ...
    print('Config error: "mech" must be in %s' % ([x.value for x in list(Mechanism)]))
    raise ValidateError()
