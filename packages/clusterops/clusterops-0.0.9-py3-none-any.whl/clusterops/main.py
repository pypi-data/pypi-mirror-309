...
def execute_with_cpu_cores(
    ...
    """
    Executes a callable using an optimal number of CPU cores, handling various edge cases and providing verbose logging.
    """
    try:
        # Retrieve CPU information to determine the optimal core count
        physical, logical, available = get_cpu_info()
        optimal_cores = get_optimal_core_count(core_count)
        
        # Log the initial core count and the adjusted optimal core count if different
        if optimal_cores != core_count:
            logger.info(f"Adjusting core count from {core_count} to {optimal_cores} based on system capabilities")
        else:
            logger.info(f"Using requested core count: {core_count}")
            
        # Check if the system supports CPU affinity and set it accordingly
        if platform.system() == "Darwin":
            logger.warning("CPU affinity not supported on macOS. Skipping affinity setting.")
        else:
            # Ensure the optimal core count does not exceed the available cores
            if optimal_cores > available:
                logger.warning(f"Optimal core count {optimal_cores} exceeds available cores {available}. Using available cores.")
                optimal_cores = available
            
            # Set CPU affinity to the selected cores
            selected_cores = list(range(optimal_cores))
            logger.info(f"Setting CPU affinity to cores {selected_cores}")
            psutil.Process().cpu_affinity(selected_cores)

        # Execute the function with the optimal core count
        result = func(*args, **kwargs)
        logger.info(f"Execution using {optimal_cores} CPU cores completed")
        return result
        
    except Exception as e:
        # Log the error with more details for better debugging
        logger.error(f"Error executing with CPU cores: {e}", exc_info=True)
        raise
