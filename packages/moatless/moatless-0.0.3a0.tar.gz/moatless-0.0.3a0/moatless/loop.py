...
class AgenticLoop(BaseModel):
    ...
    def run(self):
        ...
        while not self.is_finished():
            ...
            self.log(
                logger.info,
                f"Run iteration {len(self.root.get_all_nodes())} (total cost: ${total_cost})",
            )

            if self.max_cost and total_cost and total_cost >= self.max_cost:
                self.log(
                    logger.warning,
                    f"Cost ${total_cost} exceeded max cost of ${self.max_cost}. Finishing loop.",
                )
                break
            ...
            try:
                ...
                self.maybe_persist()
                self.log(logger.info, generate_ascii_tree(self.root, current_node, f"Executed Node{current_node.node_id}"))
    ...
    def log(self, logger_fn: Callable, message: str, **kwargs):
        ...
        metadata_str = " ".join(f"{k}: {str(v)[:30]}" for k, v in metadata.items())
