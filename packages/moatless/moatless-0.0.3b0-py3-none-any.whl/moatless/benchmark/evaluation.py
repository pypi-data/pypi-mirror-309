...
class Evaluation:
                ...
                if not patch:
                    ...
                    if self.use_testbed and patch:
                        ...
                        if not result:
                            ...
                            logger.info(f"Evaluated patch for {instance_id} in {time.time() - start_time:.2f} seconds. Status: {eval_result['status']}")
            ...
            for future in pbar:
                try:
                    ...
                    if eval_result:
                        ...
                        logger.warning("No eval result found")

                    stats = self._create_stats(eval_results)
    ...
    def _create_stats(self, eval_results: list[dict]):
        stats = {}
        if eval_results:
            resolved = sum(1 for r in eval_results if r["status"] == "resolved")
            if resolved > 0:
                stats["resolved"] = f"{(resolved / len(eval_results)) * 100:.2f}%"
            else:
                stats["resolved"] = "0%"

            stats["errors"] = sum(1 for r in eval_results if r["status"] == "error")
