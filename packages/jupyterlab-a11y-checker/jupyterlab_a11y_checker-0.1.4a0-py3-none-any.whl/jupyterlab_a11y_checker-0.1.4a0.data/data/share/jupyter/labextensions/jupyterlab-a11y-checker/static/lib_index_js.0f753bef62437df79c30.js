...
class AltCellList extends _lumino_widgets__WEBPACK_IMPORTED_MODULE_3__.Widget {
    ...
    addCell(cellId, buttonContent, buttonId) {
        ...
        button.addEventListener('click', () => {
            this.scrollToCell(cellId, buttonId);
        });
    ...
    async generateAltText(cell) {
        var _a;
        const imageUrl = (_a = cell.node.querySelector('img')) === null || _a === void 0 ? void 0 : _a.src;
        const altText = await sendImageToModel(imageUrl || " ");
        this.updateCell(cell.model.id, altText);
    }
    //scroll to cell once clicked
    scrollToCell(cellId, buttonId) {
        const notebookPanel = this._notebookTracker.currentWidget;
        const notebook = notebookPanel.content;
        for (let i = 0; i < notebook.widgets.length; i++) {
            const cell = notebook.widgets[i];
            if (cell.model.id === cellId) {
                ...
                if (buttonId == "alt") {
                    this.updateCell(cell.model.id, "Generating alt-text for this image...");
                    this.generateAltText(cell);
                }
            }
        }
    }
...
//# sourceMappingURL=lib_index_js.0f753bef62437df79c30.js.map