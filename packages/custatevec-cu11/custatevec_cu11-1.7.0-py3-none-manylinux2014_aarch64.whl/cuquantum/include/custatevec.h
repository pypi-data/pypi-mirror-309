...
/** @file custatevec.h
 *  @details custatevec API Version 1.7.0.22
 */
...
#define CUSTATEVEC_VER_MINOR 7 //!< cuStateVec minor version.
#define CUSTATEVEC_VER_PATCH 0 //!< cuStateVec patch version.
#define CUSTATEVEC_VERSION 1700 //!< cuStateVec Version.
 ...
 *  As the semaphore is used for synchornization between processes, the allocation should be shared among processes
 ...
 * transfer workspace can improve performance.
 *
 * SVSwapWorker can be used in a single process.  For this case, the \communicator argument
 * can be set to NULL. The internal CUDA calls are serialized on the stream given as the \p stream argument.
 * If NULL is passed to the \communicator argument, the \orgSemaphore argument should be set to NULL.
 *
 ...
 * or \p cuMemExportToShareableHandle() and \p cuMemImportFromSharableHandle() in CUDA Driver API.
 *
 * If \p the orgSemaphore argument is set to NULL when calling  \p custatevecSVSwapWorkerCreateWithSemaphore(),
