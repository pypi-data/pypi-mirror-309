...
class Client(SockRunable):
 ...
 connection         : show tunnel latest or active conn..
 latency            : show tunnel ping
 ...
 stop               : force tunnel to stop

\033[01;33mCustom firewall:\033[0m
 ban <range/ip>     : ban ip or cidr on tunnel
 unban <range/ip>   : unban ip or cidr on tunnel
 rule <a1> <a2>     : update rule in firewall
 list               : list all banned ips

\033[01;33mSystem firewall:\033[0m
 iunban <range/ip>  : unban ip or cidr from system firewall
 priory <range/ip>  : add or remove priory to ip address

\033[01;33mExamples:\033[0m
 ban 8.8.8.8/32     : specify ip or cidr to block
 unban 8.8.8.8/32   : specify ip or cidr to unblock
 iunban 8.8.8.8/32  : specify ip or cidr to unblock in system firewall
 priory 8.8.8.8/32  : adds ip or cidr to priority
 rule rate 1        : change rate in firewall only numbers 0,1,2,3
...
def main():
    ...
    if args.version:
        ...
        if console_check_connection(port) != False:
            ...
            while True:
                try:
                    ...
                    print("connection \033[01;31mclosed\033[0m by remote host")
                    break
        else:
        ...
        try: cfg["network"]["compression"]
            ...
            ccmp = True
        try: int(cfg["network"]["buffer_size"])
        except:
            print(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] wrong or not enough arguments «buffer_size» in «network»")
            sys.exit(7)
        try:
            pm = cfg["ping"]["method"]
