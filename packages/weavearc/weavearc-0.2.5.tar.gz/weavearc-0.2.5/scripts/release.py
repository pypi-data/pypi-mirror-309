...
def main() -> None:
    ...
    custom_theme = Theme(
        {"success": "bold green", "error": "bold red", "question": "blue"}
    )
    ...
    # Check for double confirmation on MAJOR or FEATURE releases
    if user_response in [ReleaseType.MAJOR, ReleaseType.FEATURE]:
        if not Confirm.ask(
            ...
            console.print("Release process terminated.", style="question")
            return
        if not Confirm.ask(
            "Please confirm again, this is a significant update."
        ):
            console.print("Release process terminated.", style="question")
            return
    ...
    if not project or "version" not in project:
        console.print(
            "The 'version' field is required in 'pyproject.toml'.", style="error"
        )
    ...
    if not gh_cli:
        console.print(
            "GitHub CLI ('gh') is required to run this script.", style="error"
        )
    ...
    if Confirm.ask("Add, commit and push everything not stashed before releasing?"):
        try:
            subprocess.run(["git", "add", "."], check=True)
            subprocess.run(["git", "commit", "-m", f"Release {new_version}"], check=True)
            subprocess.run(["git", "push"], check=True)
            console.print("Changes committed and pushed to the repository.", style="success")
        except subprocess.CalledProcessError as e:
            console.print(f"Failed to commit or push changes: {e}", style="error")
            return
    ...
    if not os.path.exists(changelog_path):
        console.print(
            f"{changelog_path} does not exist. Creating an empty file.",
            style="question",
        )
        ...
        console.print(
            f"{changelog_path} is empty. Would you like to add some notes?",
            style="question",
        )
        if Confirm.ask("Add notes to CHANGELOG.md?"):
    ...
    # Git commit and push
    try:
        subprocess.run(["git", "add", "pyproject.toml", changelog_path], check=True)
        subprocess.run(["git", "commit", "-m", f"Release {new_version}"], check=True)
        subprocess.run(["git", "push"], check=True)
        console.print(
            "Changes committed and pushed to the repository.", style="success"
        )
    except subprocess.CalledProcessError as e:
        console.print(f"Failed to commit or push changes: {e}", style="error")
        return
    ...
    release_title = Prompt.ask(
        "Enter release title (default: version tag):", default=new_version
    )
