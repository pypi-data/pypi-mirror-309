...
class JSONHTTPDialect(DefaultDialect):
    ...
    def create_connect_args(self, url):
        ...
        print(f"#########URL OPTS:::{([], opts)}##############################")
        #return ([], opts)
        host = opts.get('database')
        username = opts.get('username')
        password = opts.get('password')
        endpoint = f"https://{host}" if not host.startswith("http") else host
        t = (endpoint,), {
            "auth": (username, password),
            "duckdb_path": opts.get("duckdb_path", ":memory:"),
            "collection": opts.get("resources", "analytics"),
        ...
        return (endpoint,), {
            "auth": (username, password),
            "duckdb_path": opts.get("duckdb_path", ":memory:"),
            "resources": opts.get("resources", "analytics"),
            "token": opts.get("token"),
        }
