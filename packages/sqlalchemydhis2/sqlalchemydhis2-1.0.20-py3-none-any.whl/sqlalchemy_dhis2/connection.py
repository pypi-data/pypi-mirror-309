...
from .constants import _HEADER, _PAYLOAD
...
class Cursor:
    ...
    @staticmethod
    # pylint: disable=too-many-arguments
    def submit_query(query, host, port, proto, db, collection, session):
        print(f"######@QUERY:{query}")
        local_payload = _PAYLOAD.copy()
        local_payload["stmt"] = query
        url = f"{proto}{host}:{str(port)/{db}/{collection}}"
        return session.get(
            url,
...
def connect(
    ...
    **cparams,
    ...
    print(f"#####{host}#######{database}#########{auth}#####")
    print(f"args:::{cargs}:::::{cparams}")
    session = Session()
    ...
    if collection is not None:
        local_url = f"/{collection}"
        if database is not None:
            local_url = f"/{database}/{collection}"
