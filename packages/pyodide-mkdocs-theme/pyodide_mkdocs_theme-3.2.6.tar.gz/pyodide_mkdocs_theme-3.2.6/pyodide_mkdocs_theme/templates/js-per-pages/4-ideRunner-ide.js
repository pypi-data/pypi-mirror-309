...
class IdeStorageAndZipManager extends TerminalRunner {
  ...
  pyodideGetStorage(key){
    if(!this.storage.hasOwnProperty(key)) throw new PythonError(
        `Cannot read localStorage property "${ key }": no data registered for this property.`
    )
  ...
  pyodideSetStorage(key, value){
    if(FORBIDDEN_LOCAL_STORAGE_KEYS_WRITE.some(k=>k==key)){
        throw new PythonError(
            `Writing the "${ key }" property of the localStorage is forbidden.`
        )
    } else {
        try{
            ...
            throw new PythonError(
                `Cannot Write the "${ key }" property of the localStorage: the given value `
                +"cannot be transformed to string, using JSON.stringify(...)."
            )
        }
...
class _IdeEditorHandler extends IdeStorageAndZipManager {
  ...
  build(){
    ...
    window.addEventListener( 'resize', _.throttle(
      this.terminalAutoWidthOnResize.bind(this), 50, {leading:false, trailing:true}
    ))
  }
  ...
  makeUpYourGui(){
    if($(this.globalIdH).is(':hidden')) return false    // Nothing to do yet.
