...
class MaestroMacros(MaestroMacroManager):
    ...
    def on_page_context(self, _ctx, page:Page, *, config:MkDocsConfig, nav:Navigation):
        ...
        page_config = self.get_page_config_and_set_base_url(page)
        chunks      = []
    ...
    def get_page_config_and_set_base_url(self, page:Optional[Page]=None):
        """
        Configure self.base_url and self.page, based on the given page (or keep the current
        self.page if not given), then return the corresponding PageConfiguration object.
        """
        page     = page or self.page
        url      = page.url
        base_url = self.level_up_from_current_page(url)
        self.setup_ctx(base_url, page)

        page_config = self._pages_configs[url]
        return page_config


    def setup_ctx(self, base_url:Optional[str], page:Optional[Page]):
        """
        Update self._page (see MacrosPlugin base class) and self.base_url.
        """
        # Make sure the getter coming from the MacrosPlugin class will keep returning the
        # correct data, even while rendering templates:
        self._page    = page
