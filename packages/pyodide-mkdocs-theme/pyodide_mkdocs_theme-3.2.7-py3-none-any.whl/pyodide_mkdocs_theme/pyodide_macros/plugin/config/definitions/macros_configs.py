    ...
    ConfigOptionIdeLink(
        ...
        docs = f"""
            ...
            { OP } ul_li([
                ...
                "`#!py {IdeMode.delayed_reveal!r}` : pour des IDEs n'ayant pas de tests (ni `tests`
                ni `secrets`) mais dont on en veut pas que la solution s'affiche dès la première
                exécution (typiquement, des exercices turtle ou p5). Chaque validation consomme un
                essai et les solutions et remarques, si elles existent, sont révélées une fois
                tous les essais consommés (une erreur est levée si l'IDE a des sections `tests` ou
                `secrets`, ou a un nombre d'essais infini).",
