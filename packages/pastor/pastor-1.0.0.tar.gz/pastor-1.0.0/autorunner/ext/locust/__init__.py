...
def is_autorunner_testcase(item):
    """ check if a variable is a AutoRunner testcase class
    """
    ...
    return bool(
        ...
        and issubclass(item, AutoRunner)
        and item.__name__ != "AutoRunner"
    )
...
def prepare_locust_tests() -> List:
    ...
    for pytest_file in pytest_files:
        ...
        for name, item in vars(module).items():
            ...
            if not is_autorunner_testcase(item):
                continue
...
def main_locusts():
    ...
    from autorunner.make import main_make
