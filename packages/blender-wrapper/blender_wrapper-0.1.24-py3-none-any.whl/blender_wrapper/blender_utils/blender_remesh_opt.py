...
import bmesh
import math
from bmesh.types import BMFace, BMVert, BMesh, BMEdge
...
def flatten_bumpy_vertices(bm: BMesh, verts: List[BMVert], max_distance_threshold: int = 0.2, is_dissolve=False):
...
def separate_chaos_vertices(bm, is_selected_only: bool = False, max_linked_face_num: int = 6):
    ...
    merge_vertices(bm, verts=verts, min_distance_threshold=0.2)


# for i in range(3):
#     with mesh_edit(mesh_name='demo_01') as bm:
#         separate_chaos_vertices(bm, is_selected_only=False, max_linked_face_num=6)

#     # apply triangulate modifier on the mesh
#     modifier = bpy.context.object.modifiers.new(name="Triangulate", type='TRIANGULATE')
#     modifier.keep_custom_normals = True
#     modifier.ngon_method = 'BEAUTY'
#     modifier.quad_method = 'FIXED'
#     bpy.ops.object.modifier_apply(modifier="Triangulate")

# with mesh_edit(mesh_name=None, auto_edit_mode=False) as bm:

#     start_time = timer()

#     verts = [v for v in bm.verts if v.select == True and len(v.link_edges) == 6]
#     slot = flatten_bumpy_vertices(bm, verts=verts, max_distance_threshold=0.3)
