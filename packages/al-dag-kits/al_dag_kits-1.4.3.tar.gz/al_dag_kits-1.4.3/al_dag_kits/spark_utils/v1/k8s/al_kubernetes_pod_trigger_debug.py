...
class ALKubernetesPodTriggerDebug(KubernetesPodTrigger):
    def __init__(
        self,
        pod_name: str,
        pod_namespace: str,
    ...
    def serialize(self) -> tuple[str, dict[str, Any]]:
        return (
            "al_dag_kits.spark_utils.v1.k8s.al_kubernetes_pod_trigger.ALKubernetesPodTriggerDebug",
            {
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        ...
        try:
            ...
            self.log.info(f"ALKubernetesPodTrigger completed {event=}")
            yield event
            return

        except PodLaunchTimeoutException as e:
            message = self._format_exception_description(e)
            self.log.warning("Pod launch timeout.")
            yield TriggerEvent(
                {
                    "name": self.pod_name,
                    "namespace": self.pod_namespace,
            ...
            self.log.warning("Exception occurred while monitoring Kubernetes pod")
            yield TriggerEvent(
                {
                    "name": self.pod_name,
                    "namespace": self.pod_namespace,
