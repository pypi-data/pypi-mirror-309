...
from .. import io_util
from . import coordinate_util, galaxy_sampling_util
from .galaxy_sampling_util import sample_position_uniform
...
def load_besancon_map_pixels(ctx, list_ipix):
    ...
    with h5py.File(filepath_map, "r") as ff:
        for ipix in list_ipix:
            if len(ff["healpix_list/%04d" % ipix]) == 0:
                besancon_pix_cat = None
            else:
                besancon_pix_cat = np.array(ff["healpix_list/%04d" % ipix])

            dist_besancon_pix_cat["%04d" % ipix] = besancon_pix_cat
...
def get_star_cat_besancon_gaia_splice(ctx):
    ...
    for pixel in pixels:
        ...
        theta, phi = hp.pix2ang(par.nside_sampling, pixel)
        ipix_besancon = hp.ang2pix(dict_besancon_info["nside"], theta, phi)
        cat_besancon = dict_cat_besancon["%04d" % ipix_besancon]
...
class Plugin(BasePlugin):
    ...
    def __call__(self):
        ...
        self.ctx.stars = galaxy_sampling_util.Catalog()
