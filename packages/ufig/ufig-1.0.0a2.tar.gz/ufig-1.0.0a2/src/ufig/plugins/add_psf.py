...
def get_moffat_coadd_psf_cnn(obj_name, ctx):
    ...
    if par.psf_type == "coadd_moffat_cnn_robust" or par.psf_type == "coadd_moffat_cnn":
        if par.psf_type == "coadd_moffat_cnn_robust":
        ...
        raise RuntimeError(
            f"Cannot use get_moffat_coadd_psf_cnn with PSF type {par.psf_type}"
        )
...
PSF_GENERATOR = {
    ...
    "coadd_moffat_cnn_robust": get_moffat_coadd_psf_cnn,
...
class Plugin(BasePlugin):
    ...
    def __call__(self):
        ...
        if psf_fwhm.size > 0:
            ...
            if not np.isfinite(self.ctx.average_seeing):
