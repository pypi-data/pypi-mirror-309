        ...
        with torch.no_grad():
            for images, targets in self.dataloader:  # Ensure you're using DataLoader here
                ...
                for pred, target in zip(preds, targets):
                    ...
                    # Extract bbox and labels from target annotations
                    target_boxes = []
                    target_labels = []

                    for ann in target:  # Each target is an annotation dict
                        target_boxes.append(ann["bbox"])  # Extracting bbox (this is a list)
                        target_labels.append(ann["category_id"])  # Extracting category_id as label

                    target_boxes = torch.tensor(target_boxes).to(device)  # Convert to tensor
                    target_labels = torch.tensor(target_labels).to(device)  # Convert to tensor
