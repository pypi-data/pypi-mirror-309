...
def init_population(population_size, chromosome_len, gene_pool):
    population = []
...
def evaluate_fitness(chromosome, configs):
    counters = {
        'v_load': 0,
        'v_store': 0,
        'v_arith': 0
...
def crossover_1_point(parent_1, parent_2):
...
def let_it_be_mutated(MUTATE_RATE):
    r = random()
...
def run_evolution(configs):
    ...
    while (True):
        population.sort(key=lambda chromosome: evaluate_fitness(chromosome, configs))
        
        if evaluate_fitness(population[0], configs) == 0:
            break
        ...
        for _ in range(rest): 
            ...
            offspring_1, offspring_2 = crossover_1_point(parent_1, parent_2)

            if let_it_be_mutated(configs['rtg']['mutate_rate']):
                offspring_1 = mutate(offspring_1, V_ALL)
            if let_it_be_mutated(configs['rtg']['mutate_rate']):
                offspring_2 = mutate(offspring_2, V_ALL)
