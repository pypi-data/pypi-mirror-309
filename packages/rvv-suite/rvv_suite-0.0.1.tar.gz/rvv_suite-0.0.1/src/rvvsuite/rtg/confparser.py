...
def validate_configs(configs, schemas):
    output_configs = {}

    for key, schema in schemas.items():
        if key not in configs:
            if key =='rtg': # sub-schemas
                output_configs['rtg'] = validate_configs({}, schema)
                continue
        ...
        if key == 'rtg':
            ...
            output_configs['rtg'] = validate_configs(config, schema)
            configs.pop(key)
            continue
...
def parse_config_file(config_file):
    ...
    good_configs = validate_configs(configs, CONFIG_SCHEMAS)
