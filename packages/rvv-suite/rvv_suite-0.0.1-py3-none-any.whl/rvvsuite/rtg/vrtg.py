...
from ..common.supported_features import V_LOAD, V_STORE, VECTOR_INSTS, SCALAR_INSTS
from .ga import run_evolution

__all__ = [
    'generate_operator_sequence',
    'add_operands',
    'insert_vm_change_placeholders',
    'replace_vm_change_placeholders',
    'display',
]
...
def replace_vm_change_placeholders(inst_sequence: list, data, use_addr=False):
    updated_sequence = []
...
def display_data_def(data_def):
    return f"{data_def['label']}: {data_def['type']} {', '.join([hex(item) for item in data_def['items']])}"
...
def display(
    ...
    asm_data_section = ''
    for addr, data_def in data_section.items():
        asm_data_section += display_data_def(data_def) + '\n'
