...
class PromptManager(Base):
    ...
    async def summarymemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(f"interaction_request: {interaction_request}")
            ...
            logger.info(f"interaction_history: {interaction_history}")
            ...
            # )
            logger.info(
                f"Getting summarized content for {user_prompt_id} and {session_id}"
            )
            ...
            if result["status"] == "success":
                ...
                logger.info(f"Found existing summarized content: {summarized_content}")
            else:
                logger.info("No existing summarized content found")

            if session_type == "new_session":
                logger.info("Adding user message to prompt collection for new session")
                prompt_collection_msg.extend(interaction_request["user_message"])

            elif session_type == "existing_session":
                logger.info("Adding content for existing session with summary")
                content = [
                    {
                        "role": "assistant",
                        "content": [
                ...
                logger.info(f"Adding content to prompt collection: {content}")
                prompt_collection_msg.extend(content)
            ...
            logger.info(f"Response1111111111111111 summary: {response}")
            ...
            logger.info(f"Saved new summarized content: {new_summarized_content}")
    ...
    async def get_summarized_content(
        ...
        try:
            logger.info(
                f"Fetching summarized content for prompt_id: {prompt_id}, session_id: {session_id}"
            )
            ...
            if cached_data and "summarized_content" in cached_data:
                logger.info("Found summarized content in cache")
                return {
                    "status": "success",
            ...
            logger.info("No summarized content found in cache")
            return {
                "status": "not_found",
...
def gemini_interaction_chat_with_prompt(
    ...
    if response_format["type"] == "text":
        ...
        try:
            logger.info("Creating response schema")
    ...
    try:
        ...
        logger.info(f"Response gemini_interaction_chat_with_prompt: {response}")
        ...
        if response_mime_type == "text/plain":
            logger.info(f"Response mime type is text/plain: {response.text}")
            assistant_reply = response.text

        else:
            logger.info(
                f"Response mime type is not text/plain: {response.candidates[0].content.parts[0].text}"
            )
...
def create_gemini_schema(response_dict: Dict[str, Any]) -> genai.protos.Schema:
    ...
    def _process_value(value: Any) -> genai.protos.Schema:
        if isinstance(value, dict):
            logger.info(f"Value is a dictionary: {value}")
            if "type" in value:
                ...
                logger.info("Value is not a dictionary")
