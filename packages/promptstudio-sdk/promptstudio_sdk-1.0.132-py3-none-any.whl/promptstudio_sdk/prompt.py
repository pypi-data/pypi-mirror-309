...
class PromptManager(Base):
    ...
    async def summarymemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(
                f"interaction_request['user_message'] content: {interaction_request['user_message']}"
            )

            if session_type == "new_session":
                logger.info("Adding 1111 user message to prompt collection")
                prompt_collection_msg.extend(interaction_request["user_message"])

            elif session_type == "existing_session":
                logger.info("Adding 2222 user message to prompt collection")
                content = [
                    {
                        "role": "assistant",
                        "content": [
                ...
                logger.info(f"Adding 3333 content to prompt collection: {content}")
