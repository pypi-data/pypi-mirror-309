...
class PromptManager(Base):
    ...
    async def chat_with_prompt(
        ...
        if memory_type == "summaryMemory":
            raise ValueError("summaryMemory is not supported")
        ...
        try:
            if self.bypass:
                ...
                logger.info(f"Bypass is false")
                # Make an asynchronous HTTP request to the PromptStudio API using the _request method
                # inherited from the Base class. This method handles authentication and error handling.
                response = await self._request( # This API call is appropriate in the SDK since:
                                             # 1. It's making requests to PromptStudio's API, not directly to AI platforms
                                             # 2. It handles authentication and error handling consistently
                                             # 3. It's part of the core SDK functionality to interact with PromptStudio services
...
def gemini_interaction_chat_with_prompt(
    ...
    last_message_parts = generated_history[-1]
