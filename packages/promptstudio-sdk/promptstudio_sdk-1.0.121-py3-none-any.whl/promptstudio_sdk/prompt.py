...
class PromptManager(Base):
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(f"User prompt interaction_request: {interaction_request}")
            ...
            logger.info(
                f"User prompt prompt_details['variables']: {prompt_details['variables']}"
            )
            ...
            if (
                ...
                def_variables = def_variables
            else:
                def_variables = []
            default_variables = self.convert_data(def_variables)

            logger.info(f"User prompt default_variables: {default_variables}")
            # platform_key = user_prompt["platformConfig"]["platform_key"]

            logger.info(
                f"User prompt interaction_request['variables']: {interaction_request['variables']}"
            )

            logger.info(
                f"User prompt interaction_request['variables']: {interaction_request.get("variables")}"
            )
