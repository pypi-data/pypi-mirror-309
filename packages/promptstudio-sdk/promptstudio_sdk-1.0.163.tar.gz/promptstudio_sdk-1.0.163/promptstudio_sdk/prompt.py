...
class PromptManager(Base):
    ...
    async def _direct_ai_request(
        ...
        if not prompt_details:
            prompt_details = await self._fetch_and_cache_prompt_details(prompt_id, version)
...
async def get_folder_prompts(self, folder_id: str) -> Dict:
    """Get prompts in a folder"""
    try:
        response = await self._request(f"/{folder_id}")
        return response
    except Exception as e:
        logger.error(f"Error in get_folder_prompts: {str(e)}")
        raise
