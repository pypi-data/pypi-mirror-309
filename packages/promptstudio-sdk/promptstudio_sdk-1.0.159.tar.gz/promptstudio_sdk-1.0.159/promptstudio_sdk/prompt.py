...
class PromptManager(Base):
    ...
    def get_all_prompts(self, folder_id: str) -> Dict:
        """
        ...
        """
        response = self._request(f"/{folder_id}")
        return response

    def windowmemory_save_log_ai_interaction_prompt(
        self,
        user_prompt_id: str,
        ...
        try:
            ...
            if not prompt_details:
                ...
                prompt_details = self._fetch_and_cache_prompt_details(
            ...
            platform_key = prompt_details["platform_config"]["platformKey"]
            response = self._make_ai_platform_request(
    ...
    def fullmemory_save_log_ai_interaction_prompt(
        self,
        user_prompt_id: str,
        interaction_request: Dict,
    ) -> Dict:
    ...
    def summarymemory_save_log_ai_interaction_prompt(
        self,
        user_id: str,
        user_prompt_id: str,
        user_prompt: dict,
        ...
        try:
            ...
            result = self.get_summarized_content(
                prompt_id=user_prompt_id,
                session_id=session_id,
            )
            ...
            logger.info(f"new_user_message   ssssssss: {new_user_message}")
            messages_collection = self.update_messages_collection(
                prompt_details["ai_platform"], system_message, old_messages
            )
            logger.info(f"messages_collection   ssssssss: {messages_collection}")

            if session_type == "new_session":
                logger.info(
                    f"new session messages_collection   ssssssss: {messages_collection}"
                )
            ...
            create_summary = self._make_ai_platform_request(
                platform_name=prompt_details["ai_platform"],
                prompt_details=prompt_details,
    ...
    def _fetch_and_cache_prompt_details(
    ...
    def claude_interaction_chat_with_prompt(
    ...
    def _direct_ai_request(
        ...
        if not prompt_details:
            prompt_details = self._fetch_and_cache_prompt_details(prompt_id, version)
        ...
        try:
            if prompt_details["ai_platform"] == "openai":
                return self._make_openai_request(prompt_details, payload, platform_key)
    ...
    def chat_with_prompt(
        ...
        try:
            if self.bypass:
                ...
                #     return  self.summarizedmemory_save_log_ai_interaction_prompt(
                #         prompt_id, session_id, payload
                #     )
                if memory_type == "windowMemory":
                    return self.windowmemory_save_log_ai_interaction_prompt(
                        prompt_id, payload
                    )
                elif memory_type == "fullMemory":
                    return self.fullmemory_save_log_ai_interaction_prompt(
                        prompt_id, payload
                    )
                else:
                    return self._direct_ai_request(prompt_id, session_id, payload)
    ...
    def _make_ai_platform_request(
        ...
        try:
            ...
            if platform_name.lower() == "openai":
                ...
                return self.claude_interaction_chat_with_prompt(
                    claude_api_key,
    ...
    def get_summarized_content(self, prompt_id: str, session_id: str) -> Dict[str, Any]:
        """
