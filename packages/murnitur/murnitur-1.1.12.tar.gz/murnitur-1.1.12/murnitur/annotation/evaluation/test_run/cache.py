...
from murnitur.annotation.evaluation.test_case import LLMTestCaseParams, LLMTestCase
from murnitur.annotation.evaluation.types import Languages
from murnitur.annotation.evaluation.test_run.api import MetricMetadata
...
CACHE_FILE_NAME = ".murnitur.annotation.evaluation-cache.json"
TEMP_CACHE_FILE_NAME = ".temp-murnitur.annotation.evaluation-cache.json"
...
class CachedTestCase(BaseModel):
    cached_metrics_data: List[CachedMetricData] = Field(default_factory=lambda: [])
...
class TestRunCacheManager:
    ...
    def get_cached_test_case(
        ...
        cached_test_case = cached_test_run.get_cached_api_test_case(test_case_cache_key)
    ...
    def cache_test_case(
        ...
        cached_test_run.test_cases_lookup_map[test_case_cache_key] = new_cache_test_case
        self.save_cached_test_run(to_temp=to_temp)

    def set_cached_test_run(self, cached_test_run: CachedTestRun, temp: bool = False):
        if self.disable_write_cache:
            return
    ...
    def save_cached_test_run(self, to_temp: bool = False):
        ...
        if to_temp:
            try:
                with portalocker.Lock(self.temp_cache_file_name, mode="w") as file:
