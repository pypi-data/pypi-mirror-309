...
from murnitur.annotation.evaluation.metrics.contextual_precision.template import (
...
class ContextualPrecisionMetric(BaseMetric):
    ...
    async def _a_generate_verdicts(
        ...
        data = trimAndLoadJson(res, self)
        verdicts = [ContextualPrecisionVerdict(**item) for item in data["verdicts"]]
        return verdicts
    ...
    def _calculate_score(self):
        ...
        node_verdicts = [
            1 if v.verdict.strip().lower() == "yes" else 0 for v in self.verdicts
        ]
