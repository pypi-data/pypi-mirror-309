...
import numpy as np
from murnitur.annotation.evaluation.utils import normalize_text
...
class Scorer:
    ...
    def faithfulness_score(
        ...
        """
        try:
            from murnitur.annotation.evaluation.models import SummaCModels
        except Exception as e:
            print(f"SummaCZS model can not be loaded.\n{e}")

        scorer = SummaCModels(model_name=model, granularity=granularity, device=device)
    ...
    def hallucination_score(
        ...
        """
        try:
            from murnitur.annotation.evaluation.models.hallucination_model import (
                HallucinationModel,
            )
        except ImportError as e:
            print(f"Vectera Hallucination detection model can not be loaded.\n{e}")
    ...
    def neural_toxic_score(
        ...
        """
        try:
            from murnitur.annotation.evaluation.models.detoxify_model import (
                DetoxifyModel,
            )
        except ImportError as e:
    ...
    def answer_relevancy_score(
        ...
        try:
            from murnitur.annotation.evaluation.models import (
        ...
        if model_type == "cross_encoder":
            ...
            docs = [predictions] if isinstance(predictions, str) else predictions
            query_embedding = answer_relevancy_model(target)
            document_embedding = answer_relevancy_model(docs)
            scores = (
                util.dot_score(query_embedding, document_embedding)[0].cpu().tolist()
            )
            score = scores[0]
        return score
    ...
    def neural_bias_score(cls, text: str, model: Optional[str] = None) -> float:
        try:
            from murnitur.annotation.evaluation.models.unbias_model import UnBiasedModel
        except Exception as e:
