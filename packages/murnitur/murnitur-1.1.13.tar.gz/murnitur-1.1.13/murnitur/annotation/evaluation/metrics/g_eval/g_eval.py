...
from murnitur.annotation.evaluation.metrics.g_eval.template import GEvalTemplate
...
class GEval(BaseMetric):
    ...
    async def a_measure(
        ...
        with metric_progress_indicator(
            ...
            self.score = float(g_score) / 10
            self.score = (
                0 if self.strict_mode and self.score < self.threshold else self.score
            )
            self.success = self.score >= self.threshold
            if self.verbose_mode:
                print_intermediate_steps(
    ...
    async def _a_evaluate(
        ...
        try:
            ...
            # since generate raw response only exist for murnitur.annotation.evaluation's native model
            ...
            try:
                weighted_summed_score = self.generate_weighted_summed_score(score, res)
    ...
    def generate_weighted_summed_score(
        ...
        """
        try:
            generated_logprobs = raw_response.response_metadata["logprobs"]["content"]
