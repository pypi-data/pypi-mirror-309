...
def parse_hash(text):
  ...
  for ln in lines:
    ...
    filepath = re.sub(r"^\[\d+\]\s+[\"']", "", filepath)  # get rid of the leading [1]  " from R output
  ...
  for nb_cell in nb_cells:
    ...
    if tag_text in tags:
      ...
      for output_x in outputs:
        ...
        elif output_x.get("output_type") == "display_data":
          data = output_x.get("data", {})
          cell_outputs.append(data.get("text/plain", ""))
