...
class BackendHandler():
    ...
    def __init__(self,backend:Backend,linkMode:int,workdir:str,syncConfig:dict):
    ...
    def _RecursivelylistFilesInPath1(self,rpath:str,mtime:bool=True,hash=False,linkMode=2) -> list[ dict ]:
        ...
        segs = str(fdir).split("/")
        if len(segs) == 2:
            log(f" scaned {rpath.ljust(25, ' ')}: {len(res)} items")
        # log(f"scan files in: {rpath}")
        return res 
...
def copyFilesWithoutCheckFatherDir(filePaths:list,srcBackend:BackendHandler,dstBackend:BackendHandler,localWorkDir):
    ...
    sizeUsed = 0
    cachedPath = []
    for dirPath, files in directory_dict.items():
        # dirPath -> files 
