...
def excel_set_alignment(source_excel, sheet_index, ranges, horizontal="center", vertical="center", output_file=None):
    """
    设置指定区域的单元格对齐方式。
    :param source_excel: 原始 Excel 文件路径。
    ...
    output_file = output_file or source_excel
    workbook.save(output_file)

def excel_freeze_panes(source_excel, sheet_index, cell_reference, output_file=None):
    """
    冻结窗口。
    :param source_excel: 原始 Excel 文件路径。
    :param sheet_index: 工作表索引(从 0 开始)。
    ...
    output_file = output_file or source_excel
    workbook.save(output_file)
    print(f"Freeze panes set at {cell_reference} and saved to {output_file}.")


def excel_set_font_size(source_excel, sheet_index, ranges, font_size, output_file=None):
    """
    设置指定区域的字体字号。
    :param source_excel: 原始 Excel 文件路径。
    :param sheet_index: 工作表索引(从 0 开始)。
...
def excel_set_bold(source_excel, sheet_index, ranges, bold=True, output_file=None):
    """
    设置指定区域的字体加粗。
    :param source_excel: 原始 Excel 文件路径。
    :param sheet_index: 工作表索引(从 0 开始)。
...
def excel_set_font_color(source_excel, sheet_index, ranges, color, output_file=None):
    """
    设置指定区域的字体颜色。
    :param source_excel: 原始 Excel 文件路径。
    :param sheet_index: 工作表索引(从 0 开始)。
    :param ranges: 单个区域 (start_row, start_col, end_row, end_col) 或多个区域 [(..), (..)]。
    :param color: 字体颜色(十六进制字符串, 如 "FF0000" 表示红色)。
    :param output_file: 输出文件路径(如果为 None, 则覆盖原文件)。
    """
    workbook = openpyxl.load_workbook(source_excel)
    sheet = workbook.worksheets[sheet_index]
...
def excel_set_fill_color(source_excel, sheet_index, ranges, color, output_file=None):
    """
    设置指定区域的单元格填充颜色。
    :param source_excel: 原始 Excel 文件路径。
    :param sheet_index: 工作表索引(从 0 开始)。
    :param ranges: 单个区域 (start_row, start_col, end_row, end_col) 或多个区域 [(..), (..)]。
    :param color: 填充颜色(十六进制字符串, 如 "FFFF00" 表示黄色)。
    :param output_file: 输出文件路径(如果为 None,则覆盖原文件)。
    """
    workbook = openpyxl.load_workbook(source_excel)
    sheet = workbook.worksheets[sheet_index]
...
def excel_merge_cells(source_excel, sheet_index, ranges_to_merge, output_file=None):
    """
    合并 Excel 中指定的单元格区域, 并设置内容居中对齐(上下、左右)。
    默认使用合并区域左上角单元格的值作为合并后单元格的值。

    :param source_excel: Excel 文件路径。
    :param sheet_index: 工作表索引, 从 0 开始。
    :param ranges_to_merge: 要合并的单个区域或区域列表, 行、列值从1开始。
                           单个区域格式为 (start_row, start_col, end_row, end_col):
                           多个区域格式为 [(start_row, start_col, end_row, end_col), ...]。
    :param output_file: 输出文件路径(默认为覆盖原文件)。
    """
    workbook = openpyxl.load_workbook(source_excel)
    sheet = workbook.worksheets[sheet_index]
    ...
    output_file = output_file or source_excel
    workbook.save(output_file)
    print(f"Specified ranges merged and saved to: {output_file}")


def excel_unmerge_cells(source_excel, sheet_index, ranges_to_unmerge, output_file=None):
    """
    取消 Excel 中指定的合并单元格, 并填充原合并单元格的内容到拆分后的所有单元格中。

    :param source_excel: Excel 文件路径。
    :param sheet_index: 工作表索引, 从 0 开始。
    :param ranges_to_unmerge: 要取消合并的单个区域或区域列表, 行、列值从1开始。
                              单个区域格式为 (start_row, start_col, end_row, end_col):
                              多个区域格式为 [(start_row, start_col, end_row, end_col), ...]。
    :param output_file: 输出文件路径(默认为覆盖原文件)。
    """
    ...
    for cell_range in ranges_to_unmerge:
        ...
        # 检查区域是否是合并单元格
        range_str = f"{openpyxl.utils.get_column_letter(start_col)}{start_row}:" \
    ...
    output_file = output_file or source_excel
    workbook.save(output_file)
    print(f"Specified ranges unmerged and saved to: {output_file}")


def excel_replace_content(source_excel, sheet_index, target_text, replacement_text, output_file=None, rows=None, cols=None):
    """
    ...
    :param output_file: 替换后的文件输出路径(默认为覆盖原文件)。
    ...
    if 0 <= sheet_index < len(workbook.worksheets):
        ...
        if modified:
            output_file = output_file or source_excel
            workbook.save(output_file)
            print(f"Content replaced and saved to: {output_file} ({sheet_names[sheet_index]})")
        else:
            print("No matching content found. No changes made.")
    else:
