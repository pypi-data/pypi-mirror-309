...
class ZIPFIT:
    ...
    def compress(self, data: str, compressLevel: int) -> int:
        ...
        if self.compression_algorithm == 'gzip':
            compressed_size = len(gzip.compress(data.encode('utf-8'), compresslevel=compressLevel))
        elif self.compression_algorithm == 'lz4':
            compressed_size = len(lz4.frame.compress(data.encode('utf-8'), compression_level=compressLevel))
        elif self.compression_algorithm == 'zstd':
            compressed_size = len(zstd.ZstdCompressor().compress(data.encode('utf-8'), level=compressionLevel))
    ...
    def similarity(self, args) -> float:
        ...
        """
        s1, s2, c1, c2 = args
        c12 = self.compress(s1 + s2, compressLevel)  # Use the cached compress function
