...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1296:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1294(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1296()))
    ...
    def _arrow1314(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1313(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1312(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1311(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1301(__unit: None=None) -> FSharpList[FsCell]:
                        ...
                        def _arrow1309(__unit: None=None) -> IEnumerable_1[FsCell]:
                            def _arrow1308(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1307(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    ...
                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1306))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1307))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1308))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1309))

                    return append(_arrow1301(), delay(_arrow1310))

                return to_list(delay(_arrow1311))

            return map_1(_arrow1312, dc)

        return append(singleton_1(headers), delay(_arrow1313))

    return transpose(to_list(delay(_arrow1314)))
