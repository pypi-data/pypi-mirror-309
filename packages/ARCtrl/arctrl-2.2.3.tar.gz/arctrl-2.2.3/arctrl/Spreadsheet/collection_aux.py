...
def Array_ofIndexedSeq(s: IEnumerable_1[tuple[int, str]]) -> Array[str]:
    def _arrow878(tuple: tuple[int, str], s: Any=s) -> int:
        return tuple[0]
    ...
    def _arrow881(i: int, s: Any=s) -> str:
        def _arrow880(arg: tuple[int, str]) -> bool:
            return i == arg[0]

        match_value: tuple[int, str] | None = try_find(_arrow880, s)
    ...
    return initialize(1 + max_by(_arrow878, s, ObjectExpr879())[0], _arrow881, None)
...
def Array_map4(f: Callable[[_A, _B, _C, _D], _T], aa: Array[Any], ba: Array[Any], ca: Array[Any], da: Array[Any]) -> Array[Any]:
    ...
    def _arrow882(i: int, f: Any=f, aa: Any=aa, ba: Any=ba, ca: Any=ca, da: Any=da) -> _T:
        return f(aa[i], ba[i], ca[i], da[i])

    return initialize(len(aa), _arrow882, None)
...
def Dictionary_tryGetValue(k: Any, dict_1: Any) -> Any | None:
    ...
    out_arg: _V = None
    def _arrow883(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> _V:
        return out_arg

    def _arrow884(v: _V | None=None, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, k, FSharpRef(_arrow883, _arrow884)), out_arg)
    if pattern_input[0]:
...
def Dictionary_tryGetString(k: Any, dict_1: Any) -> str | None:
    ...
    out_arg: str = None
    def _arrow885(__unit: None=None, k: Any=k, dict_1: Any=dict_1) -> str:
        return out_arg

    def _arrow886(v: str, k: Any=k, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, k, FSharpRef(_arrow885, _arrow886)), out_arg)
