...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2952(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2951(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2950(__unit: None=None) -> IEnumerable_1[str]:
                    ...
                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2949))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2950))

            return append(singleton("{"), delay(_arrow2951))

        return join("\n", to_list(delay(_arrow2952)))
...
ValidationPackage_reflection = _expr2960
