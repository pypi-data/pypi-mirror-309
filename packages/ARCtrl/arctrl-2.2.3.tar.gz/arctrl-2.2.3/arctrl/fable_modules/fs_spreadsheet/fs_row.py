    ...
    def create_at(index: int, cells: FsCellsCollection) -> FsRow:
        def get_index_by(f: Callable[[Callable[[FsCell], int], IEnumerable_1[FsCell]], FsCell]) -> int:
            if length(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)) == 0:
                ...
                def _arrow164(c: FsCell, f: Any=f) -> int:
                    return FsAddress__get_ColumnNumber(c.Address)

                return FsAddress__get_ColumnNumber(f(_arrow164)(FsCellsCollection__GetCellsInRow_Z524259A4(cells, index)).Address)


        def _arrow167(projection: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow166(source_1: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr165:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return min_by(projection, source_1, ObjectExpr165())

            return _arrow166

        min_col_index: int = get_index_by(_arrow167) or 0
        def _arrow170(projection_1: Callable[[FsCell], int]) -> Callable[[IEnumerable_1[FsCell]], FsCell]:
            def _arrow169(source_2: IEnumerable_1[FsCell]) -> FsCell:
                class ObjectExpr168:
                    @property
                    def Compare(self) -> Callable[[int, int], int]:
                        return compare_primitives

                return max_by(projection_1, source_2, ObjectExpr168())

            return _arrow169

        max_col_index: int = get_index_by(_arrow170) or 0
