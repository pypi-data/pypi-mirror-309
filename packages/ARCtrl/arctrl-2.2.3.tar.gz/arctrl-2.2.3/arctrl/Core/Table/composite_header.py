...
class IOType(Union):
    ...
    def as_input(self, __unit: None=None) -> str:
        ...
        def string_create(x: Any | None=None) -> str:
            def _arrow711(__unit: None=None, x: Any=x) -> str:
                copy_of_struct: __A = x
                return to_string(copy_of_struct)

            return ("Input [" + _arrow711()) + "]"
    ...
    def as_output(self, __unit: None=None) -> str:
        this: IOType = self
        def string_create(x: Any | None=None) -> str:
            def _arrow712(__unit: None=None, x: Any=x) -> str:
                copy_of_struct: __A = x
                return to_string(copy_of_struct)

            return ("Output [" + _arrow712()) + "]"
...
CompositeHeader_reflection = _expr717
