...
class OntologyAnnotation:
    ...
    def to_string_object(oa: OntologyAnnotation, as_ontobee_purl_url_if_short: bool | None=None) -> dict[str, Any]:
        ...
        def _arrow610(__unit: None=None) -> str:
            url: str = oa.TermAccessionAndOntobeeUrlIfShort
            return default_arg(oa.TermAccessionNumber, "") if (url == "") else url

        return {
            "TermAccessionNumber": _arrow610() if as_ontobee_purl_url_if_short_1 else default_arg(oa.TermAccessionNumber, ""),
            "TermName": TermName,
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def _arrow618(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow611(__unit: None=None) -> str:
                arg: str = value_4(this.Name)
                return to_text(printf("Name = %A"))(arg)

            def _arrow617(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow612(__unit: None=None) -> str:
                    arg_1: str = value_4(this.TermSourceREF)
                    return to_text(printf("TSR = %A"))(arg_1)

                def _arrow616(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow613(__unit: None=None) -> str:
                        arg_2: str = value_4(this.TermAccessionNumber)
                        return to_text(printf("TAN = %A"))(arg_2)

                    def _arrow615(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow614(__unit: None=None) -> str:
                            arg_3: Array[Comment] = this.Comments
                            return to_text(printf("Comments = %A"))(arg_3)

                        return singleton(_arrow614()) if (len(this.Comments) != 0) else empty()

                    return append(singleton(_arrow613()) if (this.TermAccessionNumber is not None) else empty(), delay(_arrow615))

                return append(singleton(_arrow612()) if (this.TermSourceREF is not None) else empty(), delay(_arrow616))

            return append(singleton(_arrow611()) if (this.Name is not None) else empty(), delay(_arrow617))

        ignore(StringBuilder__Append_Z721C83C5(sb, join("; ", to_list(delay(_arrow618)))))
        ignore(StringBuilder__Append_Z721C83C5(sb, "}"))
        return to_string(sb)
    ...
    def __hash__(self, __unit: None=None) -> Any:
        ...
        def _arrow622(__unit: None=None) -> IEnumerable_1[Any]:
            ...
            return append(singleton(box_hash_option(this.Name)), delay(_arrow621))

        return box_hash_array(to_array(delay(_arrow622)))
...
OntologyAnnotation_reflection = _expr623
