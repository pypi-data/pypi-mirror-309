...
def ValidationPackagesConfig_encoder(validationpackage: ValidationPackagesConfig) -> YAMLElement:
    ...
    def _arrow3063(value: str, validationpackage: Any=validationpackage) -> YAMLElement:
        return string(value)

    def _arrow3064(validationpackage_1: ValidationPackage, validationpackage: Any=validationpackage) -> YAMLElement:
        return ValidationPackage_encoder(validationpackage_1)

    obj_seq: FSharpList[tuple[str, YAMLElement]] = choose(chooser, of_array([try_include("arc_specification", _arrow3063, validationpackage.ARCSpecification), ("validation_packages", resizearray(_arrow3064, validationpackage.ValidationPackages))]))
    def _arrow3066(__unit: None=None, validationpackage: Any=validationpackage) -> IEnumerable_1[YAMLElement]:
        def _arrow3065(match_value: tuple[str, YAMLElement]) -> IEnumerable_1[YAMLElement]:
            return singleton(YAMLElement(0, YAMLContent_create_27AED5E3(match_value[0]), match_value[1]))

        return collect(_arrow3065, obj_seq)

    return YAMLElement(3, to_list(delay(_arrow3066)))
