...
class FileSystemTree(Union):
    ...
    def try_get_child_by_name(name: str) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow434(fst: FileSystemTree) -> FileSystemTree | None:
    ...
    def contains_child_with_name(name: str) -> Callable[[FileSystemTree], bool]:
        def _arrow437(fst: FileSystemTree) -> bool:
    ...
    def AddFile(self, path: str) -> FileSystemTree:
        ...
        def _arrow439(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow438(__unit: None=None) -> IEnumerable_1[str]:
                return existing_paths

            return append(singleton(path), delay(_arrow438))

        file_paths: Array[str] = to_array(delay(_arrow439))
        return FileSystemTree.from_file_paths(file_paths)

    @staticmethod
    def add_file(path: str) -> Callable[[FileSystemTree], FileSystemTree]:
        def _arrow440(tree: FileSystemTree) -> FileSystemTree:
    ...
    def from_file_paths(paths: Array[str]) -> FileSystemTree:
        def loop(paths_1: Array[Array[str]], parent: FileSystemTree) -> FileSystemTree:
            ...
            class ObjectExpr442:
                ...
                def Equals(self) -> Callable[[str, str], bool]:
                    def _arrow441(x_3: str, y_2: str) -> bool:
                        return x_3 == y_2
            ...
            folders: Array[FileSystemTree] = map(mapping_2, Array_groupBy(projection, filter(predicate_1, paths_1), ObjectExpr442()), None)
            if parent.tag == 1:
        ...
        class ObjectExpr447:
            ...
            def Equals(self) -> Callable[[Array[str], Array[str]], bool]:
                def _arrow446(x: Array[str], y: Array[str]) -> bool:
                    def _arrow445(x_1: str, y_1: str) -> bool:
                        return x_1 == y_1

                    return equals_with(_arrow445, x, y)
        ...
        return loop(Array_distinct(map(split, paths, None), ObjectExpr447()), FileSystemTree.create_folder(FileSystemTree.ROOT_NAME(), []))
    ...
    def to_file_paths(remove_root: bool | None=None) -> Callable[[FileSystemTree], Array[str]]:
        def _arrow448(root: FileSystemTree) -> Array[str]:
    ...
    def filter_folders(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow453(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def filter(predicate: Callable[[str], bool]) -> Callable[[FileSystemTree], FileSystemTree | None]:
        def _arrow456(tree: FileSystemTree) -> FileSystemTree | None:
    ...
    def Union(self, other_fst: FileSystemTree) -> FileSystemTree:
        this: FileSystemTree = self
        def _arrow459(__unit: None=None) -> Array[str]:
        ...
        class ObjectExpr461:
            @property
            def Equals(self) -> Callable[[str, str], bool]:
                def _arrow460(x: str, y: str) -> bool:
                    return x == y
        ...
        paths: Array[str] = Array_distinct(_arrow459(), ObjectExpr461())
