...
def encoder_compressed_column(column_index: int, row_count: int, cell_table: Any, table: ArcTable) -> IEncodable:
    if True if table.Headers[column_index].IsIOType else (row_count < 100):
        def _arrow2516(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2515(r: int) -> IEncodable:
                return encode_cell(cell_table, get_item_from_dict(table.Values, (column_index, r)))

            return map(_arrow2515, range_big_int(0, 1, row_count - 1))

        values: Array[IEncodable] = to_array(delay(_arrow2516))
        ...
        from_: int = 0
        def _arrow2534(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2525(i: int) -> IEnumerable_1[IEncodable]:
                next_1: CompositeCell = get_item_from_dict(table.Values, (column_index, i))
                def _arrow2523(__unit: None=None) -> IEncodable:
                    def _arrow2519(__unit: None=None) -> IEncodable:
                    ...
                    def _arrow2521(__unit: None=None) -> IEncodable:
                    ...
                    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2519()), ("t", _arrow2521()), ("v", encode_cell(cell_table, current))])
                ...
                def _arrow2524(__unit: None=None) -> IEnumerable_1[IEncodable]:
                    nonlocal current, from_
                    current = next_1
                    from_ = i or 0
                    return empty()

                return append(singleton(_arrow2523()), delay(_arrow2524)) if (not equals(next_1, current)) else empty()

            def _arrow2533(__unit: None=None) -> IEnumerable_1[IEncodable]:
                def _arrow2531(__unit: None=None) -> IEncodable:
                    def _arrow2527(__unit: None=None) -> IEncodable:
                    ...
                    values_2: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2527()), ("t", _arrow2529()), ("v", encode_cell(cell_table, current))])
                ...
                return singleton(_arrow2531())

            return append(collect(_arrow2525, range_big_int(1, 1, row_count - 1)), delay(_arrow2533))

        values_3: Array[IEncodable] = to_array(delay(_arrow2534))
    ...
    class ObjectExpr2546(Decoder_1[None]):
        def Decode(self, helper: IDecoderHelpers_1[Any], column: Any, cell_table: Any=cell_table, table: Any=table, column_index: Any=column_index) -> FSharpResult_2[None, tuple[str, ErrorReason_1[__A_]]]:
            ...
            if match_value.tag == 1:
                def _arrow2544(get: IGetters) -> None:
                    from_: int
                ...
                range_decoder: Decoder_1[None] = object(_arrow2544)
...
def arrayi(decoderi: Callable[[int], Decoder_1[_VALUE]]) -> Decoder_1[Array[Any]]:
    class ObjectExpr2554(Decoder_1[Array[_VALUE_]]):
                ...
                def folder(acc: FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]], value_1: __A_) -> FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]]:
                    ...
                    if acc.tag == 0:
                        ...
                        if match_value.tag == 0:
                            ...
                            def _arrow2550(__unit: None=None, acc: Any=acc, value_1: Any=value_1) -> tuple[str, ErrorReason_1[__A_]]:
                            ...
                            return FSharpResult_2(1, _arrow2550())
...
def encoder_compressed(string_table: Any, oa_table: Any, cell_table: Any, table: ArcTable) -> IEncodable:
    def _arrow2564(__unit: None=None, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow2563(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow2556(__unit: None=None) -> IEnumerable_1[IEncodable]:
                return map(encoder_1, table.Headers)

            def _arrow2562(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                if len(table.Values) != 0:
                    row_count: int = table.RowCount or 0
                    def _arrow2558(__unit: None=None) -> IEnumerable_1[IEncodable]:
                        def _arrow2557(c: int) -> IEncodable:
                            return encoder_compressed_column(c, row_count, cell_table, table)

                        return map(_arrow2557, range_big_int(0, 1, table.ColumnCount - 1))

                    columns: Array[IEncodable] = to_array(delay(_arrow2558))
                    ...
                    return singleton(("c", ObjectExpr2561()))

                else: 
                    return empty()


            return append(singleton(("h", list_1(to_list(delay(_arrow2556))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2562))

        return append(singleton(("n", encode_string(string_table, table.Name))), delay(_arrow2563))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2564))
...
def decoder_compressed(string_table: Array[str], oa_table: Array[OntologyAnnotation], cell_table: Array[CompositeCell]) -> Decoder_1[ArcTable]:
    def _arrow2582(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table) -> ArcTable:
        def _arrow2574(__unit: None=None) -> FSharpList[CompositeHeader] | None:
            arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
            object_arg: IOptionalGetter = get.Optional
            return object_arg.Field("h", arg_1)

        decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2574(), empty_1()))
        ...
        table: ArcTable = ArcTable.create(_arrow2578(), decoded_header, Dictionary([], ObjectExpr2579()))
        def _arrow2581(__unit: None=None) -> Array[None] | None:
            def _arrow2580(column_index: int) -> Decoder_1[None]:
                return decoder_compressed_column(cell_table, table, column_index)

            arg_5: Decoder_1[Array[None]] = arrayi(_arrow2580)
