...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3049(__unit: None=None) -> IEnumerable_1[str]:
                ...
                def _arrow3047(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3046(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3045(__unit: None=None) -> IEnumerable_1[str]:
                            ...
                            return append(singleton("]"), delay(_arrow3044))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3045))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3046))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3047))

            return append(singleton("{"), delay(_arrow3048))

        return join("\n", to_list(delay(_arrow3049)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3052(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3051(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3051, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3052))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3053
