...
class FsTable(FsRangeBase):
    ...
    def GetFields(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsTableField]:
        this: FsTable = self
        def _arrow182(i: int) -> FsTableField:
            return this.GetFieldAt(i, cells_collection)

        return initialize(FsRangeBase__ColumnCount(this), _arrow182)
    ...
    def GetColumns(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsColumn]:
        this: FsTable = self
        def _arrow184(__unit: None=None) -> IEnumerable_1[FsColumn]:
            def _arrow183(i: int) -> IEnumerable_1[FsColumn]:
            ...
            return collect(_arrow183, range_big_int(FsAddress__get_ColumnNumber(FsRangeAddress__get_FirstAddress(FsRangeBase__get_RangeAddress(this))), 1, FsAddress__get_ColumnNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(this)))))

        return delay(_arrow184)

    def GetRows(self, cells_collection: FsCellsCollection) -> IEnumerable_1[FsRow]:
        this: FsTable = self
        def _arrow186(__unit: None=None) -> IEnumerable_1[FsRow]:
            def _arrow185(i: int) -> IEnumerable_1[FsRow]:
            ...
            return collect(_arrow185, range_big_int(FsAddress__get_RowNumber(FsRangeAddress__get_FirstAddress(FsRangeBase__get_RangeAddress(this))), 1, FsAddress__get_RowNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(this)))))
    ...
    def Field(self, name: str, cells_collection: FsCellsCollection) -> FsTableField:
        ...
        if match_value is None:
            def _arrow188(__unit: None=None) -> int:
                ...
                return 0 if (length(s) == 0) else max(s, ObjectExpr187())

            def _arrow189(__unit: None=None) -> FsRangeAddress:
            ...
            new_field: FsTableField = FsTableField__ctor_726EFFB(name, _arrow188() + 1, FsRangeColumn__ctor_6A2513BC(_arrow189()), None, None)
