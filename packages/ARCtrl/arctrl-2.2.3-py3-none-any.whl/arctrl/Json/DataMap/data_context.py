...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2537(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2539(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2541(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2544(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2548(value_2: str, dc: Any=dc) -> IEncodable:
        class ObjectExpr2547(IEncodable):
            def Encode(self, helpers_1: IEncoderHelpers_1[Any]) -> Any:
                return helpers_1.encode_string(value_2)

        return ObjectExpr2547()

    def _arrow2554(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2537, DataContext__get_Explication(dc)), try_include("unit", _arrow2539, DataContext__get_Unit(dc)), try_include("objectType", _arrow2541, DataContext__get_ObjectType(dc)), try_include("description", _arrow2544, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2548, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2554, DataContext__get_Label(dc))]))
...
def _arrow2569(get: IGetters) -> DataContext:
    ...
    def _arrow2561(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2563(__unit: None=None) -> OntologyAnnotation | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2561(), _arrow2563(), _arrow2564(), _arrow2565(), _arrow2567(), _arrow2568(), data.Comments)
