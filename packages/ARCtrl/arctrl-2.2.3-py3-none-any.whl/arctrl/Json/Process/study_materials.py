...
def encoder(id_map: Any | None, ps: FSharpList[Process]) -> IEncodable:
    ...
    def _arrow2400(oa: Source, id_map: Any=id_map, ps: Any=ps) -> IEncodable:
        return ISAJson_encoder(id_map, oa)

    def _arrow2401(oa_1: Sample, id_map: Any=id_map, ps: Any=ps) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include_list("sources", _arrow2400, source), try_include_list("samples", _arrow2401, samples), try_include_list("otherMaterials", _arrow2402, materials)]))
