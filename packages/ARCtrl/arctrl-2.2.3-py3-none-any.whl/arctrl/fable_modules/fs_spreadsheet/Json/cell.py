...
def encode_rows(cell: FsCell) -> IEncodable:
    def _arrow274(__unit: None=None, cell: Any=cell) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("column", _arrow274()), ("value", encode(cell.Value))])
...
def decode_rows(row_number: int | None=None) -> Decoder_1[FsCell]:
    def _arrow281(builder: IGetters, row_number: Any=row_number) -> FsCell:
        def _arrow277(__unit: None=None) -> tuple[Any, DataType] | None:
            object_arg: IOptionalGetter = builder.Optional
            return object_arg.Field("value", decode)

        pattern_input: tuple[Any, DataType] = default_arg(_arrow277(), ("", DataType(4)))
        def _arrow278(__unit: None=None) -> int | None:
            object_arg_1: IOptionalGetter = builder.Optional
            return object_arg_1.Field("column", int_1)

        c: int = default_arg(_arrow278(), 0) or 0
...
def encode_cols(cell: FsCell) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("row", _arrow283()), ("value", encode(cell.Value))])
