...
def Config__get_WhitespaceString(this: Config) -> str:
    def _arrow333(_arg: int, this: Any=this) -> str:
        return " "

    return initialize(this.Level * this.Whitespace, _arrow333)
...
Preprocessor_reflection = _expr334
...
class PreprocessorElement(Union):
    ...
    def __str__(self, __unit: None=None) -> str:
        ...
        def innerprint(next_1: PreprocessorElement, level: int) -> None:
            def _arrow335(_arg: int, next_1: Any=next_1, level: Any=level) -> str:
                return " "

            indent: str = initialize(level * 2, _arrow335)
            if next_1.tag == 1:
...
PreprocessorElement_reflection = _expr336
...
YAMLContent_reflection = _expr337
...
YAMLElement_reflection = _expr338
