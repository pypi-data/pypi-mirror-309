...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow715(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow714(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow714, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow715))
...
CompositeColumn_reflection = _expr716
