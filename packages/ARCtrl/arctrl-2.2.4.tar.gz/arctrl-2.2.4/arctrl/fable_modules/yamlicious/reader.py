...
def collect_sequence_elements(eles: FSharpList[PreprocessorElement]) -> FSharpList[FSharpList[PreprocessorElement]]:
    ...
    if pattern_matching_result == 0:
        def _arrow353(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
            def _arrow352(__unit: None=None) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                return collect_sequence_elements(rest)

            return append(singleton(cons(PreprocessorElement(2, value_2(v["Value"])), yaml_ast_list)) if (v["Value"] is not None) else singleton(yaml_ast_list), delay(_arrow352))

        return to_list(delay(_arrow353))
        ...
        if is_empty(eles):
            ...
            if active_pattern_result_1 is not None:
                ...
                def _arrow355(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                    def _arrow354(__unit: None=None) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                        return collect_sequence_elements(tail(eles))

                    return append(singleton(singleton_1(PreprocessorElement(2, value_2(v_1["Value"])))), delay(_arrow354))

                return to_list(delay(_arrow355))
                ...
                def _arrow357(__unit: None=None, eles: Any=eles) -> IEnumerable_1[FSharpList[PreprocessorElement]]:
                    ...
                    return append(singleton(singleton_1(head(eles))), delay(_arrow356))
...
def tokenize(yaml_list: FSharpList[PreprocessorElement], string_dict: Any, comment_dict: Any) -> YAMLElement:
    def loop_read(restlist_mut: FSharpList[PreprocessorElement], acc_mut: FSharpList[YAMLElement], yaml_list: Any=yaml_list, string_dict: Any=string_dict, comment_dict: Any=comment_dict) -> YAMLElement:
        while True:
            ...
            if pattern_matching_result == 0:
                def _arrow358(e: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                    return is_sequence_element(e)

                sequence_elements: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow358, rest0)))
                def _arrow359(e_1: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                    return is_sequence_element(e_1)

                restlist_mut = to_list(skip_while(_arrow359, rest0))
                def _arrow365(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                    def _arrow364(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                        def _arrow363(i: FSharpList[PreprocessorElement]) -> YAMLElement:
                            return loop_read(i, empty())

                        return map_1(_arrow363, sequence_elements)

                    return append(singleton(loop_read(cons(PreprocessorElement(2, v["Key"]), yaml_ast_list), empty())), delay(_arrow364))

                acc_mut = cons(YAMLElement(2, to_list(delay(_arrow365))), acc)
                continue
                ...
                if pattern_matching_result_1 == 0:
                    def _arrow366(e_2: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_2)

                    sequence_elements_1: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow366, rest0_1)))
                    def _arrow367(e_3: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_3)

                    restlist_mut = to_list(skip_while(_arrow367, rest0_1))
                    def _arrow373(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                        def _arrow372(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                            def _arrow371(i_1: FSharpList[PreprocessorElement]) -> YAMLElement:
                                return loop_read(i_1, empty())

                            return map_1(_arrow371, sequence_elements_1)

                        return append(singleton(loop_read(singleton_1(PreprocessorElement(2, v_1["Key"])), empty())), delay(_arrow372))

                    acc_mut = cons(YAMLElement(2, to_list(delay(_arrow373))), acc)
                    continue
                    ...
                    def _arrow374(e_4: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_4)

                    sequence_elements_2: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow374, rest0_2)))
                    def _arrow375(e_5: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                        return is_sequence_element(e_5)

                    restlist_mut = to_list(skip_while(_arrow375, rest0_2))
                    def _arrow381(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                        def _arrow380(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                            def _arrow379(i_2: FSharpList[PreprocessorElement]) -> YAMLElement:
                                return loop_read(i_2, empty())

                            return map_1(_arrow379, sequence_elements_2)

                        return append(singleton(loop_read(object_list_1, empty())), delay(_arrow380))

                    acc_mut = cons(YAMLElement(2, to_list(delay(_arrow381))), acc)
                    continue
                    ...
                    if pattern_matching_result_2 == 0:
                        def _arrow382(e_6: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                            return is_sequence_element(e_6)

                        sequence_elements_3: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow382, rest0_3)))
                        def _arrow383(e_7: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                            return is_sequence_element(e_7)

                        restlist_mut = to_list(skip_while(_arrow383, rest0_3))
                        def _arrow389(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow388(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                def _arrow387(i_3: FSharpList[PreprocessorElement]) -> YAMLElement:
                                    return loop_read(i_3, empty())

                                return map_1(_arrow387, sequence_elements_3)

                            return append(singleton(loop_read(singleton_1(PreprocessorElement(2, value_2(v_3["Value"]))), empty())), delay(_arrow388))

                        acc_mut = cons(YAMLElement(2, to_list(delay(_arrow389))), acc)
                        continue
                        ...
                        def _arrow391(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow390(value: str) -> YAMLElement:
                                return loop_read(singleton_1(PreprocessorElement(2, value.strip())), empty())

                            return map_1(_arrow390, split)

                        current_4: YAMLElement = YAMLElement(2, to_list(delay(_arrow391)))
                        restlist_mut = rest_4
                        ...
                        def _arrow393(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                            def _arrow392(i_4: PreprocessorElement) -> IEnumerable_1[YAMLElement]:
                            ...
                            return collect(_arrow392, i_list)

                        current_5: YAMLElement = YAMLElement(2, to_list(delay(_arrow393)))
                        restlist_mut = rest_5
                        def _arrow394(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c2_2: str = c2
                            return of_array_with_tail([YAMLElement(4, c2_2), current_5], acc)

                        def _arrow395(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c1_2: str = c1
                            return of_array_with_tail([current_5, YAMLElement(4, c1_2)], acc)

                        def _arrow396(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> FSharpList[YAMLElement]:
                            c1_1: str = c1
                            c2_1: str = c2
                            return of_array_with_tail([YAMLElement(4, c2_1), current_5, YAMLElement(4, c1_1)], acc)

                        acc_mut = (cons(current_5, acc) if (c2 is None) else _arrow394()) if (c1 is None) else (_arrow395() if (c2 is None) else _arrow396())
                        continue
                        ...
                        if pattern_matching_result_3 == 0:
                            ...
                            def _arrow398(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                def _arrow397(value_1: str) -> IEnumerable_1[YAMLElement]:
                                ...
                                return collect(_arrow397, split_1)

                            current_6: FSharpList[YAMLElement] = reverse(to_list(delay(_arrow398)))
                            restlist_mut = rest_6
                            ...
                            def _arrow400(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                def _arrow399(i_5: PreprocessorElement) -> IEnumerable_1[YAMLElement]:
                                ...
                                return collect(_arrow399, i_list_1)

                            current_7: YAMLElement = YAMLElement(0, YAMLContent_create_27AED5E3(opener_1["Key"], c1_3), YAMLElement(3, to_list(delay(_arrow400))))
                            restlist_mut = rest_7
                            ...
                            if pattern_matching_result_4 == 0:
                                ...
                                if pattern_matching_result_5 == 0:
                                    ...
                                    def _arrow401(e_8: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                        return is_sequence_element(e_8)

                                    sequence_elements_4: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow401, rest0_4)))
                                    def _arrow402(e_9: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                        return is_sequence_element(e_9)

                                    rest_9: FSharpList[PreprocessorElement] = to_list(skip_while(_arrow402, rest0_4))
                                    def _arrow405(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                        def _arrow404(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                            def _arrow403(i_6: FSharpList[PreprocessorElement]) -> YAMLElement:
                                                return loop_read(i_6, empty())

                                            return map_1(_arrow403, sequence_elements_4)

                                        return append(singleton(loop_read(object_list_2, empty())), delay(_arrow404))

                                    seq: YAMLElement = YAMLElement(2, to_list(delay(_arrow405)))
                                    restlist_mut = rest_9
                                    ...
                                    if pattern_matching_result_6 == 0:
                                        ...
                                        def _arrow406(e_10: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                            return is_sequence_element(e_10)

                                        sequence_elements_5: FSharpList[FSharpList[PreprocessorElement]] = collect_sequence_elements(to_list(take_while(_arrow406, rest0_5)))
                                        def _arrow407(e_11: PreprocessorElement, restlist: Any=restlist, acc: Any=acc) -> bool:
                                            return is_sequence_element(e_11)

                                        rest_10: FSharpList[PreprocessorElement] = to_list(skip_while(_arrow407, rest0_5))
                                        def _arrow410(__unit: None=None, restlist: Any=restlist, acc: Any=acc) -> IEnumerable_1[YAMLElement]:
                                            def _arrow409(__unit: None=None) -> IEnumerable_1[YAMLElement]:
                                                def _arrow408(i_7: FSharpList[PreprocessorElement]) -> YAMLElement:
                                                    return loop_read(i_7, empty())

                                                return map_1(_arrow408, sequence_elements_5)

                                            return append(singleton(loop_read(singleton_1(PreprocessorElement(2, value_2(w_1["Value"]))), empty())), delay(_arrow409))

                                        seq_1: YAMLElement = YAMLElement(2, to_list(delay(_arrow410)))
