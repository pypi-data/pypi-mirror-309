...
class FsWorkbook(IDisposable):
    ...
    def RemoveWorksheet(self, name: str) -> None:
        self_1: FsWorkbook = self
        def _arrow192(__unit: None=None) -> FsWorksheet:
            try: 
                def _arrow191(ws: FsWorksheet) -> bool:
                    return ws.Name == name

                return default_arg(try_find_1(_arrow191, self_1._worksheets), None)
        ...
        ignore(remove_in_place(_arrow192(), self_1._worksheets, ObjectExpr193()))
...
FsWorkbook_reflection = _expr194
