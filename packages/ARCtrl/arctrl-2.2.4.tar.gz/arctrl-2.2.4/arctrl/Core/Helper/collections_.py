...
def Dictionary_ofSeq(s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr509())
...
def Dictionary_tryFind(key: Any, dict_1: Any) -> Any | None:
    ...
    out_arg: _T = None
    def _arrow510(__unit: None=None, key: Any=key, dict_1: Any=dict_1) -> _T:
        return out_arg

    def _arrow511(v: _T | None=None, key: Any=key, dict_1: Any=dict_1) -> None:
        nonlocal out_arg
        out_arg = v

    pattern_input = (try_get_value(dict_1, key, FSharpRef(_arrow510, _arrow511)), out_arg)
    if pattern_input[0]:
...
def Dictionary_ofSeqWithMerge(merge: Callable[[_T, _T], _T], s: IEnumerable_1[tuple[_KEY, _T]]) -> Any:
    ...
    dict_1: Any = Dictionary([], ObjectExpr512())
...
def ResizeArray_distinct(a: Array[Any]) -> Array[Any]:
    ...
    try: 
        while enumerator.System_Collections_IEnumerator_MoveNext():
            ...
            if not contains(i, b, ObjectExpr513()):
                (b.append(i))
