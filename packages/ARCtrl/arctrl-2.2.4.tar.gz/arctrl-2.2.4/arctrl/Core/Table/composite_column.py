...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow712(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow711(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow711, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow712))
...
CompositeColumn_reflection = _expr713
