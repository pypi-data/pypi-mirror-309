...
def _arrow1115(__unit: None=None) -> Callable[[str], str]:
    clo_1: Callable[[str], str] = to_text(printf("%s %s"))(Person_AssayIdentifierPrefix)
    return clo_1


Person_createAssayIdentifierKey: Callable[[str], str] = _arrow1115()
...
def ProcessParsing_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], Component] | None:
    if value_header.tag == 0:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], Component]:
            def _arrow1117(i: int, matrix: Any=matrix) -> Component:
...
def ProcessParsing_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], ProcessParameterValue] | None:
    if value_header.tag == 3:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], ProcessParameterValue]:
            def _arrow1118(i_1: int, matrix: Any=matrix) -> ProcessParameterValue:
...
def ProcessParsing_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], FactorValue] | None:
    if value_header.tag == 2:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], FactorValue]:
            def _arrow1119(i_1: int, matrix: Any=matrix) -> FactorValue:
...
def ProcessParsing_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], MaterialAttributeValue] | None:
    if value_header.tag == 1:
        ...
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], MaterialAttributeValue]:
            def _arrow1122(i_1: int, matrix: Any=matrix) -> MaterialAttributeValue:
...
def ProcessParsing_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], OntologyAnnotation] | None:
    if header.tag == 4:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], OntologyAnnotation]:
            def _arrow1123(i: int, matrix: Any=matrix) -> OntologyAnnotation:
...
def ProcessParsing_tryGetProtocolDescriptionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 5:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow1125(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetProtocolURIGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 6:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow1126(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessParsing_tryGetInputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessInput] | None:
    if header.tag == 11:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessInput]:
            def _arrow1128(i: int, matrix: Any=matrix) -> ProcessInput:
...
def ProcessParsing_tryGetOutputGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], ProcessOutput] | None:
    if header.tag == 12:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], ProcessOutput]:
            def _arrow1129(i: int, matrix: Any=matrix) -> ProcessOutput:
...
def ProcessParsing_tryGetCommentGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], Comment] | None:
    if header.tag == 14:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], Comment]:
            def _arrow1130(i: int, matrix: Any=matrix) -> Comment:
...
def ProcessParsing_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], Process]:
    ...
    if match_value is None:
        def _arrow1132(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow1131(i_1: int) -> FSharpList[ProcessInput]:
        ...
        input_getter_1 = _arrow1132

    else: 
        input_getter: Callable[[Any, int], ProcessInput] = match_value
        def _arrow1134(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessInput]]:
            def _arrow1133(i: int) -> FSharpList[ProcessInput]:
    ...
    if match_value_1 is None:
        def _arrow1136(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow1135(i_3: int) -> FSharpList[ProcessOutput]:
        ...
        output_getter_1 = _arrow1136

    else: 
        output_getter: Callable[[Any, int], ProcessOutput] = match_value_1
        def _arrow1138(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], FSharpList[ProcessOutput]]:
            def _arrow1137(i_2: int) -> FSharpList[ProcessOutput]:
        ...
        output_getter_1 = _arrow1138

    def _arrow1140(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Process]:
        def _arrow1139(i_4: int) -> Process:
...
def ProcessParsing_groupProcesses(ps: FSharpList[Process]) -> FSharpList[tuple[str, FSharpList[Process]]]:
    ...
    class ObjectExpr1142:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1141(x_1: str, y: str) -> bool:
                return x_1 == y
    ...
    return List_groupBy(projection, ps, ObjectExpr1142())
...
def ProcessParsing_mergeIdenticalProcesses(processes: FSharpList[Process]) -> FSharpList[Process]:
    ...
    l: FSharpList[tuple[tuple[str, Any, Any | None, Any | None], FSharpList[Process]]] = List_groupBy(projection, processes, ObjectExpr1143())
...
def ProcessParsing_processToRows(p: Process) -> FSharpList[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow1148(__unit: None=None, p: Any=p) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow1147(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow1146(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow1145(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(singleton_1((CompositeHeader(6), CompositeCell(1, value_10(prot_1.Uri)))) if (prot_1.Uri is not None) else empty_1(), delay(_arrow1144))

                    return append(singleton_1((CompositeHeader(5), CompositeCell(1, value_10(prot_1.Description)))) if (prot_1.Description is not None) else empty_1(), delay(_arrow1145))

                return append(singleton_1((CompositeHeader(4), CompositeCell(0, value_10(prot_1.ProtocolType)))) if (prot_1.ProtocolType is not None) else empty_1(), delay(_arrow1146))

            return append(singleton_1((CompositeHeader(8), CompositeCell(1, value_10(prot_1.Name)))) if (prot_1.Name is not None) else empty_1(), delay(_arrow1147))

        prot_vals = to_list(delay(_arrow1148))
    ...
    def mapping_6(tupled_arg_1: tuple[tuple[str, str], FSharpList[tuple[ProcessInput, ProcessOutput]]], p: Any=p) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
        ...
        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_5, sort_by(projection_1, append_1(chars, append_1(components, append_1(pvs, map_2(mapping_4, ProcessOutput_getFactorValues_Z42C11600(output_for_factors))))), ObjectExpr1149()))
        def _arrow1154(__unit: None=None, tupled_arg_1: Any=tupled_arg_1) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow1153(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow1152(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow1151(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(comments, delay(_arrow1150))

                    return append(vals, delay(_arrow1151))

                return append(prot_vals, delay(_arrow1152))

            return append(singleton_1(JsonTypes_decomposeProcessInput(input_for_type)), delay(_arrow1153))

        return to_list(delay(_arrow1154))
    ...
    def _arrow1155(__unit: None=None, p: Any=p) -> FSharpList[tuple[ProcessInput, ProcessOutput]]:
    ...
    return map_2(mapping_6, List_groupBy(projection, _arrow1155(), ObjectExpr1156()))
