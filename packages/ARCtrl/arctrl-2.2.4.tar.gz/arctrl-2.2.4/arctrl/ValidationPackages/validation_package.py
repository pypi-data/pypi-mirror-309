...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow3035(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3034(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3033(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3032(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow3032))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow3033))

            return append(singleton("{"), delay(_arrow3034))

        return join("\n", to_list(delay(_arrow3035)))
...
ValidationPackage_reflection = _expr3043
