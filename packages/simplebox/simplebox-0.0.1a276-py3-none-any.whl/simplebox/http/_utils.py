...
@_call_stack_check("request", "get", "post", "put", "delete", "patch", "head", "options")
def request_pk(api, method: HttpMethod or str):
    if isinstance(method, HttpMethod):
        return f"{method.value}:{api}"
    elif isinstance(method, str):
        return f"{method.upper()}:{api}"
    else:
        return f"{method}:{api}"
