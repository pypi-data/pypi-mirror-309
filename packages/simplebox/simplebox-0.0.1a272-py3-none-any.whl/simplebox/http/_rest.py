...
class RestContext(BaseContext):
    ...
    def put(self, **kwargs: BaseRest):
        for k, v in kwargs.items():
            if not isinstance(v, BaseRest):
                raise TypeError(f"'{k}'\'s value '{v}' expected type '{BaseRest.__name__}', got a '{type(v).__name__}'")
...
class RestWrapper(BaseRestWrapper):
    ...
    def __init__(self, rest: BaseRest = None, name: str = None, host: str = None,
    ...
    def stats_datas(self) -> 'StatsSentUrl':
        return self.__rest_.stats_datas if self.__rest_ else self.__rest.stats_datas

    def copy(self) -> 'BaseRestWrapper':
        return RestWrapper(self.__rest_.copy())
    ...
    def __parser_rest(self, func, args):
        if self.__rest_ is None:
            if inspect.ismethod(func):
                api_aware: ApiAware = func.__self__
                self.__rest_ = api_aware.context.get(api_aware.tag)
            elif len(args) > 0:
                if isinstance(args[0], ApiAware):
                    api_aware: ApiAware = args[0]
                    self.__rest_ = api_aware.context.get(api_aware.tag)
            else:
    ...
    def __request(self, api: str, func=None, method: HttpMethod or str = None, allow_redirection: bool = True,
        ...
        try:
            ...
            if self.__stats is True and stats is True:
                self.__stats_datas.add((_url, method))
            return rest_resp
