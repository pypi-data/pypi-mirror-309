...
from mybpm.constant import APPLY_USER_ID, FORM_VAR, URL_CAMUNDA_ENGINE
...
def get_instance_start_user_email(instance_id):
    getlist = hisvariableinst.GetVariables(
        url=URL_CAMUNDA_ENGINE, process_instance_id=instance_id, variable_name=APPLY_USER_ID
    )
    variables = getlist()
    if len(variables) == 0:
        return None
