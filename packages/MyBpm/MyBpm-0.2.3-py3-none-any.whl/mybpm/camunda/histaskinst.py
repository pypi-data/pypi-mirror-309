...
class HistoryTaskInstance:
    ...
    @classmethod
    def load(cls, data: typing.Mapping[str, typing.Any]) -> HistoryTaskInstance:
...
class GetTasks(pycamunda.base.CamundaRequest):
    task_id = QueryParameter("taskId")
    ...
    finished = QueryParameter("finished")
    ...
    def __call__(self, *args, **kwargs) -> typing.Tuple[HistoryTaskInstance]:
        """Send the request."""
        response = super().__call__(pycamunda.base.RequestMethod.GET, *args, **kwargs)
        return tuple(HistoryTaskInstance.load(data_json) for data_json in response.json())
