...
log.add(sys.stderr, format=loguru_formatter)
...
async def process_stream(address: str, port: int, response: aiohttp.ClientResponse) -> List[Dict[str, Any]]:
    ...
    async for line in response.content:
        ...
        if line.startswith("data: "):
            ...
            for message_type in ["message", "progress", "error"]:
                if message_type in json_obj:
                    message_type = message_type
                    break
            else:
                log.error(f"Unknown message type: {json_obj}")
                # It's a bug report, we store in results
                results.append(json_obj)
                continue

            client_message = HackBotClientMessage(type=message_type, message=json_obj.get(message_type))
            client_message.log()

            if client_message.type == "message" and client_message.message.startswith("Starting HackBot session"):
                session_id = client_message.message.split("'")[1]
                log.info(f"Access the results at https://hackbot.org/dashboard/security-report#{session_id}")
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix=".zip") as temp_zip:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url, data=data, headers=headers) as response:
                ...
                results = await process_stream(address, port, response)
...
async def cli_hack_target(address: str, port: int, api_key: str, source_path: str = ".", output: Optional[str] = None) -> None:
    ...
    if len(results) == 0:
        ...
        log.info(f"\U0001F7E1 Found {len(results)} issues")
        for result in results:
            log.info(" - " + result.get("title"))
...
def main() -> None:
    ...
    try:
        ...
        if args.issues_repo:
            ...
            log.info("No github repository for reporting issues has been specified. Skipping github issue generation.")
