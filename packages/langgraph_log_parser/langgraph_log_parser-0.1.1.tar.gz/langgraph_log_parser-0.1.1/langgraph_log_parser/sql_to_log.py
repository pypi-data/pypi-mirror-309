...
def export_sqlite_to_log(db_path="checkpoints.sqlite", output_folder="files/logs"):
    """
    ...
    # Zapisz dane dla każdego thread_ID w osobnym pliku logów
    for thread_id, logs in data_by_thread.items():
        output_path = os.path.join(output_folder, f"thread_{thread_id}.log")
        try:
            with open(output_path, 'w') as log_file:
                # Zapisz dane jako JSON
                json.dump(logs, log_file, indent=4)
            print(f"Log file created: {output_path}")
        except Exception as e:
            print(f"Error writing log file for thread_ID {thread_id}: {e}")
