...
class NiftiViewer(QMainWindow):
    ...
    def init_tabs(self):
        ...
        action = QAction("Perfusion DSC", self)
        action.triggered.connect(lambda x: self.transforms("Perfusion DSC"))
    ...
    def run_transform(self, *args):
        if args[0] == "Sort DICOM":
            ...
            if cbv: 
                new_path = signal.solve_CBV(clip=True)
                if new_path not in self.nifti_found:
                    self.load_sequence(new_path)
                    self.nifti_found.append(new_path)
                    self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
            if cbf: 
                new_path = signal.solve_CBF()
                if new_path not in self.nifti_found:
                    self.load_sequence(new_path)
                    self.nifti_found.append(new_path)
                    self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
            if mtt: 
                new_path = signal.solve_MTT()
                if new_path not in self.nifti_found:
                    self.load_sequence(new_path)
                    self.nifti_found.append(new_path)
                    self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
            if ttp: 
                new_path = signal.solve_TTP()
                if new_path not in self.nifti_found:
                    self.load_sequence(new_path)
                    self.nifti_found.append(new_path)
                    self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
            if tmax: 
                new_path = signal.solve_Tmax()
                if new_path not in self.nifti_found:
                    self.load_sequence(new_path)
                    self.nifti_found.append(new_path)
                    self.thumbnail_title.setText(f"<h2 style='margin-left: 10px'>{len(self.nifti_found)} images loaded:</h2>")
...
class TransformsGUI(QWidget):
    def __init__(self, path, selected_path, transform, run_transform, conf):
        ...
        if transform == "Sort DICOM":
            ...
            baseline = QLineEdit(self)
            baseline.setText("8")
            title_6 = QLabel("Offset")
            offset = QLineEdit(self)
            offset.setText("0")

            vbox_layout.addWidget(title_5)
            vbox_layout.addWidget(te)
            vbox_layout.addWidget(title_4)
            vbox_layout.addWidget(baseline)
            vbox_layout.addWidget(title_6)
            vbox_layout.addWidget(offset)
            vbox.setLayout(vbox_layout)
            hbox_layout.addWidget(vbox)

            vbox = QWidget()
            vbox_layout = QVBoxLayout()
            title_8 = QLabel("Perfusion maps")
            cbv = QCheckBox("CBV")
            cbf = QCheckBox("CBF")
            mtt = QCheckBox("MTT")
            ttp = QCheckBox("TTP")
            ...
            vbox_layout.addWidget(title_8)
            vbox_layout.addWidget(cbv)
            vbox_layout.addWidget(cbf)
            vbox_layout.addWidget(mtt)
            vbox_layout.addWidget(ttp)
            vbox_layout.addWidget(tmax)
            vbox.setLayout(vbox_layout)
            hbox_layout.addWidget(vbox)
            ...
            vbox_layout.addWidget(title_7)
            vbox_layout.addWidget(svd)
            vbox_layout.addWidget(tsvd)
            vbox_layout.addWidget(osvd)
            vbox_layout.addWidget(tikhonov)
            vbox.setLayout(vbox_layout)
            hbox_layout.addWidget(vbox)
            ...
            button = QPushButton("Run Function", self)
            button.clicked.connect(lambda: self.launch_transform(transform, combo_box_1.currentText(), combo_box_2.currentText(), combo_box_3.currentText(), self.deconv.text(), te.text(), baseline.text(), offset.text(), cbv.isChecked(), cbf.isChecked(), mtt.isChecked(), ttp.isChecked(), tmax.isChecked()))
            layout.addWidget(button, alignment=Qt.AlignmentFlag.AlignHCenter)
