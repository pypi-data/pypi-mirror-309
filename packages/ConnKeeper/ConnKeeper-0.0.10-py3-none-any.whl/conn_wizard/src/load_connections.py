...
def make_string(conn_det):
    if conn_det['ConnectionType'] == 'postgres': 
        conn_str = f"{conn_det['driver']}://{conn_det['user']}:{conn_det['password']}@{conn_det['host']}/{conn_det['database']}"
        return conn_str

    elif conn_det['ConnectionType'] == 'sqlserver':
        conn_str = f"DRIVER={conn_det['driver']};SERVER={conn_det['host']};DATABASE={conn_det['database']};Trusted_Connection=yes" 
        if 'user' in conn_det and 'password' in conn_det:
            conn_str += f";USER={conn_det['user']};PASSWORD={conn_det['password']}" 
            return conn_str
