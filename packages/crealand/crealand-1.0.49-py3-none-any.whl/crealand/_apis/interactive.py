...
class Interactive:
    ...
    def show_tag(runtime_id: int, content: str, height: int=0):
        height_value = max(-5, min(height, 30))
        _interface.call_api('unity', "unity.actor.showTextHUD", [runtime_id, content, height_value])


    @_utils.check_type
    @staticmethod
    def hide_tag(runtime_id: int):
        _interface.call_api('unity', "unity.actor.hideTextHUD", [runtime_id])
