...
class MessageBar(Dialog):
    ...
    @classmethod
    def info(cls, title: str, text: str, parent=None) -> ButtonRole:
        return cls(title, text, 'info', parent=parent).exec()

    @classmethod
    def warning(cls, title: str, text: str, parent=None) -> ButtonRole:
        return cls(title, text, 'warning', parent=parent).exec()

    @classmethod
    def error(cls, title: str, text: str, parent=None) -> ButtonRole:
        return cls(title, text, 'error', parent=parent).exec()

    @classmethod
    def success(cls, title: str, text: str, parent=None) -> ButtonRole:
        return cls(title, text, 'success', parent=parent).exec()

    @classmethod
    def question(cls, title: str, text: str, parent=None) -> bool:
        buttons = [ButtonRole.OK, ButtonRole.Cancel]
        bar = cls(title, text, 'info', buttons, parent)
        return bar == ButtonRole.OK
    ...
    def __create_frame(self,
        ...
        match intent:
            case 'info':
                qss = {'background': '#fff', 'border': f'2 solid #d1d1d1'}
            case 'warning':
