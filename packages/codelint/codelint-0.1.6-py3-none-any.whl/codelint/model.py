    ...
    def check_snake_case(self):
        ...
        with open(self.file_path, "r") as file:
            ...
            for line_number, line in enumerate(lines, start=1):
                ...
                if "def " in line:
                    # Extract function name by splitting on `def` and handling the parentheses
                    function_name = line.split("def ")[1].split("(")[0].strip()
                    if not re.match(r"^[a-z_][a-z0-9_]*$", function_name):
                        issues.append(f"Snake case issue on line {line_number}: {function_name}")
                
                # Checking variable names (using '=')
                if "=" in line:
                    # Check for assignment only (avoid parameters or special cases)
                    if "(" not in line and ")" not in line:  # Skip function arguments or method calls
                        variable_name = line.split("=")[0].strip()
                        # Check variable name for snake_case (excluding special cases like self, cls, etc.)
                        if variable_name not in ("self", "cls") and not re.match(r"^[a-z_][a-z0-9_]*$", variable_name):
