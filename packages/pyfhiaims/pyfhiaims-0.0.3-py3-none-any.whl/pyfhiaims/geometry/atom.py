...
from pyfhiaims.utils.typecast import Vector3D
...
class FHIAimsAtom:
    ...
    def to_string(self):
        ...
        content_str = [f"{atom_str} " + " ".join([f'{pi:>20.12e}' for pi in pos]) + f" {self.symbol}"]

        if self.velocity is not None:
            content_str.append(f"    velocity " + " ".join([f"{vel:>20.12e}" for vel in self.velocity]))
        ...
        if self.magnetic_response is not None and self.magnetic_response:
            content_str.append(f"    magnetic_response")
        ...
        if self.RT_TDDFT_initial_velocity is not None:
            content_str.append(f"    RT_TDDFT_initial_velocity " +
