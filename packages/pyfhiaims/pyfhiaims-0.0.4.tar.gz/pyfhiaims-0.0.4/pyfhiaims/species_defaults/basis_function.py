...
class BasisFunction:
    ...
    def content_block(self, content_fields: Sequence[str]) -> str:
        ...
        return (f"{'#' if not self.enabled else ' '} "
                f"{('for_aux' if self.aux else ''):>7}{self.type:>9} {self.n:6d}  {self.l}  {' '.join(content)}")
...
class GaussianBasisFunction(BasisFunction):
    ...
    def __post_init__(self):
        ...
        if self.n > 1 and not self.coeff_i:
            raise BasisFunctionError(f"Total number of Gaussians is {self.n} so coeff_i must be passed.")
    ...
    def from_string(cls, string: str) -> GaussianBasisFunction:
        ...
        if int(n[0]) == 1:
            ...
            for line in lines[1:]:
                alpha_i.append(float(line.split()[0]))
                coeff_i.append(float(line.split()[1]))
        return cls(
            n=int(n[0]),
            l=l,
            alpha_i=alpha_i,
    ...
    def __str__(self) -> str:
        """Get the content block for the control.in file"""
        title_line = (f"{'#' if not self.enabled else ' '} "
        ...
        lines = [title_line, ]
        for alpha, coeff in zip(self.alpha_i, self.coeff_i):
            lines.append(f"{'#' if not self.enabled else ' '}   {alpha:.7f}  {coeff:.7f}")

        return "\n".join(lines)


BASIS_TYPES = {cls.type: cls for _, cls in
               inspect.getmembers(sys.modules[__name__], lambda member: inspect.isclass(member) and
                                                                        issubclass(member, BasisFunction) and
                                                                        member.__name__ != "BasisFunction")
               }
