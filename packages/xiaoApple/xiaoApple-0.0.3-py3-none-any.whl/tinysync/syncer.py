...
class Sync:
    ...
    def __init__(self,workdir,syncConfig, backendA:Backend, backendB:Backend, break_lock=None):
        ...
        backendHandlerA = BackendHandler(backendA,linkMode,workdir,syncConfig=syncConfig)
        backendHandlerB = BackendHandler(backendB,linkMode,workdir,syncConfig=syncConfig)
        ...
        if len(self.currA)>0:
            log("==================[delta on A]:")
            for f in self.currA:
                log(f" {sc(f['Path'],30)}  {sc(str(f['mtime']),20)}    {sc(str(f['Size']),20)}")         
