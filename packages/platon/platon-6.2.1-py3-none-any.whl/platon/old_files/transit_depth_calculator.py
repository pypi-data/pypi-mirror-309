...
class TransitDepthCalculator:
    ...
    def compute_depths(self, star_radius, planet_mass, planet_radius,
                       ...
                       full_output=False, min_abundance=1e-99, min_cross_sec=1e-99, stellar_blackbody=False, zero_opacities=[], cloud_frac=1):

        custom_T_profile[custom_T_profile > 3000] = 3000
        custom_T_profile[custom_T_profile < 200] = 200
        #plt.semilogy(custom_T_profile.get(), custom_P_profile.get())
       ...
       '''
        ...
        '''below_clouds = (P_profile[0:-1] + P_profile[1:])/2 > cloudtop_pressure
        absorption_fraction[:, below_clouds] = 1        
        transit_depths_cloudy = (radii.min() / star_radius)**2 \
            + 2 / star_radius**2 * absorption_fraction.dot(radii[1:] * dr)
        transit_depths = cloud_frac * transit_depths_cloudy + (1 - cloud_frac) * transit_depths_clear'''
