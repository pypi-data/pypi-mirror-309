...
class CombinedRetriever:
    ...
    def _ln_like(self, params, transit_calc, eclipse_calc, fit_info, measured_transit_depths,
        ...
        #Set constant abundances
        ones = xp.ones((30, 13))
        custom_abunds = {}
        total = 0.
        for key in ["CH4", "CO2", "CO", "H2O", "H2S", "HCN", "K", "Na", "NH3", "SO2"]:
            total += 10.**params_dict["log_" + key]
            custom_abunds[key] = 10.**params_dict["log_" + key] * ones
        if total > 1: return -np.inf
        custom_abunds["H2-He"] = (1 - total) * ones
        '''custom_abunds["He"] = 0.15 * ones

        total = 0.
        for key in custom_abunds:            
            total += custom_abunds[key].mean()

        for key in custom_abunds:
            custom_abunds[key] /= total'''
        ...
        try:
            if measured_transit_depths is not None:
                ...
                transit_wavelengths, calculated_transit_depths, transit_info_dict = transit_calc.compute_depths(
                    Rs, Mp, Rp, temperature=None, logZ=None, CO_ratio=None,
                    custom_T_profile=t_p_profile.temperatures,
                    custom_P_profile=t_p_profile.pressures,
                    custom_abundances=custom_abunds,
                    ...
                    part_size=part_size, ri=ri, P_quench=P_quench, full_output=ret_best_fit, zero_opacities=zero_opacities, cloud_frac=1)
                ...
                '''transit_wavelengths, calculated_transit_depths_clear, transit_info_dict = transit_calc.compute_depths(
                    ...
                    part_size=part_size, ri=None, P_quench=P_quench, full_output=ret_best_fit, zero_opacities=zero_opacities, cloud_frac=0)'''
                
                #calculated_transit_depths = cloud_frac * calculated_transit_depths_cloudy + (1 - cloud_frac) * calculated_transit_depths_clear
            ...
            if measured_eclipse_depths is not None:
                ...
                eclipse_wavelengths, calculated_eclipse_depths, eclipse_info_dict = eclipse_calc.compute_depths(
                    t_p_profile, Rs, Mp, Rp, T_star, logZ=None, CO_ratio=None,
                    custom_abundances=custom_abunds,
                    scattering_factor=scatt_factor, scattering_slope=scatt_slope,
    ...
    def get_default_fit_info(Rs, Mp, Rp, T=None, logZ=0, CO_ratio=0.53,
                             log_CH4=-5, log_CO2=-3, log_CO=-3, log_H2O=-3, log_H2S=-5, log_HCN=-4, log_K=-5, log_Na=-4, log_NH3=-4, log_SO2=-4,
