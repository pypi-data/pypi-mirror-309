...
def create_module_py(args):
    ...
    template_path = pkg_resources.resource_stream(
        __name__, 'templates/module.py'
    )
    
    # Read the template contents
    content = template_path.read().decode('utf-8')  # Ensure to decode if it's in bytes

    # with importlib.resources.path('ecidacli.templates', 'module.py') as template_path:
    #     # Read template contents
    #     with open(template_path, 'r') as template_file:
    #         content = template_file.read()
    # with open("./templates/module.py", 'r') as template_file:
    #         content = template_file.read()
    content = content.replace('MODULE_NAME', module_name)
    
    output_path = os.path.join(os.getcwd(), f"{module_name}.py")
...
def main():
    ...
    create_subparsers = add_resource_command(create_subparsers, "create", "module", "module", create_module_py)
...
def load_function(file_path, func_name):
    ...
    for func in functions:
        if func.name == func_name:
            ...
            exec(new_module, module.__dict__)
