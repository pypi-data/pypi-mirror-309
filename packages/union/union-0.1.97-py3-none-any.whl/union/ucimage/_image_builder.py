    ...
    if execution is None:
        ...
        execution = remote.execute(
            ...
            options=Options(
                ...
                labels=Labels(values={UNIONAI_IMAGE_NAME_KEY: _sanitize_image_name(target_image)}),
            ),
        )
...
def _sanitize_image_name(image: str) -> str:
    """Sanitize image name for Flyte labels."""
    return re.sub("[:/]", "_", image)
...
def _get_latest_image_build_execution(remote: FlyteRemote, image_name: str) -> Optional[FlyteWorkflowExecution]:
    ...
    """
    # Replace ':' with '_' since flyte does not allow ':' in the label value
    target_image = _sanitize_image_name(image_name)
