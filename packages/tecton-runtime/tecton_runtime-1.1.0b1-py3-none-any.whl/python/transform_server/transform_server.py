...
def main():
    ...
    if transform_server_groups_enabled:
        # Double the number of workers if transform server groups are enabled
        # This is necessary because every request invokes transformation pipeline
        # in a 2nd thread as a way to enforce timeouts
        max_workers = os.environ.get("TRANSFORM_SERVER_MAX_WORKERS", 2 * calculate_optimal_workers())
    ...
    transform_service_pb2_grpc.add_TransformServiceServicer_to_server(TransformServer(threadpool_executor), server)
