...
def main():
	...
	parser.add_argument('-t','--target-number',type = int, required=False,
		help = "Specifies the number of targets that the uRAD can detect as the same time. Must be integer between 1 and 5. If missing, it defaults to 1"
		)
	...
	parser.add_argument('-T','--test',action='store_true', help='Runs the scripts only for testing tests the connection between PC and the uRAD.'+ 
		'It will usually used after installation to test is the uRAD can communicate witht the PC.')
