/*+
    ARES/HADES/BORG Package -- ./extra/borg/libLSS/physics/likelihoods/negative_binomial_alt.hpp
...
+*/
#ifndef __LIBLSS_ALT_NEGATIVE_BINOMIAL_HPP
#define __LIBLSS_ALT_NEGATIVE_BINOMIAL_HPP
...
namespace LibLSS {
  ...
  class AltNegativeBinomialLikelihood {
  public:
    AltNegativeBinomialLikelihood(LikelihoodInfo const & = LikelihoodInfo()) {}

    static const size_t numberLikelihoodParams = 2;

    static inline double
    log_nb_proba(double d, double lambda, double sigma_nb) {
      double r = 1 / sigma_nb;
      double q = sigma_nb * lambda;
      double u = q / (1 + q);
      double loglambda = std::log(lambda); // This will have to change
      return gsl_sf_lngamma(d + r) - gsl_sf_lngamma(r) - r * std::log(1 + q) +
             d * std::log(u);
    }

    static inline double diff_log_nb_proba(double d, double lambda, double r) {
      return (d + 1. / r) * lambda * r / (r * lambda + 1.) - d;
    }

    template <typename RandomGen>
    static inline double
    gen_nb_sample(RandomGen &rgen, double lambda, double sigma_nb) {
      double r = 1 / sigma_nb;
      double a = sigma_nb * lambda;
      return rgen.negative_binomial(1 / (1 + a), r);
    }
