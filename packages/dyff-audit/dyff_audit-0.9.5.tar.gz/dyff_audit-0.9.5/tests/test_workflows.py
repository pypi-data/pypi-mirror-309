...
@pytest.mark.datafiles(DATA_DIR)
def test_models_create_mock_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    ...
    model = dyffapi.models.create(model_request)
    print(f"model compare 1: {model.id}")
    ctx["model_mock_compare_1"] = model
...
@pytest.mark.depends(
    on=[
        "test_models_create_mock_compare_1",
    ]
)
def test_inferenceservices_create_mock_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    if not pytestconfig.getoption("enable_comparisons"):
        pytest.skip()
    ...
    account = ctx["account"]
    model: Model = ctx["model_mock_compare_1"]
    ...
    if pytestconfig.getoption("test_remote"):
        ...
        print(f"inferenceservice_mock_compare_1: {inferenceservice.id}")
        ctx["inferenceservice_mock_compare_1"] = inferenceservice
    else:
        assert isinstance(dyffapi, DyffLocalPlatform)
...
@pytest.mark.depends(
    on=[
        "test_inferenceservices_create_mock_compare_1",
    ]
)
def test_inferencesessions_create_mock_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    if not pytestconfig.getoption("enable_comparisons"):
        pytest.skip()
    ...
    account = ctx["account"]
    service: InferenceService = ctx["inferenceservice_mock_compare_1"]
    ...
    print(f"inferencesession_mock_compare_1: {session.id}")
    ctx["inferencesession_mock_compare_1"] = session
    ctx["session_token_mock_compare_1"] = session_and_token.token
...
@pytest.mark.depends(
    on=[
        "test_inferencesessions_create_mock_compare_1",
    ]
)
def test_inferencesessions_infer_mock_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    if not pytestconfig.getoption("enable_comparisons"):
        pytest.skip()

    session: InferenceSession = ctx["inferencesession_mock_compare_1"]

    if pytestconfig.getoption("test_remote"):
        assert isinstance(dyffapi, Client)
        session_client = dyffapi.inferencesessions.client(
            session.id,
            ctx["session_token_mock_compare_1"],
            interface=session.inferenceService.interface,
        )
        response = session_client.infer({"text": "Open the pod bay doors, Hal!"})
    else:
...
def test_evaluations_create_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    if not pytestconfig.getoption("enable_comparisons"):
        pytest.skip()
    ...
    inferenceservice = ctx["inferenceservice_mock_compare_1"]
    evaluation_request = EvaluationCreateRequest(
        account=account,
        dataset=dataset.id,
        inferenceSession=EvaluationInferenceSessionRequest(
    ...
    evaluation = dyffapi.evaluations.create(evaluation_request)
    print(f"evaluation compare 1: {evaluation.id}")
    ctx["evaluation_compare_1"] = evaluation
...
@pytest.mark.depends(
    on=[
        "test_datasets_create",
        "test_evaluations_create_compare_1",
        "test_methods_create_python_function",
    ]
)
def test_measurements_create_python_function_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx
):
    if pytestconfig.getoption("skip_analyses"):
        pytest.skip()
    ...
    account = ctx["account"]
    evaluation = ctx["evaluation_compare_1"]
    inferenceservice = ctx["inferenceservice_mock_compare_1"]
    model = ctx["model_mock_compare_1"]
    dataset = ctx["dataset"]
    method_python_function = ctx["method_python_function"]
    measurement_python_function_request = AnalysisCreateRequest(
        account=account,
    ...
    print(f"measurement_python_function_compare_1: {measurement_python_function.id}")
    ctx["measurement_python_function_compare_1"] = measurement_python_function
...
@pytest.mark.depends(
    on=[
        "test_methods_create_jupyter_notebook",
        "test_measurements_create_python_function_compare_1",
    ]
)
@pytest.mark.parametrize("replication", range(1))
def test_safetycase_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, replication
):
    if pytestconfig.getoption("skip_analyses"):
        pytest.skip()
    ...
    measurement_python_function = ctx["measurement_python_function_compare_1"]
    ...
    print(f"safetycase_jupyter_notebook_compare_1: {safetycase_jupyter_notebook.id}")
    ctx["safetycase_jupyter_notebook_compare_1"] = safetycase_jupyter_notebook
...
@pytest.mark.depends(
    on=[
        "test_safetycase_compare_1",
    ]
)
def test_safetycase_publish_compare_1(
    pytestconfig, dyffapi: Client | DyffLocalPlatform, ctx, datafiles
):
    if pytestconfig.getoption("skip_analyses"):
        pytest.skip()
    ...
    safetycase: SafetyCase = ctx["safetycase_jupyter_notebook_compare_1"]
