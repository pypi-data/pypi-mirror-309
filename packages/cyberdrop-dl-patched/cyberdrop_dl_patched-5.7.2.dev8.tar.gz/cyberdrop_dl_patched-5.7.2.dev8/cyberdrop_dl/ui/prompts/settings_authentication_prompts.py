...
def edit_forum_authentication_values_prompt(manager: Manager) -> None:
    ...
    while True:
        ...
        if action == 1:
            ...
            profile = manager.config_manager.settings_data["Browser_Cookies"]["profiles"]
            if profile == "Default":
                profile = None
            
            if action == "chrome":
                get_cookies_from_browser(manager, "chrome", profile=profile)
            elif action == "firefox":
                get_cookies_from_browser(manager, "firefox", profile=profile)
            elif action == "edge":
                get_cookies_from_browser(manager, "edge", profile=profile)
            elif action == "safari":
                get_cookies_from_browser(manager, "safari", profile=profile)
            elif action == "opera":
                get_cookies_from_browser(manager, "opera", profile=profile)
            elif action == "brave":
                get_cookies_from_browser(manager, "brave", profile=profile)
            elif action == "libreWolf":
                get_cookies_from_browser(manager, "librewolf", profile=profile)
            elif action == "opera_gx":
                get_cookies_from_browser(manager, "opera_gx", profile=profile)
            elif action == "vivaldi":
                get_cookies_from_browser(manager, "vivaldi", profile=profile)
            elif action == "chromium":
                get_cookies_from_browser(manager, "chromium", profile=profile)
            return
