...
def monitor_func(logger,
                 ...
                 reset_default_data: bool = True,
                 wait_for_handler: bool = True,
                 max_wait_for_handler: float | int = config.DEFAULT_MAX_WAIT_FOR_HANDLER) -> Callable:
    ...
    Parameters:
        ...
        wait_for_send (bool, optional): Whether to wait for the handler to finish sending all log records before returning the result.
    ...
    Description:
        - This decorator wraps a function to:
          ...
          4. Optionally wait for the queue listener to finish sending records. This is useful in Google Cloud functions. There, when
             returning the result, the handler thread is also killed, potentially leaving some records unsend. 
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if wait_for_handler:
                    print('start waiting')
                    start = time.time()
                    logger.shutdown(timeout=max_wait_for_handler)
                    print('end waiting')
                    while not logger.log_queue.empty() and not (time.time() - start > max_wait_for_handler):
                        print('in while loop')
                        time.sleep(1.0)
