...
class Freezer:
    ...
    def scrap_item(self, item_url):
        ...
        try:
            ...
            if content_type in self.html_mimes:
                ...
                css: cssutils.css.cssstylesheet.CSSStyleSheet = cssutils.parseString(
                    response.content
                )
                    ...
                    if not url.startswith("data:"):
                        ...
                        if validated_link:
                            css_as_string = css_as_string.replace(
                                url, rebuild_link(validated_link)
                            )
                self._save_item_to_disk(item_url_no_anchor, content_type, css_as_string)
            else:
    ...
    def _patch_css_file(self, local_path: Union[Path, str], trans_table: dict) -> bool:
            ...
            for attrib in ["href", "src"]:
                for link in soup.select(f"[{attrib}]"):
                    ...
                    if parts[0] in trans_table.keys():
                        link[attrib] = trans_table[link.get(attrib)] + (
