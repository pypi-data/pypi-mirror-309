...
class Freezer:
    def __init__(
        ...
        self.start_from_domain = parsed_url.netloc
        ...
        self.patch_executor_pool = ThreadPoolExecutor(max_workers=10)
        self.links_done = {}
        self.links_to_do = []
        ...
        self.requests_session.keep_alive = False
        self.before_save_callback = before_save_callback
        self.http_errors = []
    ...
    def validate_candidate_url(
        ...
        if (
            url_no_anchor not in self.links_done.keys()
            and url_no_anchor not in self.links_to_do
        ):  # Not in queue, submit to executor
            self.scrap_executor_pool.submit(self.scrap_item, url_no_anchor)
            self.links_to_do.append(url_no_anchor)
            ...
            if content_type in self.html_mimes:
                ...
                for attrib in ["href", "src"]:
                    for link in soup.select(f"[{attrib}]"):
                        ...
                        if validated_link:
                            link[attrib] = rebuild_link(
                                validated_link
                            )  # Anchor missing ?
                self._save_item_to_disk(item_url_no_anchor, content_type, soup)
            elif content_type == "text/css":
                try:
                    css: cssutils.css.cssstylesheet.CSSStyleSheet = (
                        cssutils.parseString(response.content)
                    )
                        ...
                        if not url.startswith("data:"):
                            ...
                            if validated_link:
                                css_as_string = css_as_string.replace(
                                    url,
                                    rebuild_link(validated_link),  # Anchor missing ?
                                )
    ...
    def _save_item_to_disk(
        ...
        if self.before_save_callback:
            getattr(self, "before_save_callback")(item_url, content_type, data)
    ...
    def _patch_links(self):
        ...
        for _, local_path in self.links_done.items():
            if local_path.endswith(".html"):
                self.patch_executor_pool.submit(
