...
def search_model_params(model_name, X_train, X_test, y_train, y_test, param_grid, result_num=5, iter_num=8,
                        n_proc: int = None, executor: ProcessPoolExecutor = None) -> list[dict]:
	"""
	Train and evaluate a model
	...
	if executor is None:
		...
		if 1 == n_proc:
			sub_n_proc = 1
			results = [_search_params(model_name, classifier, X_train, X_test, y_train, y_test, params) for params in param_grid]
		else:
...
def search_test_size(model, X, y, test_sizes=np.arange(0.15, 0.36, 0.01), random_state=42, evaluate_func=None,
                     n_proc: int = None, topK=5, executor: ProcessPoolExecutor = None) -> float:
	"""
	Examples
	--------
...
def search_random_state(model, X, y, random_states=np.arange(0, 20, 1), test_size=0.2, evaluate_func=None,
                        n_proc: int = None, topK=5, executor: ProcessPoolExecutor = None) -> int:
	"""
	Examples
	--------
...
def multi_round_evaluate(X: np.ndarray, y: np.ndarray, executor: ProcessPoolExecutor = None, *models, **kwargs):
	""" 对比多个模型的稳定评分
