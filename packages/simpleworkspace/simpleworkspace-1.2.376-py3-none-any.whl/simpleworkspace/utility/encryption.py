...
class CrossCryptV2:
    ...
    def GetEncryptor(self):
        ...
        cipher = Cipher(algorithms.AES(self._GetDerivedKey_ForEncryption()), modes.CTR(nonce), backend=default_backend())
        encryptor = cipher.encryptor()
    ...
    def GetDecryptor(self, headers:Headers):
        cipher = Cipher(algorithms.AES(self._GetDerivedKey_ForEncryption()), modes.CTR(headers.nonce), backend=default_backend())
    ...
    def _GetDerivedKey_ForHMAC(self):
        '''used for message authentication of (schema + nonce + ciphertext)'''
