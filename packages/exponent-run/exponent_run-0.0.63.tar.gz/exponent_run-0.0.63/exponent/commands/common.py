...
async def start_client(
    ...
    async with RemoteExecutionClient.session(
        ...
        coros = [
            # We always want the exponent client running to
            # listen for tool call requests from the server
        ...
        if prompt:
            ...
            coros.append(start_chat(client, chat_uuid, prompt))
        elif workflow_id:
            # Similarly, if given a workflow ID, we need to send
            # a request to kick off the workflow
            coros.append(run_workflow(client, chat_uuid, workflow_id))
