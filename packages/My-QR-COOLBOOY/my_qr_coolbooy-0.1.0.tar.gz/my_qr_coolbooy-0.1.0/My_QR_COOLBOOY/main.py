...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: My-QR-COOLBOOY <data_to_encode>")
        sys.exit(1)
...
def create_qr_tool(data, logo_path, frame_color, qr_color, bg_color, text, frame_margin=40, text_color="black"):
    try:
        ...
        max_logo_size = min(qr_width, qr_height) // 4  # Logo should not exceed 1/4th of the QR code's size
...
def main():
    ...
    parser.add_argument('--frame_color', default="#3B3131", help="Color of the frame (default: #3B3131)")
    parser.add_argument('--qr_color', default="black", help="QR code fill color (default: black)")
    parser.add_argument('--bg_color', default="#32CD32", help="QR code background color (default: #32CD32)")
    ...
    parser.add_argument('--text_color', default="black", help="Color of the text (default: black)")
