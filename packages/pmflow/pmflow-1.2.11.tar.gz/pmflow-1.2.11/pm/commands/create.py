...
def create(command: Annotated[str, typer.Argument()],
    ...
    proc = subprocess.run(command, shell=True, stdout=PIPE, stderr=PIPE, text=True, start_new_session=True)
    pid = proc.pid

    stdout, stderr = proc.stdout.strip(), proc.stderr.strip()  # Capture stdout and stderr
