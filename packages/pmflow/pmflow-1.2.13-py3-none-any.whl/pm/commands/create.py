...
def create(command: Annotated[str, typer.Argument()],
    ...
    proc = subprocess.Popen(command, shell=True, stdout=PIPE, stderr=PIPE, text=True, start_new_session=True)
    pid = proc.pid

    # stdout, stderr = proc.stdout.readline(), proc.stderr.readline()  # Capture stdout and stderr
    #
    # # Check if the command failed
    # if proc.returncode != 0:
    #     typer.echo(f"Error 803: Invalid Command")
    #     if verbose:
    #         typer.echo(f"Error output: {stderr.strip()}")
    #     raise typer.Exit(code=1)
    # else:
    #     if verbose:
