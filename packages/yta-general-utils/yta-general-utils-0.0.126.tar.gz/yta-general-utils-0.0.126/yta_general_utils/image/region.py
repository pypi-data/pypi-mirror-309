...
class Coordinate:
    ...
    properly with the 'x' and 'y' value.
    """
    ...
    def __init__(self, x: float, y: float, type: CoordinateType, is_normalized: bool = False):
        if not NumberValidator.is_number_between(x, NORMALIZATION_MIN_VALUE, NORMALIZATION_MAX_VALUE) or not NumberValidator.is_number_between(y, NORMALIZATION_MIN_VALUE, NORMALIZATION_MAX_VALUE):
            raise Exception(f'The "x" and "y" parameters must be values between {str(NORMALIZATION_MIN_VALUE)} and {str(NORMALIZATION_MAX_VALUE)}.')
    ...
    # TODO: This method below should be moved to a VideoCoordinate
    # class that I should create in the 'yta_multimedia' lib
    def recalculate_for_video(self, video: VideoClip, background_video: VideoClip):
        """
        ...
        Imagine a scene of 1920x1080 as the background 
        scenario, and the 'x' and 'y' of the coordinate
        are the corner or the center of the video,
        according to the coordinate 'type'. We will
        calculate the position to place the provided
        'video' ovher the 'background_video' (that could
        be not 1920x1080).
        """
        ...
        if self.type == CoordinateType.CENTER:
            # Coordinates were from center, so adapt to
            # upper left corner
            x -= int(video.w / 2)
            y -= int(video.h / 2)
