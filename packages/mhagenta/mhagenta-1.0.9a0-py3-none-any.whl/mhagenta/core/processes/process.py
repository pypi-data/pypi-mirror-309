...
class Task:
    def __init__(self,
        ...
        self._periodic_add_task: Self | None = None
        self._args = args
        self._kwargs = kwargs
    ...
    @property
    def periodic_add_task(self) -> Self:
        return self._periodic_add_task

    @periodic_add_task.setter
    def periodic_add_task(self, task: Self) -> None:
        self._periodic_add_task = task
...
class ExecQueue:
    ...
    def push(self,
        ...
        if periodic:
            task.periodic_add_task = self._periodic_add(task)
        if priority:
    ...
    def pop(self, priority: bool = False, stop: bool = False) -> Task | None:
        ...
        if task.periodic and not stop and not task.stop_check:
            if task.priority:
                heapq.heappush(self._priority_queue, task.periodic_add_task.queue_item)
    ...
    def _periodic_add(self, task: Task) -> Task:
        def add() -> None:
