...
class TSForecastParameters(BaseSchema):
    ...
    :param observable_columns: An optional array of column headings which identify the observables in the data. Observables are features (commonly called channels in timeseries forecasting problems) which we have knowledge about in the past and future. For example, weather conditions such as temperature or precipitation may be known or estimated in the future but cannot be changed. This field supports specialized uses of timeseries forecasting that the average user is unlikely to encounter.
    :type observable_columns: list[str], optional

    :param control_columns: An optional array of column headings which identify the control channels in the input. Control channels are similar to observable channels, except that future values may be controlled. For example, the discount percentage of a particular product is known and controllable in the future. Similar to observable_columns, control_columns is intended for advanced use cases not typical in most timeseries forecasting problems.
    :type control_columns: list[str], optional

    :param conditional_columns: An optional array of column headings which constitute the conditional variables. The conditional_columns in the data are those known in the past, but not known in the future.
    :type conditional_columns: list[str], optional

    :param static_categorical_columns: An optional array of column headings which identify categorical-valued channels in the input which are fixed over time.
    :type static_categorical_columns: list[str], optional
