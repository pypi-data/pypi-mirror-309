...
class RadialIntegrationGrid(AimsControlChunk):
    """Radial part of integration grid.
    ...
    def from_strings(cls, strings: Sequence[str]) -> "RadialIntegrationGrid":
        """Get a RadialIntegrationGrid instance from a sequence of strings from species' defaults file."""
        num = 0
        r = 0.
        multiplier = 2
        for line in strings:
    ...
    def to_string(self) -> str:
        """A string representation of the AngularIntegrationGrid."""
        result = [f"  radial_base  {self.number:4d}   {self.radius:6.4f}",
...
class AngularIntegrationGrid(AimsControlChunk):
    ...
    keywords: tuple[str] = ("angular_grids", "division", "outer_grid", "angular", "angular_acc", "angular_min")
    ...
    def from_strings(cls, strings: Sequence[str]) -> "AngularIntegrationGrid":
        """Get an AngularIntegrationGrid from a list of strings from species' defaults file."""
        cls_type = "specified"
        r = []
        n = []
        n_outer = 0
    ...
    def to_string(self) -> str:
        """A string representation of the AngularIntegrationGrid."""
        result = [f"  angular_grids  {self.type}", ]
        for r_i, n_i in zip(self.r, self.n):
            result.append(f"  division       {r_i:6.4f} {n_i:4d}")
        result.append(f"  outer_grid     {self.n_outer:<4d}")
        return "\n".join(result)
...
class IntegrationGrid(AimsControlChunk):
    ...
    def from_strings(cls, strings: Sequence[str]) -> "IntegrationGrid":
        radial_str = [s for s in strings if any([k in s for k in RadialIntegrationGrid.keywords])]
        angular_str = [s for s in strings if any([k in s for k in AngularIntegrationGrid.keywords])]
        return cls(
    ...
    def to_string(self) -> str:
        """A string representation of the integration grid."""
