...
from pyaims.control.chunk import AimsControlChunk
from pyaims.errors import InvalidSpeciesInput
from pyaims.species_defaults.basis_function import BASIS_TYPES
from pyaims.species_defaults.basis_set import BasisSet
from pyaims.species_defaults.electronic_configuration import (
    ValenceElectronicConfiguration,
    IonicElectronicConfiguration
)
from pyaims.species_defaults.integration_grid import IntegrationGrid

replace_last = lambda line, _: line.split()[-1]
replace_last_float = lambda line, _: float(line.split()[-1].replace("d", "e"))
replace_last_bool = lambda line, _: line.split()[-1] == ".true."
replace_last_int = lambda line, _: int(line.split()[-1])
replace_last_float_list = lambda line, _: tuple([float(ll.replace("d", "e")) for ll in line.split()[1:]])
append_div = lambda line, val: val + [(float(line.split()[1].replace("d", "e")), int(line.split()[2]))]
append_inact_div = lambda line, val: val + [(float(line.split()[2].replace("d", "e")), int(line.split()[3]))]
...
SPECIES_KEYWORD_MAP = {
    ...
    "cut_core": ("cut_core", lambda line, _: (line.split()[1], float(line.split()[2].replace("d", "e")))),
    "cut_free_atom": ("cut_free_atom", lambda line, _: (line.split()[1], float(line.split()[2].replace("d", "e")))),
    ...
    "plus_u": ("plus_u", lambda line, val: val + [(int(line.split()[1]), line.split()[2], float(line.split()[3].replace("d", "e")))]),
...
class SpeciesDefaults:
    ...
    prodbas_acc: float
        Technical cutoff criterion for on-site orthonormalization of auxiliary radial functions.
    max_l_prodbas: int
        Specifies the maximal angular quantum number for the auxiliary (product) basis function. 
        Any possible auxiliary basis with an angular momentum higher than max_l_prodbas is excluded.
    max_n_prodbas: int
    ...
    def __post_init__(self):
        ...
        for key, val in asdict(self).items():
            if key in VALID_INPUTS:
                if val not in VALID_INPUTS[key]:
                    message += (
                        f"The input for key {key} is invalid. "
    ...
    def content(self):
        ...
        for key in (
            ...
            dct.pop(key)
        file_contents = [
            f"#     Suggested defaults for {self.label} atom",
        ...
        for key, val in dct.items():
            ...
            if key == "plus_u":
                for pu in val:
                    file_contents.append(f"\n    {key:<10s}{' '.join([str(v) for v in pu])}")
            elif isinstance(val, tuple):
                file_contents.append(f"\n  {key:<22s}{' '.join([str(v) for v in val])}")
            else:
                file_contents.append(f"\n  {key:<22s}{val}")
    ...
    def from_strings(cls, config_str: list[str]) -> "SpeciesDefaults":
        ...
        Parameters:
            config_str: a list of strings from the species' defaults.
        """
        parameters = {}
        ...
        for il, line in enumerate(lines):
            ...
            if (len(split_line) <= 1) and (split_line[0] == "#"):
                continue
            key = split_line[0] if split_line[0] != "#" else f"{split_line[0]} {split_line[1]}"
