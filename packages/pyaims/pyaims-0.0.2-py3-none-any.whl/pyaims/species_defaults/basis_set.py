...
from pyaims.species_defaults.basis_function import BasisFunction, BASIS_TYPES

from pyaims.utils import is_number_string
...
class BasisSet:
    ...
    def from_string(cls, string: str) -> "BasisSet":
        ...
        for line in lines:
            ...
            if line.startswith("#"):
                ...
                if not any([basis_type in line for basis_type in BASIS_TYPES]) or is_number_string(line):
                    continue
            ...
            if is_gaussian and is_number_string(line):
                ...
                basis_set[tier-1].append(BasisFunction.from_string("\n".join(basis_lines)))
    ...
    def tier(self, n: int, enabled: bool | None = None, aux: bool = False) -> list[BasisFunction]:
        """Returns all basis functions pertaining to the current tier.
        ...
        enabled: bool | None
            A flag indicating whether only enabled or only disabled functions should be returned.
            If None, all functions are returned.
        aux: bool
    ...
    def _switch_tier(
        ...
        """ A helper function that switches the basis functions in the tier to `enable` state.
        
        Parameters
        ----------
        tier: int
            Ordinal number of the tier you want to activate, First -> 1, Second -> 2, etc.
        n: int | str
        ...
        if isinstance(n, int) and n > len(self.tier(tier, enabled=not enable, aux=aux)):
            warn(f"Requesting to {'de' if not enable else ''}activate "
    ...
    def activate_tier(self, tier: int, n: int | str = "all", aux: bool = False) -> None:
        """Moves basis functions in a specified tier from inactive to active.
        ...
        tier: int
            Ordinal number of the tier you want to activate, First -> 1, Second -> 2, etc.
        n: int | str
            The number of functions you want to activate in that tier (all for entire)
        aux: bool
            the flag indicating if auxiliary functions should be activated.
        """
        self._switch_tier(tier, n, aux, enable=True)

    def deactivate_tier(self, tier: int, n: int | str = "all", aux: bool = False) -> None:
        """Moves basis functions in a specified tier from active to inactive.

        Parameters
        ----------
        tier: int
            Ordinal number of the tier you want to deactivate, First -> 1, Second -> 2, etc.
        n: int | str
            The number of functions you want to deactivate in that tier (all for entire)
        aux: bool
            the flag indicating if auxiliary functions should be activated.
        """
    ...
    def _switch_functions(
        ...
        """ A helper function that switches the basis functions (disregarding the tier) to `enable` state.
        ...
        enable: bool
            A flag indicating in which state should the basis functions be.
        """
        valid_functions = sum([len(self.tier(i+1, not enable, aux)) for i in range(self.n_tiers)])
        ...
        for tier in range(1, self.n_tiers + 1):
            ...
            self._switch_tier(tier, valid_fs_tier, aux, enable)
            if valid_fs_tier < n:
                n -= valid_fs_tier
            else:
                return
    ...
    def activate_functions(self, n: int | str = "all", aux: bool = False) -> None:
        """Moves basis functions from inactive to active.
    ...
    def deactivate_functions(self, n: int | str = "all", aux: bool = False) -> None:
        """Moves basis functions from active to inactive.
