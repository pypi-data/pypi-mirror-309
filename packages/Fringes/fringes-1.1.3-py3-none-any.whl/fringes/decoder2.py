...
def decode(
    ...
    for d in range(D):
        ...
        m, lcm = moduli(l[d], v[d])
        gcd = np.mean(l[d] / m)  # each l_i should be identical; we average to get more precision
            ...
            for y in nb.prange(Y):
                ...
                for c in nb.prange(C):
                    ...
                    if K == 1:
                        ...
                        #     # which means that the measurement noise
                        #     # should not cause any p_ to cross an INT boundary.
