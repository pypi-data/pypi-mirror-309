...
class Task:
    ...
    def launch(self):
        ...
        while i < len(self.options.docker_command):
            ...
            i += 1
        if parent_config:
            lps, src_lang, tgt_lang = get_lps_from_config(parent_config, src_lang, tgt_lang, lps)
        ...
        while i < len(self.options.docker_command):
            ...
            # get config if present to validate it against schema
                ...
                lps, src_lang, tgt_lang = get_lps_from_config(config, src_lang, tgt_lang, lps)
                i += 1
...
def get_lps_from_config(config, src_lang, tgt_lang, lps):
    ...
    if src_lang == 'xx' or tgt_lang == 'yy':
        if "default_lp" in config:
            ...
            if lp in lps:
                ...
                raise RuntimeError(f"default_lp {lp} is not supported")
