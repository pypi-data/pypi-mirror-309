...
from bibdivers.bibGlobal import glb
...
class Modell():
    ...
    def save(self):
        """save - Schreibt den Datensatz in die DB (insert oder update, je nach id)
            ...
            Schreibt den Datensatz in die DB (insert oder update, je nach id)
            und liefert als Ergebnis im Falle des Erfolges None, sonst
        ...
        with glb.DB.cursor() as Cur:
            ...
            if self.id is None or self.id == '':
                ...
                try:
                    ...
                    glb.DB.commit()
                    Ergebnis = 'Datensatz erfolgreich angelegt mit id = {}.'.format(self.id)
                ...
                try:
                    ...
                    glb.DB.commit()
                    Ergebnis = 'Datensatz erfolgreich gespeichert.'
                except psycopg2.Error as Fehler:
                    Ergebnis = DB_Fehler(Fehler)
                    glb.DB.rollback()
                except:
    ...
    def FactorySaverValues(self):
        """FactorySaverValues - Liefert Saver für Dictionary der Werte
            ...
            Der Saver gibt eine lesbare Nachricht (Flash in Anlehnung an Flask)
            über den Erfolg des INSERT/UPDATE zurück.
