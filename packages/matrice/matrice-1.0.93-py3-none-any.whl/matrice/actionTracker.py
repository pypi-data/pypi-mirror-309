...
class ActionTracker:
        ...
        try:
            ...
            if status.lower() == 'success':
                if self.action_type == 'model_train':
                    batch_params = {k: v for k, v in self.job_params.items() if 'batch' in k.lower()} # TODO: check if this is correct
                    batch_size = next(iter(batch_params.values()), 1)  # Get first batch-related value, or default to 1
                    self.save_benchmark_results(latency_ms=(sum(self.epochs_times) / len(self.epochs_times) ) * 1000, batch_size=batch_size)
                elif self.action_type != 'deploy_add':
    ...
    def save_benchmark_results(self, latency_ms, batch_size=1):  # TODO: get the values from the action details and test the function
        ...
        payload = {
            ...
            "exportFormat": self.action_details.get("runtimeFramework", ""), # TODO: update as required to handle all actions
