...
class Parameter:
    ...
    def __check_root(self, root: str) -> Path:
        ...
        if r := self.__check_root_again(r):
            ...
            return r
        self.logger.warning(
            f"root 参数 {root} 不是有效的文件夹路径，程序将使用项目根路径作为储存路径"
        )
        return self.ROOT
    ...
    def __check_split(self, split: str) -> str:
        for i in split:
            if i in self.CLEANER.rule.keys():
                self.logger.warning(
                    f"split 参数 {split} 包含非法字符，程序将使用默认值：-"
                )
                return "-"
    ...
    def __check_proxy(self, proxy: str | dict, url="https://www.baidu.com/") -> dict:
        ...
        if isinstance(proxy, str):
            ...
            self.logger.warning(
                f"proxy 参数 {proxy} 设置错误，程序将不会使用代理",
            )
            return {"proxies": self.NO_PROXY}
        try:
            response = get(
    ...
    def __check_max_retry(self, max_retry: int) -> int:
        if isinstance(max_retry, int) and max_retry >= 0:
            ...
            return max_retry
        self.logger.warning(
            f"max_retry 参数 {max_retry} 设置错误，程序将使用默认值：5",
        )
        return 5
    ...
    async def __update_cookie(
        self, headers: dict, cookie: dict, cache: str, tiktok=False
    ) -> None:
        if cookie:
