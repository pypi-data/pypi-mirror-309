...
from mlops7sem_hw2.config import MODELS_DIR, PROCESSED_DATA_DIR, load_config
...
X_test, y_test = pd.read_csv(PROCESSED_DATA_DIR / "X_test.csv"), pd.read_csv(
...
def test_generate_train_dataset():
    ...
    assert len(dataset[0]) == config["n_samples"]
    assert len(dataset[1]) == config["n_samples"]
...
def test_train_process():
    assert train_process(config["model"], X_test, y_test, config["model_parameters"]) is not None

    X, y = generate_train_dataset(n_samples=100, n_features=5)
...
def test_logistic_regression():
    ...
    assert round(f1_score(y_test, y_pred, average=average), 3) == 0.588
    assert round(precision_score(y_test, y_pred, average=average), 3) == 0.614
    assert round(recall_score(y_test, y_pred, average=average), 3) == 0.605
...
def test_decision_tree():
    ...
    assert round(f1_score(y_test, y_pred, average=average), 3) == 0.856
    assert round(precision_score(y_test, y_pred, average=average), 3) == 0.861
    assert round(recall_score(y_test, y_pred, average=average), 3) == 0.855
...
def test_random_forest():
    ...
    assert round(accuracy_score(y_test, y_pred), 3) == 0.855
    assert round(f1_score(y_test, y_pred, average=average), 3) == 0.855
    assert round(precision_score(y_test, y_pred, average=average), 3) == 0.866
    assert round(recall_score(y_test, y_pred, average=average), 3) == 0.855
...
def test_make_prediction_print():
    ...
    with patch("builtins.print") as mock_print:
        make_prediction(n_classes, y_test, y_pred, config["catalog_name"] + config["metrics_path"])
