    ...
    for d in data:
        ...
        _colorize(
            ...
            "lifecycleStatus",
            {
                ...
                "MAINTENANCE": "yellow",
                "CUSTOMIZING": "blue",
                "AGENT_UPDATING": "yellow",
                "AGENT_REINSTALLING": "yellow",
            },
        )
...
def list_vms(
    ...
    **kwargs,
    ...
    if template_id and template_id.lower() == "all":
        ...
        def filter_template(t):
            if cloud:
                s = t.get("providerLabel")
                if not s or s.lower() not in cloud:
                    return False
            if type:
                s = t.get("templateType")
                if not s or s.lower() not in type:
                    return False
            return True

        templates = admin.template.list(org_id=org_id, fn_filter=filter_template, **kwargs)
