...
class WkfContext(WorkflowContext):
  ...
  async def call(self, pipe: Inputtable[A, B], x: A, /) -> B:
    if self.step < len(self.states):
      ...
      #   self.ctx.log(f'Possibly impure workflow. At step {self.step}, calling "{pipe.id}": expected "{param}" but got "{x}"', level='WARNING')
      ...
      param = pipe.input_adapter.dump_python(x)
      with Session(self.ctx.db.engine) as s:
        s.add(PipeInp(key=f'{self.step}_{self.key}', value=x, output=self.output))
        s.add(self.State(key=self.key, step=self.step, param=param, pipeline=pipe.id))
        s.commit()
