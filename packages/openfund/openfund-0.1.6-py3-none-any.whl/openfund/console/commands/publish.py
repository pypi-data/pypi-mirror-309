...
"""
    ...
    loggers = ["openfund.publishing.publisher"]

    def handle(self) -> int:
        from openfund.publishing.publisher import Publisher

        if not self.openfund.is_package_mode:
            self.line_error("Publishing a package is not possible in non-package mode.")
            return 1

        dist_dir = self.option("dist-dir")

        publisher = Publisher(self.openfund, self.io, Path(dist_dir))
