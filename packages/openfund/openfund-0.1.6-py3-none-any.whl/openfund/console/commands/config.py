...
from openfund.config.config import PackageFilterPolicy
from openfund.config.config import boolean_normalizer
from openfund.config.config import boolean_validator
from openfund.config.config import int_normalizer
from openfund.console.commands.command import Command


if TYPE_CHECKING:
    from openfund.config.config_source import ConfigSource
...
To remove a repository (repo is a short alias for repositories):
    ...
    def handle(self) -> int:
        ...
        from openfund.config.file_config_source import FileConfigSource
        ...
        print("----------------------- config_file %s " % config_file.path.resolve())
        print("----------------------- Path.cwd() %s " % Path.cwd())

        try:
            local_config_file = TOMLFile(
                self.openfund.file.path.parent / "openfund.toml"
            )

            print(
                "----------------------- local_config_file %s "
                % self.openfund.file.path.parent
            )

            if local_config_file.exists():
                config.merge(local_config_file.read())
        except (RuntimeError, PyProjectException):
            local_config_file = TOMLFile(Path.cwd() / "openfund.toml")
        ...
        if m:
            from openfund.utils.password_manager import PasswordManager
