...
from openfund.console.commands.add import AddCommand
from openfund.console.commands.self.self_command import SelfCommand


class SelfAddCommand(SelfCommand, AddCommand):
    name = "self add"
    description = "Add additional packages to Openfund's runtime environment."
    options = [
        o
    ...
    help = f"""\
The <c1>self add</c1> command installs additional packages to Openfund's runtime \
environment.
