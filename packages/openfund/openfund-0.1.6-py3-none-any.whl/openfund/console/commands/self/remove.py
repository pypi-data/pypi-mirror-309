...
from openfund.console.commands.remove import RemoveCommand
from openfund.console.commands.self.self_command import SelfCommand


class SelfRemoveCommand(SelfCommand, RemoveCommand):
    name = "self remove"
    description = "Remove additional packages from Openfund's runtime environment."
    options = [o for o in RemoveCommand.options if o.name in {"dry-run"}]
    help = f"""\
The <c1>self remove</c1> command removes additional package's to Openfund's runtime \
environment.
