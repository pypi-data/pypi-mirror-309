...
pub fn powershap_statistical_analysis(
    ...
    let random_uniform_feature = df.column("random_uniform_feature").unwrap();
    // Handle both f32 and f64 types
    let random_uniform_series = match random_uniform_feature.dtype() {
        DataType::Float32 => random_uniform_feature.cast(&DataType::Float64).unwrap(),
        _ => random_uniform_feature.clone(),
    };
    let random_uniform_f64 = random_uniform_series.f64().unwrap();
    let mean_random_uniform = random_uniform_f64.mean().unwrap();
    let std_random_uniform = random_uniform_f64.std(1).unwrap();

    let results: Vec<(f64, f64, f64, f64, f64)> = (0..n_columns).into_par_iter().map(|i| {
        let col = &df.get_columns()[i];
        // Handle both f32 and f64 types
        let col_f64 = match col.dtype() {
            DataType::Float32 => col.cast(&DataType::Float64).unwrap(),
            _ => col.clone(),
        };
        let col_series = col_f64.f64().unwrap();
        let col_mean = col_series.mean().unwrap();
        let col_std = col_series.std(1).unwrap();
        
        // Calculate p-value
        let col_values: Vec<f64> = col_series.into_iter().filter_map(|x| x).collect();
