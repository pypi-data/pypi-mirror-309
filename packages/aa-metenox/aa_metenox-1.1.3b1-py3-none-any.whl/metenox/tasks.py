...
@shared_task
def load_region_moons(region_id: int):
    """
    Will scan the region to have all the available non highsec moons.
    If they are not present yet in the database they will be created and can be found with the 0 isk value
    """
    logger.info("Starting to load moons for region id %s", region_id)
    ...
    for constellation in region.eve_constellations.all():
        for solar_system in constellation.eve_solarsystems.all():
            if not solar_system.is_high_sec:
                system = esi.client.Universe.get_universe_systems_system_id(
                    system_id=solar_system.id,
                ).result()
                # eve_solar_system, _ = EveSolarSystem.objects.update_or_create_esi(id=solar_system.id,
                for planet in system["planets"]:
                    # for planet in eve_solar_system.eve_planets.all():
                    # eve_planet, _ = EvePlanet.objects.update_or_create_esi(id=planet.id, include_children=True,
                    #                                                        enabled_sections=[EvePlanet.Section.MOONS])
                    if planet["moons"]:
                        for moon_id in planet["moons"]:
                            # for moon in eve_planet.eve_moons.all():
                            moon, _ = EveMoon.objects.get_or_create_esi(id=moon_id)
                            try:
                                Moon.objects.get(eve_moon=moon)
                            except Moon.DoesNotExist:
                                Moon.objects.create(eve_moon=moon)
